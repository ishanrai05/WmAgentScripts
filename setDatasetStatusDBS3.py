...
def setStatusDBS3(url3, dataset3, newStatus, files):
    ...
    try:
        dbsapi.updateDatasetType(
            dataset=dataset3,
            dataset_access_type=newStatus)
    except Exception as ex:
...
def main():
    ...
    parser.add_option(
        ...
        dest='dsets',
        default=None,
        help='file with the list of dataset names, or comma separated list of dataset')
    parser.add_option(
        '-s',
        '--status',
        dest='status',
        default=None,
        help='This will be the new status of the dataset/files')
    parser.add_option(
        '-f',
        '--files',
        action="store_true",
        default=False,
        dest='files',
        help='Validate or invalidate all files in dataset')
