...
try:
    ...
    print(
        "Error importing jira: {}\nDoing the following commands: \n\t{}\n\t{}".format(
            str(e),
            cmd1,
            cmd2))
    cmd1out, cmd1err = Popen(
        cmd1, shell=True, stderr=PIPE, stdout=PIPE).communicate()
    print(cmd1out)
    print(cmd1err)
    cmd2out, cmd2err = Popen(
        cmd2, shell=True, stderr=PIPE, stdout=PIPE).communicate()
    print(cmd2out)
    print(cmd2err)
...
class JIRAClient:
    ...
    def last_time(self, j):
        try:
            ...
            j = self.get(j)
        last_comment_time = self.time_to_time(j.fields.comment.comments[-1].updated) if (
            hasattr(j.fields, 'comment') and j.fields.comment.comments) else self.created(j)
    ...
    def find(self, specifications):
        ...
        summary = specifications.get(
            'prepid', specifications.get(
                'summary', None))
        if summary:
...
if __name__ == "__main__":
    ...
    print [time.asctime(time.gmtime(JC.created(io)))
           for io in sorted(ii, key=lambda o:JC.created(o))]
