...
class WorkflowWithInput(Workflow):
    ...
    def __init__(self, name, url='cmsweb.cern.ch', workflow=None):
        ...
        for li in self.inputlists:
            ...
            if not isinstance(self.info[li], list):
                # single element
...
def getWorkflowInfo(url, workflow):
    ...
    while retries:
        try:
            request = requestManagerGet(
                url, '/reqmgr2/data/request?name=' + workflow)
...
def outputdatasetsWorkflow(url, workflow):
    ...
    """
    results = requestManagerGet(
        url,
        '/reqmgr2/data/request?name=' +
        workflow)['result']
...
def getInputEvents(url, workflow):
    ...
    if requestType == 'ReReco':
        ...
        if request['BlockBlacklist']:
            events = (
                ...
                dbs3.getEventCountDataSet(
                    inputDataSet,
                    request['BlockBlacklist']))
            return events
        # same if a run whitelist
        if request['RunWhitelist']:
            events = dbs3.getEventCountDataSetRunList(
                inputDataSet, request['RunWhitelist'])
            return events
    ...
    if request['BlockBlacklist']:
        events = events - \
            dbs3.getEventCountDataSetBlockList(inputDataSet, request['BlockBlacklist'])
...
def getInputLumis(url, workflow):
    ...
    if requestType == 'ReReco':
        ...
        if request['BlockBlacklist']:
            lumis = (
                totalLumis -
                dbs3.getLumiCountDataSetBlockList(
                    inputDataSet,
                    request['BlockBlacklist']))
            return lumis
        # same if a run whitelist
        if request['RunWhitelist']:
            lumis = dbs3.getLumiCountDataSetRunList(
                inputDataSet, request['RunWhitelist'])
            return lumis
        # otherwize, the full lumi count
        else:
    ...
    # if white list, only the ones in the whitelist.
    if request['RunWhitelist']:
        lumis = totalLumis.getLumiCountDataSetRunList(
            inputDataSet, request['RunWhitelist'])
    # if white list of blocks
    if request['BlockWhitelist']:
        lumis = dbs3.getLumiCountDataSetBlockList(
...
def assignWorkflow(url, workflowname, team, parameters):
    ...
    if not set(assignWorkflow.mandatories).issubset(set(parameters.keys())):
        ...
        assignWorkflow.errorMessage += str(
            list(set(assignWorkflow.mandatories) - set(parameters.keys())))
        ...
        if aux in defaults:
            ...
            if aux == 'EventsPerJob':
                ...
                params.update({"requestName": workflowname,
                               ...
                               "splittingAlgo": par})
                # swap values
                if "avg_events_per_job" in params and "events_per_job" not in params:
    ...
    if 'execute' not in defaults or not defaults['execute']:
...
assignWorkflow.defaults = {
    ...
    # 44H watchdog, regardless of the requested time ...
    "SoftTimeout": 159600,
    "GracePeriod": 300,
...
assignWorkflow.keys = assignWorkflow.mandatories + \
    assignWorkflow.defaults.keys() + assignWorkflow.auxiliaries
...
def changePriorityWorkflow(url, workflowname, priority):
    ...
    if isRequestMgr2Request(url, workflowname):
        ...
        data = requestManagerPut(
            url, "/reqmgr2/data/request/%s" %
            workflowname, params)
    else:
        params = {
            workflowname +
...
def closeOutWorkflow(url, workflowname, cascade=False):
    ...
    if isRequestMgr2Request(url, workflowname):
        ...
        try:
            return None if (
                json.loads(data)['result'][0][workflowname] == 'OK') else "Error"
        except BaseException:
            return "Error"
    else:
        if cascade:
...
def cloneWorkflow(url, workflowname, overrideArgs=None):
    ...
    if isRequestMgr2Request(url, workflowname):
        data = requestManagerPost(
            url, "/reqmgr2/data/request/clone/%s" %
            workflowname, overrideArgs)
    else:
...
def reqmgr2_to_1_Splitting(params):
    if len(params) > 1:
        print "cannot set multiple splitting in reqmgr1 at once : truncating. please migrate."
    params = params[0]
    reqmgr1Params = {}
...
def setWorkflowSplitting(url, workflowname, schema):
    ...
    def ifOldSchema(schema):
        return(not isinstance(schema, list))
    ...
    if isRequestMgr2Request(url, workflowname):
        ...
        data = requestManager1Post(
            url, "/reqmgr/view/handleSplittingPage", schema)
    #print data
...
def setWorkflowAssignment(url, workflowname, schema):
    ...
    if isRequestMgr2Request(url, workflowname):
        ...
        data = requestManagerPut(
            ...
            workflowname, schema)
        try:
            ok = any(ro[workflowname] == 'OK' for ro in json.loads(
                data)['result'] if workflowname in ro)
            return ok
        except BaseException:
            print data
        ...
        data = requestManager1Post(
            url,
            "/reqmgr/assign/handleAssignmentPage",
            schema,
            nested=True)
        if 'Assigned' in data:
            return True
...
def getInputEventsTaskChain(request):
    ...
    if 'InputDataset' in request['Task1']:
        ...
        if blockWhitelist:
            return dbs3.getEventCountDataSetBlockList(
                inputDataSet, blockWhitelist)
        if blockBlacklist:
            return dbs3.getEventCountDataSet(
                inputDataSet) - dbs3.getEventCountDataSetBlockList(inputDataSet, blockBlacklist)
        if runWhitelist:
...
if __name__ == "__main__":
    ...
    parser.add_option(
        '-j',
        '--json',
        help='A json file with "createRequest" or just a request schema')
    parser.add_option(
        '--testbed',
        help='Create in testbed',
        action='store_true',
        default=False,
        dest='testbed')
    parser.add_option(
        '--test',
        help='Just print the schema',
        default=False,
        action='store_true')
    parser.add_option(
        '--no_approve',
        help='In case you do not want the workflow in assignment-approved right away',
        default=False,
        action='store_true')
