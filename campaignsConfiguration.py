...
def parseArgs():
    ...
    parser = argparse.ArgumentParser(
        description="Script to manage Campaign configurations")
    parser.add_argument(
        '-d',
        '--dump',
        default=None,
        help="dump the whole content in this file")
    parser.add_argument(
        '-l',
        '--load',
        default=None,
        help="synchronize the db with the content of this file")
    parser.add_argument(
        '-n',
        '--name',
        help="a campaign name to be viewed/added/updated")
    parser.add_argument(
        '-r',
        '--remove',
        default=False,
        action='store_true',
        help="remove the specified campaign")
    parser.add_argument(
        '-c',
        '--configuration',
...
def replaceCampaigns(campaigns):
    ...
    for rec in data:
        ...
        if not updateCampaignConfig(rec):
            print(
                "FAILED to update campaign: %s. Full content was: %s" %
                (campName, rec))
        else:
            print(
                "Campaign '%s' successfully updated in central CouchDB" %
                campName)
...
def createCampaign(content):
    ...
    for rec in data:
        ...
        if not updateCampaignConfig(rec):
            print(
                "FAILED to create campaign: %s. Full content was: %s" %
                (campName, rec))
        else:
            print(
                "Campaign '%s' successfully created in central CouchDB" %
                campName)
