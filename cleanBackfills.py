...
def transitiontime(wf, status):
    logs = filter(
        lambda change: change["Status"] == status,
...
for checkin, (warn, timeout) in delays.items():
    ...
    for wf in wfs:
        if 'backfill' not in wf['RequestName'].lower():
            continue
        ...
        if transition and (now - transition) > (timeout * 24 * 60 * 60):
            ...
            reqMgrClient.invalidateWorkflow(
                url, wf['RequestName'], current_status=wf['RequestStatus'])
...
for who in warnings:
    sendEmail(
        'Old Backfill in the system',
        'The following backfill should be removed or moved to rejected/announced\n\n%s' %
        ('\n'.join(
            sorted(
                warnings[who]))),
        destination=[
