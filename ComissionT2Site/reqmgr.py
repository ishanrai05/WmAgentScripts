...
class RESTClient(object):
    ...
    def __init__(self, url, cert=None, key=None):
        ...
        if url.startswith("https://"):
            logging.info(
                "Using HTTPS protocol, getting user identity files ...")
            proxyFile = "/tmp/x509up_u%s" % os.getuid()
            if not os.path.exists(proxyFile):
                proxyFile = "UNDEFINED"
            certFile = cert or os.getenv(
                "X509_USER_CERT", os.getenv(
                    "X509_USER_PROXY", proxyFile))
            ...
            self.conn = HTTPSConnection(
                url, key_file=keyFile, cert_file=certFile)
...
class ReqMgrClient(RESTClient):
    ...
    def _createRequestViaRest(self, requestArgs):
        ...
        """
        logging.info(
            "Injecting a request for arguments (REST API):\n%s ..." %
            requestArgs["createRequest"])
        jsonArgs = json.dumps(requestArgs["createRequest"])
        status, data = self.httpRequest(
            "PUT", "/reqmgr/reqMgr/request", data=jsonArgs)
        if status > 216:
    ...
    def _createRequestViaWebPage(self, requestArgs):
        ...
        logging.info(
            "Injecting a request for arguments (webpage):\n%s ..." %
            requestArgs["createRequest"])
    ...
    def createRequest(self, config, restApi=True):
        ...
        if config.assignRequests or config.changeSplitting:
            # if --assignRequests or --changeSplitting at the same time, it
            # will be checking requestNames
    ...
    def approveRequest(self, requestName):
        ...
        status, data = self.httpRequest(
            "PUT", "/reqmgr/reqMgr/request", data=encodedParams, headers=self.textHeaders)
        if status != 200:
    ...
    def queryRequests(self, config, toQuery=None):
        ...
        if requestsToQuery:
            ...
            # returns data on requests in the same order as in the
            # config.requestNames
            return requestsData
        else:
    ...
    def deleteRequests(self, config):
        for requestName in config.requestNames:
            ...
            status, data = self.httpRequest(
                "DELETE", "/reqmgr/reqMgr/request/%s" %
                requestName)
            if status != 200:
                print data
                sys.exit(1)
    ...
    def cloneRequest(self, config):
        ...
        status, data = self.httpRequest(
            "PUT", "/reqmgr/reqMgr/clone/%s" %
            requestName, headers=headers)
        if status > 216:
            logging.error("Error occurred, exit.")
            print data
            sys.exit(1)
    ...
    def changePriority(self, requestName, priority):
        ...
        """
        logging.info(
            "Changing request priority: %s for %s ..." %
            (priority, requestName))
        ...
        encodedParams = urllib.urlencode(params)
        status, data = self.httpRequest(
            "PUT", "/reqmgr/reqMgr/request/%s" %
            requestName, data=encodedParams, headers=self.textHeaders)
        if status > 200:
            logging.error("Error occurred, exit.")
            print data
            sys.exit(1)
    ...
    def testResubmission(self, config):
        ...
        resubmissionArgs = {
            "createRequest": {
                ...
                "InitialTaskPath": "/" +
                originalRequest +
                "/Production/ProductionMergeRAWSIMoutput",
    ...
    def checkOracleCouchDbConsistency(self, config, testRequestName):
        ...
        couchDbConn, uri = self.getCouchDbConnectionAndUri(config)
        status, data = couchDbConn.httpRequest(
            "GET", uri + "/" + testRequestName)
        ...
        for field in fields:
            ...
            if not check(reqOracle, field, "Oracle"):
                continue
            if not check(reqCouch, field, "CouchDB"):
                continue
    ...
    def checkCouchRequestFields(self, config, requestName):
        ...
        for arg in optionalArgs:
            try:
                ...
                print (
                    "Request %s doesn't have optional parameter defined: %s" %
                    (requestName, arg))
    ...
    def allTests(self, config):
        ...
        # save the first created request name (testRequestName) for some later
        # tests
        ...
        testRequestData = self.queryRequests(
            config, toQuery=testRequestName)[0]
        ...
        clonedRequest = self.queryRequests(
            config, toQuery=clonedRequestName)[0]
        msg = (
            "Priorities don't match: original request: %s cloned request: %s" %
        ...
        assert testRequestData.get(
            "DbsUrl") == clonedRequest.get("DbsUrl"), msg

        # test Resubmission request, only if we have MonteCarlo request
        # template in input
        if config.requestArgs["createRequest"]["RequestType"] == "MonteCarlo":
            logging.info(
                "MonteCarlo request in input detected, running Resubmission test ...")
        ...
        logging.info(
            "%s requests in the system before this test." %
            len(currentRequests))
...
def processCmdLine(args):
    ...
    if opts.assignRequests and not opts.createRequest and not opts.requestNames:
        errExit(
            ...
            parser)
    if not opts.requestNames and (
        opts.queryRequests or opts.deleteRequests or (
    ...
    if (opts.json and not opts.createRequest) and (opts.json and not opts.allTests) and (
            opts.json and not opts.assignRequests) and (opts.json and not opts.changeSplitting):
        errExit(
    ...
    for action in filter(lambda name: getattr(opts, name), actions):
        if opts.allTests and action and action != "allTests":
            errExit(
                "Arguments --allTests and --%s mutually exclusive." %
                action, parser)
...
def defineCmdLineOptions(parser):
    ...
    # -i --------------------------------------------------------------------
    help = (
        "Action: Create and inject a request. Whichever from the config "
    ...
    # -s --------------------------------------------------------------------
    help = (
        "Action: Approve and assign request(s) specified by --requestNames "
...
def processRequestArgs(intputConfigFile, commandLineJson):
    ...
    if commandLineJson:
        ...
        # if a key exists in cliJson, update values in the main requestArgs
        # dict
        ...
        logging.warn(
            "No request arguments to override (--json)? Some values will be wrong.")
...
def main():
    ...
    # config is all options for this script but also request creation
    # parameters
