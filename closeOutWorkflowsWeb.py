...
def printTableHeaderWeb(title, output):
    ...
    output.write(
        '<tr><th>Request</th><th>OutputDataSet</th><th>%Compl</th>'
...
def main():
    ...
    print '| Request' + (' ' * 74) + '| OutputDataSet' + \
        (' ' * 86) + '|%Compl|Dupl|Tran|Subscr|ClosOu|'
    print '-' * 220

    printTableHeaderWeb('ReReco', output)
    noSiteWorkflows = closeOutReRecoWorkflowsWeb(
        url, workflowsCompleted['ReReco'], output)
