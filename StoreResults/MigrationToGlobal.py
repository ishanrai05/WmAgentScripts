...
class MigrationToGlobal:
    ...
    def updateRequest(self, over):
        ...
        for task in self.migrationRequests.keys():
            ...
            if self.migrationRequests[task]['status'] == 'submitted':
                request_status = self.dbsApi.statusMigration(
                    migration_rqst_id=self.migrationRequests[task]['id'])
                status = request_status[0]['migration_status']
                if status == 2:  # Migration completed
                    self.migrationRequests[task]['status'] = 'successful'
                    print 'Migration to global succeed: ' + \
                        self.migrationRequests[task]['dataset']
                elif status == 9:  # Migration failed
                    self.migrationRequests[task]['status'] = 'migration failed'
                    print 'Migration to global fail: ' + \
                        self.migrationRequests[task]['dataset']
                elif status == 3 and over:  # Migration failed, no more retries due to script timeout
                    self.removeRequest(self.migrationRequests[task])
                    self.migrationRequests[task]['status'] = 'migration failed'
                    print 'Migration to global fail: ' + \
                        self.migrationRequests[task]['dataset']
                elif status == 0 and over:  # Migration is still pending, remove it
                    self.removeRequest(self.migrationRequests[task])
                    self.migrationRequests[task]['status'] = 'migration not processed'
                    print 'Migration to global not proccesed: ' + \
                        self.migrationRequests[task]['dataset']
                elif status == 1 and over:  # Migration in progress...
                    self.migrationRequests[task]['status'] = 'still processing'
                    print 'DBS3 is still processing Migration of %s' % self.migrationRequests[
                        task]['id']
    ...
    def createReport(self):
        ...
        for task in self.migrationRequests.keys():
            ...
            print "%20s %15s %25s %150s" % (task,
                                            self.migrationRequests[task]['id'],
    ...
    def Migrates(self, reportSet):
        ...
        for task in self.migrationRequests.keys():
            ...
            try:
                ...
                print 'Migrate: from url ' + \
                    request['object']['migration_url'] + ' dataset: ' + request['object']['migration_input']
                migration_request = self.dbsApi.submitMigration(
                    request['object'])
