...
def get_command_line_options():
    parser = OptionParser(
        usage='%prog --dataset=</specify/dataset/path> --status=<newStatus> --url=<DBS_Instance_URL> + optional options')
    parser.add_option(
        "-u",
        "--url",
        dest="url",
        help="DBS Instance url",
        metavar="DBS_Instance_URL",
        default=DEFAULT_PROD_DBS3)
    parser.add_option(
        "-r",
        "--recursive",
        dest="recursive",
        help="Invalidate all children datasets,too?",
        metavar="True/False")
    parser.add_option(
        "-d",
        "--dataset",
        dest="dataset",
        help="Dataset to change status",
        metavar="/specify/dataset/path")
    parser.add_option(
        "-s",
        "--status",
        dest="new_status",
        help="New status of the dataset",
        metavar="newStatus")
    parser.add_option(
        "-v",
        "--verbose",
    ...
    parser.add_option(
        ...
        "--proxy",
        dest="proxy",
        help="Use Socks5 proxy to connect to server",
        metavar="socks5://127.0.0.1:1234")
    (options, args) = parser.parse_args()

    if not (
            options.url and options.dataset and options.new_status and options.recursive):
        parser.print_help()
        parser.error(
            'Mandatory options are --dataset, --status, --url and --recursive')
...
def list_dataset_children(dataset):
    for child_dataset in api.listDatasetChildren(dataset=dataset):
        logging.debug(
            'Found children dataset %s' %
            (child_dataset['child_dataset']))
        for grand_child in list_dataset_children(
                dataset=child_dataset['child_dataset']):
            yield grand_child
...
def update_dataset_type(dataset, new_status):
    logging.debug(
        'Update dataset type for dataset %s to %s' %
        (dataset, new_status))
...
def update_file_status(dataset, new_status):
    ...
    file_status = (1, 0)[options.new_status in [
        'DELETED', 'DEPRECATED', 'INVALID']]

    for this_file in files:
        logging.debug(
            'Update file status for file %s to status %s' %
            (this_file['logical_file_name'], file_status))
