...
parser.add_option(
    ...
    default=False,
    action='store_true',
    help='Look for exsiting recovery json')
parser.add_option(
    '--norecovery',
    default=False,
    action='store_true',
    help='Do not run the recovery process')

parser.add_option(
    '--nocreation',
    default=False,
    action='store_true',
    help='Do not inject new request')
parser.add_option(
    '--assignoptions',
    default=def_assignoptions,
    help='The options to pass to assign.py (default is %s)' %
    def_assignoptions)
parser.add_option(
    '--workflows',
    help='Coma separated list of workflows to recover')
...
for wf in wfs:
    ...
    if options.existing:
        ...
        com = 'python recoverMissingLumis.py -q %s -g DATAOPS -f -r %s' % (
            os.getenv('USER'), wf)
        print com
    ...
    if options.nocreation:
        ...
        familly = filter(
            lambda r: r['RequestStatus'] == 'assignment-approved' and 'recovery' in r['RequestName'],
            getWorkflowById(
                reqmgr_url,
                pid,
                details=True))
        ...
        for json in json_files:
            ...
            print com
            y = (raw_input('go ?')
                 if not options.dry else 'dry') if not options.go else 'y'
            if y.lower() in ['y', 'yes', 'go', 'ok']:
                go = os.popen(com)
                full_log = go.read().split('\n')
                wf_created = None
