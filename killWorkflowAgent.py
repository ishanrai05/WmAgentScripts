...
def killWorkflowAgent(WorkflowName):
    ...
    # take wf from args in case no elements exist for workflow (i.e. work was
    # negotiating)
    requestNames = set([x['RequestName'] for x in elements]
                       ) | set([wf for wf in [WorkflowName]])
    if not requestNames:
