...
class dynamoClient(object):
    ...
    def get_site(self, s):
        ...
        os.system(
            'cd %s ; wget -q -N http://dynamo.mit.edu/consistency/%s_protected.db ; cd -' %
            (d, s))
        os.system(
            "touch -t `date +'%%Y%%m%%d%%H%%M.%%S'` %s" %
            self._local_file(s))
    ...
    def files_in_dir(self, dirs):
        ...
        for site, dirs in dirs.items():
            ...
            for dir in dirs:
                ...
                com = 'echo "SELECT file FROM files JOIN directories ON id=dir WHERE dirname=\'%s\';" | sqlite3 %s' % (
                    dir, self._local_file(site))
                print "com:", com
...
if __name__ == "__main__":
    DC = dynamoClient()
    res = DC.files_in_dir({"T2_US_Wisconsin": [
                          "/store/unmerged/PhaseIISummer17wmLHEGENOnly/TTTT_TuneCUETP8M2_14TeV-madgraph-pythia8/GEN/93X_upgrade2023_realistic_v5_ext1-v3/60001"]})
