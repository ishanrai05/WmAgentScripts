...
# The following groups should be updated according to
...
def siteName(candidate):
    ...
    if re.compile(regexp).match(candidate) is not None:
        return True
    else:
        return False
...
def jobType(id, schedd, typeToExtract):
    ...
    if schedd in relvalAgents:
        ...
        type = 'Processing'
        jobs_failedTypeLogic[id] = dict(
            scheduler=schedd, BaseType=typeToExtract)
    return type
...
def relativePending(siteToExtract):
    ...
    # if total = 0, it means that there is not available slots for any site,
    # set the same for all sites
    if total == 0.0:
...
def getJobsOveralls():
    ...
    for site in jobCounting.keys():
        ...
        for schedd in jobCounting[site].keys():
            ...
            if schedd not in totalByServer.keys():
...
def createReports(currTime):
    ...
    for status in ['Running', 'Pending']:
        ...
        # Fill output files with all the SITES info. Also print out reports to
        # stdout
        for site in sites:
            site_line = "| %25s |" % site
            for type in jobTypes:
        ...
        # Fill output files with all the SERVERS info. Also print out reports
        # to stdout
        print aux_line, '\n', title_line, '\n', aux_line
...
def main():
    ...
    for collector_name in all_collectors:
        ...
        for schedd_name in schedds:
            ...
            # Only care about production Schedds
            if schedds[schedd_name]['schedd_type'] != 'prodschedd':
                continue

            print "INFO: Getting jobs from collector: %s scheduler: %s" % (
                collector_name, schedd_name)
            ...
            for job in jobs:
                ...
                siteToExtract = str(
                    job['DESIRED_Sites']).replace(
                    ' ', '').split(",")

                if schedd_name in relvalAgents:  # If RelVal job
                    type = 'RelVal'
                # If PromptReco job (Otherwise type is Processing)
                elif task == 'Reco':
                    type = 'Reco'
                ...
                if collector_name == tier0_pool[0]:
                    if 'T2_CH_CERN_T0' not in siteToExtract:
                        continue
                ...
                if status == 2:  # Running
                    ...
                    pendingCache.append(
                        [schedd_name, type, cpus, siteToExtract])
    ...
    for job in pendingCache:
        ...
        for penSite in siteToExtract:
            # calculate relative pending weight
            relative_pending = relative[penSite] / total
    ...
    if jobs_failedTypeLogic != {}:
        ...
        send_mail(mailingSender,
                  ...
                  body_text)
        print 'ERROR: I find jobs that failed the type assignment logic, I will send an email to: %s' % str(
            mailingList)
