...
for p in ['/usr/lib64/python2.7/site-packages',
...
dbs_url_writer = os.getenv('UNIFIED_DBS3_WRITER',
...
unified_url = os.getenv(
    'UNIFIED_URL',
    'https://cms-unified.web.cern.ch/cms-unified/')
...
unified_pub_url = os.getenv(
    'UNIFIED_URL',
    'https://cms-unified.web.cern.ch/cms-unified/public/')
...
def _searchLog(q, actor, limit, conn, prefix, h=None):
    ...
    goodquery = {"query": {"bool": {"must": [{"wildcard": {"meta": "*%s*" % q}}]}}, "sort": [
        {"timestamp": "desc"}], "_source": ["text", "subject", "date", "meta"]}
...
def es_header():
    entrypointname, password = open(
        'Unified/secret_es.txt').readline().split(':')
    import base64
...
def _try_sendLog(
    ...
    if wfi:
        ...
        out = filter(lambda d: not any(
            [c in d for c in ['FAKE', 'None']]), wfi.request['OutputDatasets'])
        if out:
...
def check_ggus(ticket):
    ...
    r1 = conn.request(
        "GET",
        '/index.php?mode=ticket_info&ticket_id=%s&writeFormat=XML' %
        ticket)
    r2 = conn.getresponse()
    print r2
    return False
    ...
    return list(
        itertools.chain.from_iterable(
            [
                (subitem['name'],
                 item['requested_by'],
                 item['id']) for subitem in item['node'] if subitem['decision'] == 'pending'] for item in items))
...
def listSubscriptions(url, dataset, within_sites=None):
    ...
    r1 = conn.request(
        "GET",
        '/phedex/datasvc/json/prod/requestlist?dataset=%s' %
        (dataset))
    r2 = conn.getresponse()
    result = json.loads(r2.read())
    items = result['phedex']['request']
    ...
    for item in items:
        for node in item['node']:
            site = node['name']
            if item['type'] == 'delete' and node['decision'] in [
                    'approved', 'pending']:
    ...
    for item in items:
        for node in item['node']:
            if item['type'] != 'xfer':
                continue
            site = node['name']
            if within_sites and site not in within_sites:
                continue
            #print item
            if 'MSS' not in site:
                # pending delete
                if site in deletes and not deletes[site]:
                    continue
                # delete after transfer
                #print  node['time_decided'],site
                if site in deletes and deletes[site] > node['time_decided']:
                    continue

                destinations[site] = (
                    item['id'], node['decision'] == 'approved')
...
class UnifiedLock:
    ...
    def deadlock(self):
        ...
        from assignSession import session, LockOfLock
        to_remove = []
        for ll in session.query(LockOfLock).filter(
                LockOfLock.lock).filter(
                LockOfLock.owner.contains(host)).all():
            print ll.owner
            try:
                host, pid = ll.owner.split('-')
                process = os.popen(
                    'ps -e -f | grep %s | grep -v grep' %
                    pid).read()
                if not process:
    ...
    def clean(self):
        ...
        for ll in session.query(LockOfLock).all():
            # one needs to go and check the process on the corresponding
            # machine
            pass
        session.commit()
    ...
    def release(self):
        from assignSession import session, LockOfLock
        for ll in session.query(LockOfLock).filter(
                LockOfLock.owner == self.owner).all():
            ll.lock = False
...
class DynamoLock:
    ...
    def deadlock(self):
        from assignSession import session, LockOfLock
        Ulocks = session.query(LockOfLock).filter(LockOfLock.lock).all()
        if not Ulocks:
...
def _lock_DDM(owner=None, lock=True, wait=True, timeout=None):
    ...
    if lock:
        while True:
            conn.request(
                "POST",
                "/registry/applock/lock?service=%s&app=detox" %
                service)
            response = conn.getresponse()
            data = response.read()
            res = json.loads(data)
        ...
        conn.request(
            "POST",
            "/registry/applock/unlock?service=%s&app=detox" %
            service)
        response = conn.getresponse()
        data = response.read()
        res = json.loads(data)
...
class lockInfo:
    ...
    def _release(self, item):
        ...
        if not l:
            sendLog(
                'lockInfo',
                "[Release] %s to be released is not locked" %
                item)
        else:
    ...
    def items(self, locked=True):
        #from dataLock import locksession, Lock
        from assignSession import session, Lock
        ret = sorted([l.item for l in session.query(
            Lock).all() if l.lock == locked])
        return ret
...
def mongo_client():
    ...
    return pymongo.MongoClient(
        'mongodb://%s/?ssl=true' %
        mongo_db_url, ssl_cert_reqs=ssl.CERT_NONE)
...
class StartStopInfo:
    ...
    def get(self, component, metric='lap'):
        res = [oo[metric] for oo in sorted(self.db.find(
            {'component': component}), key=lambda o: o['start']) if metric in oo]
        return res
...
class unifiedConfiguration:
    def __init__(self):
        # switch to None once you want to read it from mongodb
        ...
        if self.configs is None:
            try:
                ...
                self.configs = json.loads(
                    open('unifiedConfiguration.json').read())
...
def checkDownTime():
    ...
    r1 = conn.request(
        "GET",
        '/couchdb/reqmgr_workload_cache/jbadillo_BTV-RunIISpring15MiniAODv2-00011_00081_v0__151030_162715_5312')
    r2 = conn.getresponse()
    r = r2.read()
...
class componentCheck(threading.Thread):
    def __init__(self, block=True, mcm=None, soft=None, keep_trying=False):
        ...
        self.daemon = True
        if soft is None:
            # components that are not mandatory
            self.soft = ['mcm', 'wtc', 'mongo', 'jira']
        else:
            self.soft = soft
        self.block = block
        self.status = {
    ...
    def check_cmsr(self):
        ...
        all_info = session.query(Workflow).filter(
            Workflow.name.contains('1')).all()
        ...
        for component in sorted(self.status):
            ...
            while True:
                try:
                    ...
                    print str(e)
                    if self.block and not (
                            self.soft and component in self.soft):
                        self.code = ecode
                        return False
                    break
    ...
    def tell(self, c):
        ...
        sendLog(
            'componentInfo', "The %s component is unreachable from %s" %
            (c, host), level='critical')
...
def read_file(target):
    ...
    if target.endswith('json'):
        if is_json(content):
            ...
            sendLog("eosRead", "Error reading json file {} at {}".format(
                target, datetime.datetime.now()), level='critical')
            return "{}"
    else:
        return content
    ...
    def close(self):
        ...
        while T < self.trials:
            ...
            try:
                ...
                r = os.system("eoscp %s %s" %
                              (self.cache_filename, self.eos_filename))
                if r == 0 and os.path.getsize(self.eos_filename) > 0:
                    return True
...
class campaignInfo:
        ...
        for c in self.campaigns:
            if 'parameters' in self.campaigns[c]:
                if 'SiteBlacklist' in self.campaigns[c]['parameters']:
                    for black in copy.deepcopy(
                            self.campaigns[c]['parameters']['SiteBlacklist']):
                        if black.endswith('*'):
                            self.campaigns[c]['parameters']['SiteBlacklist'].remove(
                                black)
                            reg = black[0:-1]
    ...
    def all(self, c_type=None):
        return [
            o['name'] for o in self.db.find() if (
                c_type is None or o.get(
                    'type', None) == c_type)]
    ...
    def go(self, c, s=None):
        ...
        if c in self.campaigns and self.campaigns[c]['go']:
            if 'labels' in self.campaigns[c]:
                if s is not None:
                    GO = (s in self.campaigns[c]['labels']) or any(
                        [l in s for l in self.campaigns[c]['labels']])
                else:
...
def notRunningBefore(component, time_out=60 * 5):
    ...
    while True:
        if time_out < 0:
            ...
            return False
        process_check = filter(
            None, os.popen(
                'ps -f -e | grep %s.py | grep -v grep  |grep python' %
                component).read().split('\n'))
    ...
    def __call__(self):
        ...
        while True:
            if not self.locking:
                nogo = False
                break
            # check from existing such lock, solely based on the component,
            # nothing else
...
def userLock(component=None):
    ...
    for who in lockers:
        if os.path.isfile(
            '/afs/cern.ch/user/%s/%s/public/ops/%s.lock' %
                (who[0], who, component)):
...
'""" % (
    ...
    TIMESTAMP = json.loads(
        os.popen(
            ...
            (conf["url"],
             conf["token"],
             query)).read())["responses"][0]["hits"]["hits"][0]["_source"]["metadata"]["timestamp"]
...
class ThreadHandler(threading.Thread):
    ...
    def _run(self):
        ...
        # 10 steps of eta verbosity
        bug_every = max(len(self.threads) / 10., 100.)
        ...
        while self.threads:
            ...
            if self.n_threads is None or running < self.n_threads:
                startme = self.n_threads - \
                    running if self.n_threads else len(self.threads)
    ...
    def __init__(self):
        ...
        self.cache['gwmsmon_totals'] = {
            ...
            'getter': lambda: json.loads(
                os.popen('curl --retry 5 -s http://cms-gwmsmon.cern.ch/poolview/json/totals').read()),
            'cachefile': None,
            'default': {}}
        self.cache['gwmsmon_pool'] = {
            ...
            'expiration': default_expiration(),
            'getter': lambda: json.loads(
                os.popen('curl --retry 5 -s http://cms-gwmsmon.cern.ch/poolview/json/summary').read()),
            'cachefile': None,
            'default': {}}
        self.cache['mcore_ready'] = {
            ...
            'expiration': default_expiration(),
            'getter': lambda: json.loads(
                os.popen('curl --retry 5 -s http://cmsgwms-frontend-global.cern.ch/vofrontend/stage/mcore_siteinfo.json').read()),
            'cachefile': None,
            'default': {}}
        self.cache['gwmsmon_prod_site_summary'] = {
            ...
            'expiration': default_expiration(),
            'getter': lambda: json.loads(
                os.popen('curl --retry 5 -s http://cms-gwmsmon.cern.ch/prodview//json/site_summary').read()),
            'cachefile': None,
            'default': {}}
        ...
        self.cache['gwmsmon_prod_maxused'] = {
            ...
            'expiration': default_expiration(),
            'getter': lambda: json.loads(
                os.popen('curl --retry 5 -s http://cms-gwmsmon.cern.ch/prodview//json/maxusedcpus').read()),
            'cachefile': None,
            'default': {}}
        ...
        self.cache['mss_usage'] = {
            ...
            'expiration': default_expiration(),
            'getter': lambda: json.loads(
                os.popen('curl -s --retry 5 http://cmsmonitoring.web.cern.ch/cmsmonitoring/storageoverview/latest/StorageOverview.json').read()),
            'cachefile': None,
            'default': {}}
        self.cache['hlt_cloud'] = {
            ...
            'expiration': default_expiration(),
            'getter': lambda: json.loads(
                os.popen('curl -s --retry 1 --connect-timeout 5 http://137.138.184.204/cache-manager/images/cloudStatus.json').read()),
            'cachefile': None,
            'default': {}}
        self.cache['wmstats'] = {
            'data': None,
...
def _getNodes(url, kind):
    ...
    result = json.loads(r2.read())
    return [node['name']
            for node in result['phedex']['node'] if node['kind'] == kind]
...
def getNodeUsage(url, node):
    ...
    r1 = conn.request(
        "GET",
        '/phedex/datasvc/json/prod/nodeusage?node=%s' %
        node)
    r2 = conn.getresponse()
    result = json.loads(r2.read())
    if len(result['phedex']['node']):
        s = max([sum([node[k] for k in node.keys() if k.endswith('_node_bytes')])
...
def getNodeQueue(url, node):
    ...
    r1 = conn.request(
        "GET",
        '/phedex/datasvc/json/prod/nodeusagehistory?node=%s' %
        node)
    r2 = conn.getresponse()
    result = json.loads(r2.read())
    print result
...
def getSiteStorage(url):
    ...
    r1 = conn.request(
        "GET",
        '/api/cms/site/query/?json&preset=data-processing',
        headers={
...
class DSS:
    ...
    def get_block_size(self, dataset):
        self._get(dataset)
        return sum(
            self.bdb[dataset].values()), copy.deepcopy(
            self.bdb[dataset])
...
class siteInfo:
    def __init__(self, override_good=None):
        ...
        try:
            ...
            for siteInfo in data:
                ...
                override = (
                    override_good and siteInfo['name'] in override_good)
                if siteInfo['name'] in self.sites_banned and not override:
                    continue
                if (self.sites_ready_in_agent and siteInfo['name']
                        in self.sites_ready_in_agent) or override:
            ...
            for aar in add_as_ready:
                if aar not in self.sites_ready:
                    self.sites_ready.append(aar)
                if aar not in self.all_sites:
        ...
        self.sites_eos = [
            s for s in self.sites_ready if s in [
        ...
        # or s.startswith('T0_'))] ## put the T0 in the T1 : who cares
        ...
        self.sites_with_goodAAA = list(
            set([s for s in self.sites_with_goodAAA if s in self.sites_ready]))
        ...
        for (phn, psn) in dataCache.get('site_storage'):
            ...
            if self.SE_to_CE(phn) == psn:
                continue
            if psn in ['T2_CH_CERN']:
                continue
        ...
        for mss in self.storage:
            ...
            if mss not in mss_usage['Tape'][use_field]:
                self.storage[mss] = 0
            else:
    ...
    def getRemainingDatasets(self, site):
        ...
        for line in os.popen(
            'curl -s http://t3serv001.mit.edu/~cmsprod/IntelROCCS/Detox/result/%s/RemainingDatasets.txt' %
                site).read().split("\n"):
    ...
    def fetch_glidein_info(self, talk=True):
        ...
        for site in self.sites_memory.keys():
            if site not in self.sites_ready:
    ...
    def sites_low_pressure(self, ratio):
        sites = [site for site, (matching, running, _) in self.sites_pressure.items() if (
            running == 0 or (matching / float(running)) < ratio) and site in self.sites_ready]
        return sites
    ...
    def sitesByArch(self, arch):
        ...
        for site, slots in self.sites_memory.items():
            for slot in slots:
                if slot['OS'] == 'any' or slot['SINGULARITY']:
    ...
    def sitesByMemory(self, maxMem, maxCore=1):
        ...
        for site, slots in self.sites_memory.items():
            ...
            if any([slot['MaxMemMB'] >= maxMem and slot['MaxCpus']
                    >= maxCore for slot in slots]):
                allowed.add(site)
        return list(allowed)
    ...
    def fetch_detox_info(
            self,
            talk=True,
            buffer_level=0.8,
            sites_space_override=None):
        ...
        for line in info:
            ...
            try:
                ...
                sendLog(
                    'fetch_detox_info',
                    'Unable to read Detox DataOps report',
                    level='critical')
                break
            # bypass
            ...
            queued_used = 0
            available = int(float(quota) * buffer_level) - \
                int(locked) - int(queued_used)
        ...
        for (site, info) in _info_by_site.items():
            ...
            if key_for_cpu in info and site in self.cpu_pledges and info[key_for_cpu]:
                if self.cpu_pledges[site] < info[key_for_cpu]:
                    if talk:
                        print site, "could use", info[key_for_cpu], "instead of", self.cpu_pledges[site], "for CPU"
                    self.cpu_pledges[site] = int(info[key_for_cpu])
                elif self.cpu_pledges[site] > 1.5 * info[key_for_cpu]:
                    if talk:
                        print site, "could correct", info[key_for_cpu], "instead of", self.cpu_pledges[site], "for CPU"
                    self.cpu_pledges[site] = int(info[key_for_cpu])
...
#                     self.disk[ssite] = int(info['FreeDisk'])
    ...
    def types(self):
        ...
        # return
        # ['sites_with_goodIO','sites_T1s','sites_T2s','sites_mcore_ready']#,'sites_veto_transfer']#,'sites_auto_approve']
    ...
    def CE_to_SE(self, ce):
        if (ce.startswith('T1') or ce.startswith(
                'T0')) and not ce.endswith('_Disk'):
            return ce + '_Disk'
    ...
    def pick_SE(self, sites=None, size=None):  # size needs to be in TB
        if size:
            return self._pick(sites, dict(
                [(se, free) for (se, free) in self.storage.items() if free > size]))
        else:
    ...
    def pick_CE(self, sites):
        ...
        # return
        # r_weights.keys()[self._weighted_choice_sub(r_weights.values())]
        ...
        if not site:
            ...
            try:
                ...
                try:
                    for fn in filter(
                        None, os.popen(
                            'ls -1 %s/remaining_*.json | sort ' %
                            monitor_dir).read().split('\n')):
                        site = fn.split('_', 1)[-1].split('.')[0]
                        if not any([site.endswith(v)
                                    for v in ['_MSS', '_Export']]):
                            sites.append(site)
                except BaseException:
                    pass
            for site in sites:
...
def isHEPCloudReady(url, limit=20):
    ...
    r1 = conn.request(
        "GET",
        '/reqmgr2/data/request?mask=RequestStatus&status=assigned&status=acquired&status=running-open&status=running-closed&team=hepcloud',
        headers={
            "Accept": "*/*"})
    r2 = conn.getresponse()
...
class reportInfo:
    ...
    def _put(self, updating_doc):
        ...
        if exist:
            ...
            for nested in ['tasks']:
                new_info = updating_doc.pop(
                    nested) if nested in updating_doc else {}
                for k in sorted(
                        exist.get(
                            nested,
                            {}).keys() +
...
<th>Priority</th>
    ...
    def one_line(self, wf, wfo, count):
        ...
        # order =
        # ['percentage','acdc','duplicate','correctLumis','missingSubs','dbsFiles','dbsInvFiles','phedexFiles']#,'updated']
        order = [
            'percentage',
            'acdc',
            'events',
        ...
        o_and_c = [(out, self.record[wf]['datasets'][out].get('percentage'))
        ...
        for io, out in enumerate([o[0] for o in o_and_c]):
            ...
            wf_text += '<a href="https://%s/reqmgr2/fetch?rid=%s" target="_blank">dts</a>' % (
                reqmgr_url, wf)
            wf_text += ', <a href="https://%s/reqmgr2/data/request/%s" target="_blank">wfc</a>' % (
                reqmgr_url, wf)
            ...
            if 'ReReco' in tpid:
                wf_text += ', <a href=%s/datalumi/lumi.%s.html>lumis</a>' % (
                    unified_url, tpid)
            ...
            for f in order:
                ...
                if f == 'acdc':
                    ...
                    frac = self.record[wf]['datasets'][out].get(
                        'fractionpass', 0)
                    if value >= frac:
                        text += '<td>%s</td>' % value
                    else:
                    ...
                    text += '<td>%s/%s</td>' % (display_N(
                        self.record[wf]['datasets'][out].get(
                            'producedN',
                            'NA')),
                        display_N(
                        self.record[wf]['datasets'][out].get(
                    ...
                    text += '<td>%s</td>' % value
            u_text = '<td rowspan="%d">%s</td>' % (n_out,
                                                   self.record[wf]['datasets'][out]['updated'])
            if io == 0:
                text += u_text
            p_text = '<td rowspan="%d">%s</td>' % (
                n_out, self.record[wf]['priority'])
            if io == 0:
                text += p_text
    ...
    def summary(self):
        ...
        html.write('<html>')
        html.write(
            'Last update on %s(CET), %s(GMT), <a href=logs/checkor/ target=_blank> logs</a> <br><br>' %
            (time.asctime(
                time.localtime()), time.asctime(
                time.gmtime())))
        ...
        for (count, wf) in enumerate(
                sorted([o['name'] for o in self.db.find()])):
    ...
    def save(self):
        ...
        os.system(
            'eoscp %s/closedout.json %s/closedout.json.last' %
        ...
        for wf in old:
            ...
            if wf not in self.record:
                ...
                old_ts = [dsi['timestamp']
                          for ds, dsi in old[wf].get('datasets', {}).items()]
                new_ts = [
                    dsi['timestamp'] for ds,
                    dsi in self.record[wf].get(
                        'datasets',
                        {}).items()]
                if not new_ts:
                    # weird
                    ...
                    if old_ts:
                        ...
                        if max(old_ts) > max(new_ts):
                            # the existing timestamp is bigger than the one we
                            # have :keep the old one
                            update_to_old = True
...
""" % (status, status, len(assist[status])))
            ...
            for (count, wfo) in enumerate(prio_ordered_wf):
                ...
                if wfo.name not in self.record:
...
def checkTransferApproval(url, phedexid):
    ...
    r1 = conn.request(
        "GET",
        '/phedex/datasvc/json/prod/requestlist?request=' +
        str(phedexid))
    r2 = conn.getresponse()
    result = json.loads(r2.read())
    items = result['phedex']['request']
...
def getDatasetFileArray(
    ...
    if not detail:
        ...
        all_files = [dict([(k, v) for k, v in f.items() if k in keys])
                     for f in all_files]
    return all_files
            ...
            for latency in destination['blocklatency']:
                for flatency in latency['filelatency']:
                    if not flatency['time_at_destination']:
                        msg = "{} is not coming out from {} to {} since {}. Last try at {} after {} attempts".format(
                            flatency['lfn'], flatency["from_node"], destination['name'], time.asctime(
                                time.gmtime(
                                    flatency['time_first_attempt'])), time.asctime(
                                time.gmtime(
                                    flatency['time_latest_attempt'])), flatency['attempts'])
...
def findLostBlocksFiles(url, datasetname):
    try:
        ...
        sendLog(
            'findLostBlocksFiles',
            'fatal execption in findLostBlocksFiles for %s, assuming no lost blocks and files' %
            datasetname,
            level='critical')
        return ([], [])
...
def try_findLostBlocksFiles(url, datasetname):
    ...
    r1 = conn.request(
        "GET",
        '/phedex/datasvc/json/prod/subscriptions?block=%s%%23*&collapse=n' %
        datasetname)
    r2 = conn.getresponse()
    result = json.loads(r2.read())
    lost = []
    ...
    for dataset in result['phedex']['dataset']:
        for item in dataset['block']:
            ...
            if not exist:
                ...
                # go deeper then
                r1 = conn.request(
                    "GET", '/phedex/datasvc/json/prod/filereplicas?block=%s' %
                    (item['name'].replace(
                        '#', '%23')))
                r2 = conn.getresponse()
    ...
    try:
        ...
        try:
            ...
            print "fatal execption in checkTransferLag\n", "%s\n%s\n%s" % (
                xfer_id, datasets, str(e))
            #sendEmail('fatal execption in checkTransferLag',"%s\n%s\n%s"%(xfer_id,datasets,str(e)))
            sendLog(
                'checkTransferLag', 'fatal execption in checkTransferLag for %s\n%s\n%s' %
                (xfer_id, datasets, str(e)), level='critical')
            v = {}
    return v
...
def try_checkTransferLag(url, xfer_id, datasets=None):
    ...
    subs_url = '/phedex/datasvc/json/prod/subscriptions?request=%s&create_since=%d' % (
        str(xfer_id), timecreate)
    subs_url += '&collapse=n'
    ...
    for item in result['phedex']['dataset']:
        if 'subscription' not in item:
            #print "sub"
            loop_on = list(itertools.chain.from_iterable(
                [[(subitem['name'], i) for i in subitem['subscription']] for subitem in item['block']]))
        else:
            #print "no sub"
            loop_on = [(item['name'], i) for i in item['subscription']]
        #print loop_on
        for item, sub in loop_on:
            if datasets and not any([item.startswith(ds) for ds in datasets]):
                continue
            if sub['percent_files'] != 100:
                ...
                if '#' in item:
                    item_url = '/phedex/datasvc/json/prod/blockreplicas?block=%s' % (
                        item.replace('#', '%23'))
                else:
                    item_url = '/phedex/datasvc/json/prod/blockreplicas?dataset=%s' % (
                        item)
                ...
                for subitem in item_result['phedex']['block']:
                    dones = [
                        replica['node'] for replica in subitem['replica'] if replica['complete'] == 'y']

                    block_size = max([replica['bytes']
                                      for replica in subitem['replica']])
                    ssitems = [
                        replica['bytes'] for replica in subitem['replica'] if replica['node'] == destination]
                    ...
                    if destination in dones:
                        continue
                    if not dones:
                        print "\t\t", subitem['name'], "lost"
                        stuck[ds][subitem['name']][destination] = (
                            block_size, destination_size, delay, rate, dones)
                        continue
                    if delay > 0:  # more than 7 days is way to much !!
                        print subitem['name'], ' of size', block_size_GB, '[GB] missing', block_size_GB - \
                            destination_size_GB, '[GB]'
...
def checkTransferStatus(url, xfer_id, nocollapse=False):
    try:
        ...
        try:
            ...
            sendLog(
                'checkTransferStatus', 'fatal exception in checkTransferStatus %s\n%s' %
                (xfer_id, str(e)), level='critical')
            v = {}
    return v
...
def try_checkTransferStatus(url, xfer_id, nocollapse=False):
    ...
    my_nodes = [phedex_nodes[n['name']]
                for n in result['phedex']['request'][0]['node']]
    subs_url = '/phedex/datasvc/json/prod/subscriptions?request=%s&create_since=%d&%s' % (
        str(xfer_id), timecreate, '&'.join(['node=%s' % n for n in my_nodes]))
    if nocollapse:
        subs_url += '&collapse=n'
    #print subs_url
    r1 = conn.request("GET", subs_url)
    ...
    if len(result['phedex']['dataset']) == 0:
        print "trying with an earlier date than", timecreate, "for", xfer_id
        subs_url = '/phedex/datasvc/json/prod/subscriptions?request=%s&create_since=%d&%s' % (
            str(xfer_id), 0, '&'.join(['node=%s' % n for n in my_nodes]))
        if nocollapse:
            subs_url += '&collapse=n'
        r1 = conn.request("GET", subs_url)
        r2 = conn.getresponse()
    ...
    for item in result['phedex']['dataset']:
        ...
        if 'subscription' not in item:
            loop_on = list(itertools.chain.from_iterable(
                [subitem['subscription'] for subitem in item['block']]))
        else:
        ...
        for sub in loop_on:
            ...
            if sub['percent_bytes']:
                ...
                completions[item['name']][sub['node']].append(
                    float(sub['percent_files']))
            else:
    ...
    for item in completions:
        for site in completions[item]:
            # average it !
            completions[item][site] = sum(
                completions[item][site]) / len(completions[item][site])
...
def findCustodialLocation(url, dataset, with_completion=False):
    ...
    r1 = conn.request(
        "GET",
        '/phedex/datasvc/json/prod/blockreplicas?dataset=%s' %
        (dataset))
    r2 = conn.getresponse()
    result = json.loads(r2.read())
    ...
    for block in request['block']:
        ...
        for replica in block['replica']:
            ...
            if replica['custodial'] == "y" and (
                    not with_completion or replica['complete'] == "y"):
    ...
    if with_completion and len(blocks) != len(cust_blocks):
        ...
        print "Missing", len(
            blocks - cust_blocks), "blocks out of", len(blocks)
        ...
        r1 = conn.request(
            "GET",
            '/phedex/datasvc/json/prod/requestlist?dataset=%s&node=T*MSS' %
            dataset)
        r2 = conn.getresponse()
        result = json.loads(r2.read())
        ...
        for nodes in request:
            ...
            for node in nodes['node']:
                ...
                more_information['nodes'][node['name']] = {
                    'id': nodes['id'], 'created': created, 'decided': decided}
...
def getDatasetFileLocations(url, dataset):
    ...
    r1 = conn.request(
        "GET",
        '/phedex/datasvc/json/prod/filereplicas?dataset=%s' %
        (dataset))
    r2 = conn.getresponse()
    result = json.loads(r2.read())
...
def getDatasetFiles(url, dataset, without_invalid=True):
    return runWithRetries(
        _getDatasetFiles, [
            url, dataset], {
            'without_invalid': without_invalid}, retries=5, wait=5)
...
def _getDatasetFiles(url, dataset, without_invalid=True):
    ...
    files = getDatasetFileArray(
        dataset,
        validFileOnly=without_invalid,
        detail=True)
    ...
    return dbs_filenames, phedex_filenames, list(
        set(dbs_filenames) - set(phedex_filenames)), list(
        set(phedex_filenames) - set(dbs_filenames))


def getDatasetBlocksFraction(
        url,
        dataset,
        complete='y',
        group=None,
        ...
        sites=None,
        only_blocks=None):
    return runWithRetries(_getDatasetBlocksFraction,
                          [url,
                           dataset],
                          {'complete': complete,
                           'group': group,
...
def _getDatasetBlocksFraction(
        ...
        sites=None,
        only_blocks=None):
    # count how manytimes a dataset is replicated < 100%: not all blocks >
    # 100% several copies exis
    if vetoes is None:
        ...
        for replica in item['replica']:
            if not any(replica['node'].endswith(v) for v in vetoes):
                if replica['group'] is None:
                    replica['group'] = ""
                if complete and not replica['complete'] == complete:
                    continue
                if group is not None and not replica['group'].lower(
                ) == group.lower():
                    continue
    ...
    if not len(block_counts):
        ...
        return 0
    first_order = float(
        len(block_counts) - block_counts.values().count(0)) / float(len(block_counts))
    if first_order < 1.:
        print dataset, ":not all", len(block_counts), " blocks are available, only", len(
            block_counts) - block_counts.values().count(0)
        return first_order
    else:
        second_order = sum(block_counts.values()) / float(len(block_counts))
        print dataset, ":all", len(
            block_counts), "available", second_order, "times"
        return second_order


def getBetterDatasetDestinations(
        url,
        dataset,
        only_blocks=None,
        group=None,
    ...
    if only_blocks:
        ...
        full_size = sum([block['file_size'] for block in all_blocks if (
            block['block_name'] in only_blocks)])
    else:
    ...
    for item in items:
        ...
        for node in item['node']:
            ...
            # if node['decision'] != 'approved': continue ## a delete is a
            # delete
    ...
    for item in items:
        ...
        for req in item['node']:
            if within_sites and not req['name'] in within_sites:
                continue
            if vetoes and any([req['name'].endswith(v) for v in vetoes]):
                continue
            if not req['time_decided']:
                continue
            if not req['decision'] in ['approved']:
                continue

            if req['name'] in deletes and int(
                    req['time_decided']) < deletes[req['name']]:
                continue
    ...
    if only_blocks:
        ...
        for block in only_blocks:
            try:
                url = '/phedex/datasvc/json/prod/subscriptions?block=%s&collapse=n' % (
                    block.replace('#', '%%23'))
                r1 = conn.request("GET", url)
                r2 = conn.getresponse()
                r = r2.read()
            ...
            for ds in result:
                for block in ds['block']:
                    for sub in block['subscription']:
                        ...
                        if within_sites and site not in within_sites:
                            continue
                        if vetoes and any([site.endswith(v) for v in vetoes]):
                            continue
                        sub['percent_bytes'] = sub['percent_bytes'] if sub['percent_bytes'] is not None else 0
                        destinations[site].add(
                            (block['name'], sub['percent_bytes'], sub['request']))
        pass
        ...
        r1 = conn.request(
            "GET",
            '/phedex/datasvc/json/prod/subscriptions?dataset=%s' %
            (dataset))
        r2 = conn.getresponse()
        ...
        for site in (all_destinations - in_full):
            if not site.startswith('T'):
                continue
            print "getting all b-sub for dataset", dataset, '@', site
            try:
                url = '/phedex/datasvc/json/prod/subscriptions?block=%s%%23*&node=%s&collapse=n' % (
                    dataset, site)
    ...
    for site in destinations:
        ...
        #print blocks_and_id
        completion = sum([b[1] for b in destinations[site]]
                         ) / float(len(destinations[site]))
...
def getOldDatasetDestinations(
    ...
    for item in items:
        ...
        for sub in item['subscription']:
            if not any(sub['node'].endswith(v) for v in vetoes):
                if within_sites and not sub['node'] in within_sites:
                    continue
                #if sub['group'] == None: sub['group']=""
                if sub['group'] is None:
                    sub['group'] = "DataOps"  # assume "" group is dataops
                if group is not None and not sub['group'].lower(
                ) == group.lower():
                    continue
                destinations[sub['node']].add(
                    (item['name'], int(sub['percent_bytes']), sub['request']))
    ...
    for item in items:
        ...
        for node in item['node']:
            if node['decision'] != 'approved':
                continue
            if not node['name'] in deletes or deletes[node['name']] < int(
                    node['time_decided']):
                # add it if not or later delete
    ...
    for item in items:
        ...
        for req in item['node']:
            ...
            if req['name'] in times:
                ...
                if int(req['time_decided']) > times[req['name']]:
                    # the node was already seen as a destination with an ealier
                    # time, and no delete in between
                    if req['name'] == 'T2_CH_CERN':
                        print "uhm", phedex_id
                    continue
            if req['name'] == 'T2_CH_CERN':
                print phedex_id, "is being registered for CERN destination"
            times[req['name']] = int(req['time_decided'])

            sites_destinations.append(req['name'])
        sites_destinations = [
            site for site in sites_destinations if not any(
                site.endswith(v) for v in vetoes)]
        # what are the sites for which we have missing information ?
        sites_missing_information = [
            site for site in sites_destinations if site not in destinations.keys()]
        ...
        if len(sites_missing_information) == 0 and not addhoc:
            continue

        r3 = conn.request(
            "GET",
            '/phedex/datasvc/json/prod/transferrequests?request=%s' %
            phedex_id)
        r4 = conn.getresponse()
        ...
        for req in sub_items:
            if group is not None and not req['group'].lower() == group.lower():
                continue
...
def getDatasetDestinations(
        ...
        complement=True):
    try:
        return getBetterDatasetDestinations(
            url, dataset, only_blocks, group, vetoes, within_sites, complement)
    except Exception as e:
        print "failed once"
        print str(e)
        sendLog(
            'getDatasetDestinations',
            'Failed the new implementation',
            level='critical')
        try:
            return getOldDatasetDestinations(
                url, dataset, only_blocks, group, vetoes, within_sites, complement)
        except BaseException:
...
def _getDatasetOnGoingDeletion(url, dataset):
    ...
    r1 = conn.request(
        "GET",
        '/phedex/datasvc/json/prod/deletions?dataset=%s&complete=n' %
        (dataset))
    r2 = conn.getresponse()
...
def getDatasetBlocks(dataset, runs=None, lumis=None):
    return runWithRetries(
        _getDatasetBlocks, [dataset], {
            'runs': runs, 'lumis': lumis})
...
def _getDatasetBlocks(dataset, runs=None, lumis=None):
    ...
    if lumis:
        ...
        for run in lumis:
            try:
                ...
                all_files = dbsapi.listFileArray(
                    dataset=dataset, lumi_list=lumis[run], run_num=int(run), detail=True)
            except Exception as e:
        ...
        for run in runs:
            ...
            while r > 0:
                r -= 1
                try:
                    all_blocks.update([b['block_name'] for b in dbsapi.listBlocks(
    ...
    if runs is None and lumis is None:
        all_blocks.update([item['block_name']
                           for item in dbsapi.listBlocks(dataset=dataset)])
...
def getDatasetBlockAndSite(
        url,
        dataset,
        group=None,
        vetoes=None,
        complete=None):
    return runWithRetries(try_getDatasetBlockAndSite, [url, dataset], {
                          'group': group, 'vetoes': vetoes, 'complete': complete}, default=None)


def try_getDatasetBlockAndSite(
        url,
        dataset,
        group=None,
        vetoes=None,
    ...
    for item in items:
        for replica in item['replica']:
            if complete and replica['complete'] != complete:
                continue
            if replica['group'] is None:
                replica['group'] = ""
            if group is not None and not replica['group'].lower(
                ...
                continue
            if vetoes and any([veto in replica['node'] for veto in vetoes]):
                continue
            blocks_at_sites[replica['node']].add(item['name'])
    # return dict([(site,list(blocks)) for site,blocks in
    # blocks_at_sites.items()])
...
def getDatasetPresence(
    ...
    try:
        ...
        sendLog(
            'getDatasetPresence', 'fatal exception in getDatasetPresence for %s\n%s' %
            (dataset, str(e)), level='critical')
        return {}


def try_getDatasetPresence(
        url,
        dataset,
        complete='y',
        only_blocks=None,
    ...
    for item in items:
        for replica in item['replica']:
            ...
            if not any(v in replica['node'] for v in vetoes):
                ...
                if within_sites and not replica['node'] in within_sites:
                    continue
                if replica['group'] is None:
                    replica['group'] = ""
                if complete and not replica['complete'] == complete:
    ...
    if set(all_block_names) != set(blocks_in_phedex):
        missing_in_phedex = sorted(
            set(all_block_names) -
            set(blocks_in_phedex))
        missing_in_dbs = sorted(set(blocks_in_phedex) - set(all_block_names))
        is_valid = False
        for block in missing_in_phedex:
            block_info = dbsapi.listFileSummaries(
                block_name=block, validFileOnly=1)
        ...
        missing_in_phedex = sorted(
            ...
            set(blocks_in_phedex))
        missing_in_dbs = sorted(set(blocks_in_phedex) - set(all_block_names))
        if missing_in_phedex:
            print missing_in_phedex, "missing in phedex"
        if missing_in_dbs:
            print missing_in_dbs, "missing in dbs"
    ...
    if only_blocks:
        ...
        full_size = sum([block['file_size']
                         for block in all_blocks if block['block_name'] in all_block_names])
    if not full_size:
        print dataset, "is nowhere"
        return {}
...
def getDatasetBlocksFromFiles(dataset, files):
    dbsapi = DbsApi(url=dbs_url)
    try:
        all_files = dbsapi.listFiles(
            dataset=dataset, detail=True, validFileOnly=1)
    except Exception as e:
...
def _getDatasetBlockSize(dataset):
    ...
    return dict([(block['block_name'], block['file_size'] / (1024.**3))
                 for block in blocks])
...
def getDatasetChops(
    ...
    # does a *flat* choping of the input in chunk of size less than chop
    # threshold
    dbsapi = DbsApi(url=dbs_url)
    blocks = dbsapi.listBlockSummaries(dataset=dataset, detail=True)
    ...
    if only_blocks:
        blocks = [
            block for block in blocks if block['block_name'] in only_blocks]
    ...
    if sum_all > chop_threshold:
        ...
        sizes.extend([block['file_size'] for block in filter(
            lambda b: b['file_size'] > chop_threshold, blocks)])
        small_block = filter(
            lambda b: b['file_size'] <= chop_threshold, blocks)
        small_block.sort(
            ...
            if len(sites) <= n_copies:
                ...
                for pick in range(n_copies):
                    ...
                    if chunk_size:
                        overflow = 10
                        while (
                                SI.disk[picked_site_se] *
                                1024.) < chunk_size and overflow > 0:
                            overflow -= 1
                            picked_site = SI.pick_CE(list(set(sites) - at))
                            picked_site_se = SI.CE_to_SE(picked_site)
                        if overflow < 0:
                            ...
                            # should I crash ?
                            print picked_site_se, "has", SI.disk[picked_site_se] * \
                                1024., "GB free, while trying to put an item of size", chunk_size, " that is", json.dumps(item, indent=2)
                            sendEmail(
                                'possibly going over quota',
                                'We have a potential over quota usage while distributing \n%s check transferor logs' %
                                (json.dumps(
                                    item,
                                    indent=2)))
                    at.add(picked_site)
...
def getFilesWithLumiInRun(dataset, run):
    ...
    start = time.mktime(time.gmtime())
    try:
        reply = dbsapi.listFiles(
            dataset=dataset,
            detail=True,
            run_num=run,
            validFileOnly=1) if run != 1 else dbsapi.listFiles(
            dataset=dataset,
            detail=True,
            validFileOnly=1)
    except BaseException:
        try:
            ...
            sendLog(
                'getFilesWithLumiInRun',
                'Fatal exception in running dbsapi.listFiles for %s %s ' %
                (dataset,
                 run),
                level='critical')
            reply = []
    ...
    while True:
        ...
        if len(these) == 0:
            break
        rreply.extend(
            dbsapi.listFileLumiArray(
                logical_file_name=these,
                run_num=run) if run != 1 else dbsapi.listFileLumiArray(
                logical_file_name=these))
        start += bucket
        #print len(rreply)
    return rreply
...
class duplicateAnalyzer:
        ...
        for f1, f2d in graph.items():
            ...
            for f2 in f2d.keys():
                ...
                # some has two colors:
                    ...
                    raise Exception(
                        "Not a bipartite graph, cannot use this algorithm for removing")
                # have same color
    ...
    def files_to_remove(self, files_per_lumis):
        ...
        for rl, fns in files_per_lumis.items():
            for fn in fns:
                lumi_count_per_file[fn] += 1
        bad_lumis = dict(
            [(rl, files) for rl, files in files_per_lumis.items() if len(files) > 1])
...
def getDatasetLumisAndFiles(
    ...
    if not cached:
        ...
        # the revers dictionnary of files by r:l
        files_per_lumi = defaultdict(set)
        ...
        class getFilesFromBlock(threading.Thread):
            ...
            def run(self):
                self.res = self.a.listFileLumis(
                    block_name=self.b, validFileOnly=int(
                        not check_with_invalid_files_too))
        ...
        for t in run_rthreads.threads:
            ...
            for f in t.res:
                ...
                for lumi in f['lumi_section_num']:
                    ...
                    files_per_lumi['{}:{}'.format(f['run_num'], lumi)].add(
                        f['logical_file_name'])
    ...
    files_json = dict([(tuple(map(int, k.split(":"))), v)
                       for (k, v) in files_per_lumi.items()])
    if runs:
        lumi_json = dict([(int(k), v)
                          for (k, v) in full_lumi_json.items() if int(k) in runs])
        files_json = dict([(tuple(map(int, k.split(":"))), v) for (
            k, v) in files_per_lumi.items() if int(k.split(':')[0]) in runs])
    elif lumilist:
        runs = map(int(lumilist.keys()))
        lumi_json = dict([(int(k), v)
                          for (k, v) in full_lumi_json.items() if int(k) in runs])
        files_json = dict([(tuple(map(int, k.split(":"))), v) for (
            k, v) in files_per_lumi.items() if map(int, k.split(":")) in lumis])

    return lumi_json, files_json


def getDatasetLumis(dataset, runs=None, with_cache=False):
    l, f = getDatasetLumisAndFiles(
        dataset, runs=runs, lumilist=None, with_cache=with_cache)
    return l
...
def invalidateFiles(files):
    ...
    for fn in files:
        ...
        try:
            r1 = conn.request(
                "GET",
                "/registry/invalidation/invalidate?item=%s" %
                (fn))
            r2 = conn.getresponse()
...
def setFileStatus(file_names, validate=True):
    ...
    for fn in files:
        ...
        if status != validate:
            ...
            dbswrite.updateFileStatus(
                logical_file_name=fn['logical_file_name'],
    ...
    while new_status != status:
        ...
        if withFiles and file_status != -1:
            ...
            for this_file in files:
                dbswrite.updateFileStatus(
                    logical_file_name=this_file['logical_file_name'],
...
def updateSubscription(
    ...
    response = phedexPost(
        url,
        "/phedex/datasvc/json/prod/updatesubscription",
        params)
    #print response
    return response
...
def allCompleteToAnaOps(url, dataset):
    ...
    for dataset in subs['dataset']:
        for sub in dataset['subscription']:
            if sub['group'] == 'DataOps' and sub['percent_bytes'] == 100 and not any(
                    [v in sub['node'] for v in ['MSS', 'Export', 'Buffer']]):
                print dataset['name'], "on", sub['node'], "changed to Anaops"
                updateSubscription(
                    reqmgr_url,
                    sub['node'],
                    dataset['name'],
                    user_group='AnalysisOps')
...
def _getWorkflowByInput(url, dataset, details=False):
    conn = make_x509_conn(url)
    #conn  =  httplib.HTTPSConnection(url, cert_file = os.getenv('X509_USER_PROXY'), key_file = os.getenv('X509_USER_PROXY'))
    there = '/couchdb/reqmgr_workload_cache/_design/ReqMgr/_view/byinputdataset?key="%s"' % (
        dataset)
    if details:
        there += '&include_docs=true'
    r1 = conn.request("GET", there)
...
def getWorkflowByOutput(url, dataset, details=False):
    conn = make_x509_conn(url)
    #conn  =  httplib.HTTPSConnection(url, cert_file = os.getenv('X509_USER_PROXY'), key_file = os.getenv('X509_USER_PROXY'))
    there = '/couchdb/reqmgr_workload_cache/_design/ReqMgr/_view/byoutputdataset?key="%s"' % (
        dataset)
    if details:
        there += '&include_docs=true'
    r1 = conn.request("GET", there)
...
def getLatestMCPileup(url, statuses=None):
    ...
    for status in statuses:
        ...
        #conn  =  httplib.HTTPSConnection(url, cert_file = os.getenv('X509_USER_PROXY'), key_file = os.getenv('X509_USER_PROXY'))
        r1 = conn.request(
            "GET", '/reqmgr2/data/request?mask=RequestDate&mask=MCPileup&status=%s' %
            status, headers={
                "Accept": "application/json"})
        r2 = conn.getresponse()
        data = json.loads(r2.read())
        ...
        for v in req.values():
            ...
            if t and len(d) == 6:
                d = time.mktime(time.strptime(
                    "-".join(map(lambda n: "%02d" % int(n), d)), "%Y-%m-%d-%H-%M-%S"))
                for tt in t:
                    those[tt].add(d)
    ret = {}
...
def _getWorkflowByMCPileup(url, dataset, details=False):
    conn = make_x509_conn(url)
    #conn  =  httplib.HTTPSConnection(url, cert_file = os.getenv('X509_USER_PROXY'), key_file = os.getenv('X509_USER_PROXY'))
    there = '/couchdb/reqmgr_workload_cache/_design/ReqMgr/_view/bymcpileup?key="%s"' % (
        dataset)
    if details:
        there += '&include_docs=true'
    r1 = conn.request("GET", there)
...
def getWorkflowById(url, pid, details=False):
    conn = make_x509_conn(url)
    #conn  =  httplib.HTTPSConnection(url, cert_file = os.getenv('X509_USER_PROXY'), key_file = os.getenv('X509_USER_PROXY'))
    there = '/couchdb/reqmgr_workload_cache/_design/ReqMgr/_view/byprepid?key="%s"' % (
        pid)
    if details:
        there += '&include_docs=true'
    r1 = conn.request("GET", there)
...
def forceComplete(url, wfi):
    ...
    for member in familly:
        ...
        # if member['RequestName'] == wl['RequestName']: continue ## set
        # himself out
        ...
        if member['RequestStatus'] in ['running-open', 'running-closed']:
            ...
            reqMgrClient.invalidateWorkflow(
                url, member['RequestName'], current_status=member['RequestStatus'])
...
class agentInfo:
    def __init__(self, **args):
        ...
        self.speed_draining_fraction = args.get(
            'speed_draining_fraction', 0.05)
    ...
    def checkTrello(self, sync_trello=None, sync_agents=None, acting=False):
        ...
        for agent in self.all_agents:
            ...
            if lid and clid and lid != clid:
                ...
                if sync_trello or (sync_trello and agent in sync_trello):
                    print "changing", agent, "into list", lid, lid_name
                    if acting:
                        tc.changeList(cn=agent, ln=lid)
                if sync_agents or (sync_agents and agent in sync_agents):
    ...
    def getStatus(self):
        ...
        if not all_agents_prod:
            ...
            return False
        prod_info = dict([(a['agent_url'].split(':')[0], a)
                          for a in all_agents_prod])
        all_agents_name = sorted(set(self.all_agents + prod_info.keys()))
        # do you want to use this to make an alarm on agent in error for too
        # long ?
        self.in_error = [
            a['agent_url'].split(':')[0] for a in all_agents_prod if a.get(
                'down_components', [])]
        ...
        for agent in all_agents_name:
            ...
            if linfo:
                ...
                # the agent is new here. let's assume it is in standby if in
                # drain
                st = 'running'
                if pinfo['drain_mode']:
                    st = 'standby'
    ...
    def flag_standby(self, agent):
        if self._getA(agent).get('status', None) == 'draining':
            ...
            print "not changing status from", self._getA(
                agent).get('status', None)
    ...
    def poll(self, wake_up_draining=False, acting=False, verbose=False):
        ...
        for agent in self.buckets.get('running', []):
            ...
            if (now - agent_update) < (last_action_timeout):
                ...
                timeout_for_running = last_action_timeout - \
                    (now - agent_update)
                if timeout_last_running is None or timeout_last_running < timeout_for_running:
        ...
        for agent in self.buckets.get('standby', []):
            ...
            if (now - agent_update) < (last_action_timeout):
                ...
                timeout_for_standby = last_action_timeout - \
                    (now - agent_update)
                if timeout_last_standby is None or timeout_last_standby < timeout_for_standby:
        ...
        for agent in self.buckets.get('draining', []):
            ...
            if (now - agent_update) < (last_action_timeout):
                ...
                timeout_for_draining = last_action_timeout - \
                    (now - agent_update)
                if timeout_last_draining is None or timeout_last_draining < timeout_for_draining:
                    timeout_last_draining = timeout_for_draining
            if (now - agent_update) < (last_action_static):
                recent_draining.add(agent)

        timeout_action = max(
            [timeout_last_running, timeout_last_standby, timeout_last_draining])
        ...
        rel_num = [
            (r, map(
                lambda frag: int(
                    frag.replace(
                        'patch', '')), r.split('.'))) for r in self.release.keys()]
        rel_num = sorted(rel_num, key=lambda o: o[1], reverse=True)
        m_rel_num = [
            (r, map(
                lambda frag: int(
                    frag.replace(
                        'patch', '')), r.split('.'))) for r in self.m_release.keys()]
        ...
        for agent, ainfo in all_agents.items():
            ...
            if agent_name in drainings:
                if agent_name not in self.release[oldest_release]:
                    # you can candidate those not running the oldest release,
                    # and in drain
                    if not stuffed and not light:
                        candidates_to_wakeup.add(agent_name)
                # if len(standbies)<=1 and (cp <=
                # cpu_running*self.speed_draining_fraction) and (r <=
                # t*self.speed_draining_fraction) and (cr <=
                # cpu_running*self.speed_draining_fraction):
                if (cp <= cpu_running *
                    self.speed_draining_fraction) and (r <= t *
                                                       self.speed_draining_fraction) and (cr <= cpu_running *
                                                                                          self.speed_draining_fraction):
                    speed_draining.add(agent_name)
                if len(standbies) <= 1 and (
                        r + p <= self.open_draining_threshold):
            ...
            if agent_name in runnings:
                ...
                # use the major release number to select agents to drain: i.e.
                # ignore patches versions.
                elif agent_name not in self.m_release[top_m_release]:
        ...
        candidates_to_wakeup = candidates_to_wakeup - \
            recent_draining - recent_standby - set(self.in_error)
        ...
        if verbose or verbose or True:
            ...
            print "These are candidates for draining", sorted(
                candidates_to_drain)
        ...
        release_deploy = ((running_top_release >= 2) or (
            standby_top_release >= 2)) and running_old_release
        ...
        if not (one_recent_running or one_recent_draining or one_recent_standby):
            ...
            print "An agent was recently put in running/draining/standby. Cannot do any further acting for another %s last running or %s last standby" % (
                display_time(timeout_action), display_time(timeout_last_standby))
        ...
        if need_one:
            ...
            if not pick_from:
                if wake_up_draining:
                    ...
                    # order by the metric
                    pick_from = [o[0] for o in sorted([(a, m) for (
                        a, m) in wake_up_metric if a in pick_from], key=lambda o:o[1], reverse=True)]
            ...
            if not pick_from:
                ...
                if wake_up in speed_draining:
                    speed_draining.remove(wake_up)
                if wake_up in open_draining:
                    open_draining.remove(wake_up)
        ...
        if (already_speed_drain & speed_draining):
            ...
            print sorted(
                already_speed_drain), "already in speed draining. not changing this"
        ...
        if fully_empty:
            msg = 'These agents are fully empty %s and ready for redeploy' % sorted(
                fully_empty)
            #sendLog('agentInfo',msg, level='critical')
        ...
        # should update trello with the agents that got manipulated at this
        # time
...
def getWorkflowsByName(url, names, details=False):
    return runWithRetries(_getWorkflowsByName, [url, names], {
                          'details': details}, retries=5, wait=5)
...
def _getWorkflowsByName(url, names, details=False):
    ...
    print "%d retrieved for %d workflow names with details: %s" % (
        len(workflows), len(names), details)
    return workflows
...
def getWorkflows(
        ...
        rtype=None,
        priority=None):
    return runWithRetries(try_getWorkflows,
                          [url,
                           status],
                          {'user': user,
                           'details': details,
    ...
    while tries < retries:
        ...
        try:
            ...
            message = "Failed to run function {} with arguments {} {} for {}/{} times and {}[s] wait (%s). Exception\n {}\n {}".format(
                glb_fcn.__name__, str(fcn_pargs), str(fcn_args), tries, retries, wait, time.asctime(), str(e), tb)
            print (message)
...
def checkIfBlockIsSubscribedToASite(url, block, site):
    ...
    r1 = conn.request(
        "GET",
        '/phedex/datasvc/json/prod/subscriptions?block=' +
        block.replace(
            '#',
            '%23'))
...
def checkIfDatasetIsSubscribedToASite(url, dataset, site):
    ...
    r1 = conn.request(
        "GET",
        '/phedex/datasvc/json/prod/subscriptions?dataset=' +
        dataset)
    r2 = conn.getresponse()
    result = json.loads(r2.read())
    ...
    if len(result['phedex']['dataset']) != 1:
        os.system(
            'echo ' +
            dataset +
            ' | mail -s \"utils.py error 1\" andrew.m.levin@vanderbilt.edu')
        sys.exit(1)
...
def closeAllBlocks(url, dataset, blocks=None):
    ...
    r1 = conn.request(
        "GET",
        '/phedex/datasvc/json/prod/blockreplicas?dataset=%s&complete=n' %
        dataset)
    r2 = conn.getresponse()
    result = json.loads(r2.read()).get('phedex', {})
    for block in result.get('block', []):
        bname = block.get('name')
        if blocks and bname not in blocks:
            continue
        if block.get('is_open') == 'n':
            continue
        sites = list()
...
def checkIfBlockIsAtASite(url, block, site):
    ...
    r1 = conn.request(
        "GET",
        '/phedex/datasvc/json/prod/blockreplicasummary?block=' +
        block.replace(
            '#',
            '%23'))
    r2 = conn.getresponse()
...
class workflowInfo:
    def __init__(
        ...
        if request is None:
            try:
                ...
                try:
                    r1 = self.conn.request(
                        "GET", '/couchdb/reqmgr_workload_cache/' + workflow)
                    # r1=self.conn.request("GET",'/reqmgr2/data/request/'+workflow,
                    # headers={"Accept":"*/*"}) ## new
                    r2 = self.conn.getresponse()
                    ret = json.loads(r2.read())
                    # ret = ret['result'][0][workflow]## new
                    ...
                    print str(e)
                    raise Exception(
                        "Failed to get workload cache for %s" %
                        workflow)
        else:
    ...
    def getFamilly(self, details=True, only_resub=False, and_self=False):
        familly = getWorkflowById(
            self.url, self.request['PrepID'], details=True)
        true_familly = []
        for member in familly:
            acdc = member['RequestType'] == 'Resubmission'
            myself = member['RequestName'] == self.request['RequestName']
            if member['RequestDate'] < self.request['RequestDate']:
                continue
            if member['RequestStatus'] in ['None', None]:
                continue

            if (myself and and_self) or (only_resub and acdc and not myself) or (
                    not only_resub and not myself):
                if details:
    ...
    def isGoodForNERSC(self, no_step=False):
        ...
        good = (self.request['RequestType'] == 'StepChain' or no_step) and self.request['RequestPriority'] <= 85000 and len(
            set(self.request['ScramArch']) & nersc_archs) >= 1
    ...
    def isGoodToConvertToStepChain(
            self,
            keywords=None,
            talk=False,
            debug=False):
        ...
        try:
            ...
            for i, info in time_info.items():
                ...
                if info['cores'] > max_ncores:
                    max_ncores = info['cores']
            if debug:
            ...
            if totalTimePerEvent > 0:
                ...
                if debug:
                    print "CPU efficiency of StepChain with %u cores: %0.1f%%" % (
                        max_ncores, efficiency * 100)
                acceptable_efficiency = efficiency > self.UC.get(
        ...
        all_tiers = map(lambda o: o.split(
            '/')[-1], self.request['OutputDatasets'])
        ...
        if keywords:
            found_in_transform_keywords = any(
                [keyword in wf for keyword in keywords])
        good = self.request['RequestType'] == 'TaskChain' and more_than_one_task and found_in_transform_keywords and single_tiers and (
    ...
    def notifyRequestor(
        ...
        try:
            ...
            self.sendLog(
                'notifyRequestor',
                'could not notify back to requestor\n%s' %
                str(e))
    ...
    def _get_spec(self):
        if not self.full_spec:
            self.conn = make_x509_conn(self.url)
            r1 = self.conn.request(
                "GET", '/couchdb/reqmgr_workload_cache/%s/spec' %
    ...
    def getWMErrors(self, cache=0):
        try:
            ...
            self.conn = make_x509_conn(self.url)
            r1 = self.conn.request(
                "GET", '/wmstatsserver/data/jobdetail/%s' %
                (self.request['RequestName']), headers={
                    "Accept": "*/*"})
            r2 = self.conn.getresponse()

            self.errors = json.loads(r2.read())[
                'result'][0][self.request['RequestName']]
            try:
                open(f_cache, 'w').write(json.dumps(
                    {'timestamp': time.mktime(time.gmtime()), 'data': self.errors}))
            except Exception as e:
    ...
    def getFullPicture(self, since=1, cache=0):
        ...
        for site in by_site:
            ...
            picture[site] = self.getDashboard(
                since, cache=cache, sortby='appexitcode', site=site)
        return picture
    ...
    def getDashboard(self, since=1, cache=0, **args):
        ...
        f_cache = '%s/%s.%s.dashb' % (cache_dir,
                                      self.request['RequestName'], hash)
        if cache:
            if os.path.isfile(f_cache):
                d_cache = json.loads(open(f_cache).read())
                now = time.mktime(time.gmtime())
                ...
                if (now - stamp) < cache and since == c_since and all(
                        [c_arg.get(k, None) == args[k] for k in args.keys()]):
        ...
        url = '/dashboard/request.py/jobsummary-plot-or-table2?' + \
            '&'.join(['%s=%s' % (k, v) for k, v in dargs.items()])
        ...
        try:
            open(f_cache, 'w').write(json.dumps({'timestamp': time.mktime(
                time.gmtime()), 'since': since, 'args': args, 'data': self.dashb}))
        except Exception as e:
        ...
        if cache:
            if os.path.isfile(f_cache):
                ...
                if (now - stamp) < cache:
                    ...
                    return self.wmstats
        r1 = self.conn.request(
            "GET", '/wmstatsserver/data/request/%s' %
            self.request['RequestName'], headers={
                "Accept": "application/json"})
        r2 = self.conn.getresponse()
        self.wmstats = json.loads(
            r2.read())['result'][0][self.request['RequestName']]
        try:
            open(f_cache, 'w').write(json.dumps(
                {'timestamp': time.mktime(time.gmtime()), 'data': self.wmstats}))
        except Exception as e:
    ...
    def getRecoveryBlocks(self, collection_name=None, for_task=None):
        ...
        for d in doc:
            ...
            if for_task and not task.endswith(for_task):
                continue
        ...
        for f in all_files:
            if not f.startswith(
                    '/store/unmerged/') and not f.startswith('MCFakeFile-'):
                if f in file_block_cache:
                    file_block = file_block_cache[f]
                else:
                    file_block = getFileBlock(f)
                    if file_block:
                        for _f in getDatasetFileArray(
                                file_block.split('#')[0], detail=True, cache_timeout=12 * 60 * 60):
                            file_block_cache[_f['logical_file_name']
                                             ] = _f['block_name']
        ...
        # skim out the files
        files_and_loc_noblock = dict(
            [(k, list(v)) for (k, v) in files_and_loc.items() if k in files_no_block])
        files_and_loc = dict(
            [(k, list(v)) for (k, v) in files_and_loc.items() if k in files_in_block])
    ...
    def getRecoveryDoc(self, collection_name=None):
        ...
        try:
            ...
            r1 = self.conn.request(
                "GET",
                '/couchdb/acdcserver/_design/ACDC/_view/byCollectionName?key="%s"&include_docs=true&reduce=false' %
                collection_name)
            r2 = self.conn.getresponse()
    ...
    def getClassicalPUOverflow(self, task):
        ...
        site_with_enough = [
            site for site,
            count in count_blocks.items() if count > 0.90 *
            max_blocks]
        SI = global_SI()
        ret = sorted(set([SI.SE_to_CE(s) for s in ret if 'Buffer' not in s]))
        inter = sorted(
            set([SI.SE_to_CE(s) for s in intersection if not ('Buffer' in s or 'MSS' in s)]))
        enough = sorted(set(
            [SI.SE_to_CE(s) for s in site_with_enough if not ('Buffer' in s or 'MSS' in s)]))
        return enough
            ...
            while TT < 5:
                ...
                try:
                    r1 = self.conn.request(
                        ...
                        self.request['RequestName'])
                    r2 = self.conn.getresponse()
                    self.workqueue = list([d['doc'] for d in json.loads(
                        r2.read())['rows'] if d['doc'] is not None])
                except Exception as e:
                    self.conn = make_x509_conn(self.url)
    ...
    def getJobs(self):
        ...
        class agentCom:
            ...
            def get(self, wfn):
                ...
                data = os.popen(
                    'ssh %s python %s/WmAgentScripts/local.py -w %s' %
                    (self.a, base_dir, wfn)).read()
    ...
    def getAgents(self):
        ...
        for status in statuses:
            ...
            for wqe in wq_s:
                active_agents[status][wqe['ChildQueueUrl']] += 1
    ...
    def getActiveAgents(self):
        ...
        for wqe in wq_running:
            active_agents[wqe['ChildQueueUrl']] += 1
            ...
            for task, errors in all_errors.items():
                ...
                for name, codes in errors.items():
                    if isinstance(codes, int):
                        continue
                    all_codes.extend([(int(code), info['jobs'], name, list(set([e['type'] for e in info['errors']])), list(
                        set([e['details'] for e in info['errors']]))) for code, info in codes.items()])
                ...
                for errorCode, njobs, name, types, details in all_codes:
                    ...
                    print ("\t\t %10d (%6s%%) failures with error code %10d (%30s) at stage %s") % (
    ...
    def getComputingTime(self, unit='h'):
        ...
        if 'InputDataset' in self.request:
            ...
            if 'BlockWhitelist' in self.request and self.request['BlockWhitelist']:
                (ne, _) = getDatasetEventsAndLumis(
                    ds, self.request['BlockWhitelist'])
            else:
                ...
                if t in self.request:
                    ...
                    if 'InputDataset' in task:
                        ...
                        if 'BlockWhitelist' in task and task['BlockWhitelist']:
                            (ne, _) = getDatasetEventsAndLumis(
                                ds, task['BlockWhitelist'])
                        else:
                            (ne, _) = getDatasetEventsAndLumis(ds)
                    elif 'Input%s' % base in task:
                        # we might have a problem with convoluted tasks, but
                        # maybe not
                    ...
                    if 'FilterEfficiency' in task:
                        carry_on[task['%sName' % base]
                                 ] *= task['FilterEfficiency']
                    cput += tpe * ne
                    #print cput,tpe,ne
                else:
                    break
            ...
            for task in splits:
                ...
                for k in ['events_per_job', 'avg_events_per_job']:
                    if k in task:
                        c_size = task[k]
                parents = filter(
                    lambda o: t.startswith(
                        o['splittingTask']) and t != o['splittingTask'],
                    splits)
                if parents:
                    for parent in parents:
                        for k in ['events_per_job', 'avg_events_per_job']:
    ...
    def heavyRead(self):
        # this is an add-hoc way of doing this. True by default. False if
        # "premix" appears in the output datasets or in the campaigns
        response = True
        if any(['premix' in c.lower() for c in self.getCampaigns()]):
            response = False
        if any(['premix' in o.lower()
                for o in self.request['OutputDatasets']]):
            response = False
        return response

    def producePremix(self):
        # Determine whether workflow is producing PREMIX
        output_datatiers = [x.split('/')[-1].lower()
                            for x in self.request['OutputDatasets']]
    ...
    def getSiteWhiteList(self, pickone=False, verbose=True):
        ...
        sites_allowed = []
        if lheinput:
            # ['T2_CH_CERN'] ## and that's it
            sites_allowed = sorted(SI.sites_eos)
        elif secondary:
            if self.heavyRead():
                sites_allowed = sorted(
                    set(SI.sites_T0s + SI.sites_T1s + SI.sites_with_goodIO))
            else:
                sites_allowed = sorted(
                    set(SI.sites_T0s + SI.sites_T1s + SI.sites_with_goodAAA))
        elif primary:
            # + SI.sites_T3s))
            sites_allowed = sorted(
                set(SI.sites_T0s + SI.sites_T1s + SI.sites_T2s))
        else:
            # no input at all
            # all site should contribute
            # + SI.sites_T3s ))
            sites_allowed = sorted(
                set(SI.sites_T0s + SI.sites_T2s + SI.sites_T1s))
        if pickone:
        ...
        (min_child_job_per_event, root_job_per_event,
         blow_up) = self.getBlowupFactors()
        UC = unifiedConfiguration()
        max_blow_up, needed_cores = UC.get('blow_up_limits')
        if blow_up > max_blow_up:
            # then restrict to only sites with >4k slots
            new_sites_allowed = list(set(sites_allowed) & set(
                [site for site in sites_allowed if SI.cpu_pledges[site] > needed_cores]))
        ...
        for campaign in self.getCampaigns():
            ...
            c_sites_allowed.extend(
                CI.parameters(campaign).get(
                    'SiteWhitelist', []))
            if c_sites_allowed:
                if verbose:
                    print "Using site whitelist restriction by campaign,", campaign, "configuration", sorted(
                        c_sites_allowed)
        ...
        if memory_allowed is not None:
            # mask to sites ready for mcore
            if verbose:
                print (
                    "[INFO] Sites allowing {} MB and {} core are".format(
                        mem, ncores, sorted(memory_allowed)))
            if ncores > 1:
                memory_allowed = list(
                    set(memory_allowed) & set(
                        SI.sites_mcore_ready))
            if verbose:
                print ("[INFO] Sites ready for multicore: {}".format(
                    sorted(list(set(SI.sites_mcore_ready)))))
            sites_removed = list(set(sites_allowed) - set(memory_allowed))
            sites_allowed = list(set(sites_allowed) & set(memory_allowed))
        print(
            "Removing sites due to {} MB and {} core(s) requirements: {}".format(
                mem, ncores, sorted(
                    sorted(sites_removed))))
        ...
        if arch_allowed is not None:
            ...
            print(
                "[INFO] Sites allowing {}: {}".format(
                    archs, sorted(
                        list(arch_allowed))))
            print "Reducing the whitelist to sites allowing", archs, ". Removing ", sorted(
                sites_removed)
    ...
    def checkSplitting(self):
        ...
        if self.request['RequestType'] == 'StepChain':
            ...
            # what you want to be on the look out is that the sum of output
            # size is not too enormeous
            # already summed up by definition of schema
            sizeperevent = self.request.get('SizePerEvent', None)
            for spl in splits:
                task = spl['splitParams']
                tname = spl['taskName'].split('/')[-1]
                avg_events_per_job = task.get('events_per_job', None)
                if avg_events_per_job and sizeperevent and (
                        avg_events_per_job *
                        sizeperevent) > (
                        GB_space_limit *
                        1024.**2):
                    print "The output size of task %s is expected to be large : %d x %.2f kB = %.2f GB > %f GB " % (
                        tname, avg_events_per_job, sizeperevent, avg_events_per_job * sizeperevent / (1024.**2), GB_space_limit)
                    avg_events_per_job_for_task = int(
                        (GB_space_limit * 1024.**2) / sizeperevent)
            ...
            for spl in splits:
                ...
                events_per_lumi_inputs = getDatasetEventsPerLumi(
                    inputs) if inputs else events_per_lumi_inputs
                ...
                # avg_events_per_job is base on 8h. we could probably put some
                # margin
                if events_per_lumi and 'events_per_job' in task:
                    ...
                    # The output size of the current task need to take into
                    # account of both
                    filter_efficiency_at_this_task = 1.
                    if 'FilterEfficiency' in t:
                        filter_efficiency_at_this_task *= t['FilterEfficiency']

                    # climb up all task to take the filter eff of the input
                    # tasks into account
                    efficiency_factor = 1.
                    ...
                    # efficiency_factor is the accummulate filter efficiency of
                    # the input tasks, not the current task
                    events_per_lumi_at_this_task = events_per_lumi * efficiency_factor
                    # This is the input events per lumi of the task, after
                    # taking filter efficiency of the previous tasks into
                    # account
                    ...
                    if timeperevent:
                        ...
                        if (time_per_input_lumi > (job_timeout * 60 * 60)):  # 45h
                            ...
                            this_max_events_per_lumi = int(
                                job_target * 60. * 60. / timeperevent)
                            # This is the possible events per lumi for this
                            # task, after taking filter efficiencies of the
                            # input tasks into account

                            max_events_per_lumi.append(
                                this_max_events_per_lumi / efficiency_factor)
                            # This is the possible events per lumi for the
                            # whole chain, before taking any filter efficiency
                            # into account
                            ...
                            msg = "The running time of task {} is expected to be too large even for one lumi section: {} x {:.2f} s = {:.2f} h > {} h. Should go as low as {}".format(
                                tname, events_per_lumi_at_this_task, timeperevent, time_per_input_lumi / (60. * 60.), job_timeout, this_max_events_per_lumi)
                            self.sendLog('assignor', msg)

                        else:
                            pass

                    # and (avg_events_per_job * sizeperevent ) >
                    # (GB_space_limit*1024.**2):
                    if sizeperevent:
                        size_per_input_lumi = events_per_lumi_at_this_task * \
                            sizeperevent * filter_efficiency_at_this_task
                        # This is the output size of this task, after taking
                        # filter efficiencies of the input task and current
                        # task into account
                        ...
                        this_max_events_per_lumi = int(
                            ...
                            sizeperevent /
                            efficiency_factor /
                            filter_efficiency_at_this_task)
                        # This is the possible events per lumi for the whole
                        # chain, before taking any filter efficiency into
                        # account

                        this_max_events_per_job = int(
                            (GB_space_limit * 1024.**2) / sizeperevent / filter_efficiency_at_this_task)
                        # This is the allowed events per job for this task, not
                        # relevant to filter efficiency of the previous task
                        ...
                        if (size_per_input_lumi > (GB_space_limit * 1024.**2)):
                            ...
                            msg = "The output size task %s is expected to be too large : %.2f GB > %f GB even for one lumi (effective lumi size is ~%d), should go as low as %d" % (
                                tname, size_per_input_lumi / (1024.**2), GB_space_limit, events_per_lumi_at_this_task, this_max_events_per_lumi)
                            self.sendLog('assignor', msg)

                            # adding this to that later on we can check and
                            # adapt the split 0
                            max_events_per_lumi.append(
                            ...
                            # should still change the avg_events_per_job
                            # setting of that task

                            msg = "The output size of task %s is expected to be too large : %d x %.2f kB * %.4f = %.2f GB > %f GB. Reducing to %d " % (
                                tname, avg_events_per_job, sizeperevent, filter_efficiency_at_this_task, avg_events_per_job * sizeperevent * filter_efficiency_at_this_task / (1024.**2), GB_space_limit, this_max_events_per_job)
                            # We don't hold the workflow here so no need to
                    ...
                    effective_output_lumi_at_this_task = events_per_lumi_at_this_task * \
                        filter_efficiency_at_this_task
                    if max_events_per_lumi:
                        effective_output_lumi_at_this_task = min(
                            events_per_lumi_at_this_task,
                            min(max_events_per_lumi)) * filter_efficiency_at_this_task

                    if (effective_output_lumi_at_this_task < min_lumi) and (not prim) and (
                            not self.isRelval()) and (not small_lumi):  # Only do this once per workflow
                        msg = "{} will get {} events per lumi in output. Smaller than {} is troublesome.".format(
                            tname, effective_output_lumi_at_this_task, min_lumi)
                        self.sendLog('assignor', msg)
                        critical_msg = msg + \
            ...
            if max_events_per_lumi:
                if events_per_lumi_inputs:
                    if min(max_events_per_lumi) < events_per_lumi_inputs:
                        # there was an input dataset somewhere and we cannot
                        # break down that lumis, except by changing to
                        # EventBased
                    ...
                    root_split = splits[0]
                    current_split = root_split.get(
                        'splitParams', {}).get(
                        'events_per_lumi', None)

                    if current_split:
                        if current_split > min(max_events_per_lumi):
                            root_split['splitParams']['events_per_lumi'] = min(
                                max_events_per_lumi)
                            modified_splits.append(root_split)

        # the return list can easily be used to call the splitting api of
        # reqmgr2
    ...
    def getSchema(self):
        ...
        #self.conn  =  httplib.HTTPSConnection(self.url, cert_file = os.getenv('X509_USER_PROXY'), key_file = os.getenv('X509_USER_PROXY'))
        r1 = self.conn.request(
            "GET", '/reqmgr2/data/request?name=%s' %
            self.request['RequestName'], headers={
                "Accept": "application/json"})
        r2 = self.conn.getresponse()
        new_schema = copy.deepcopy(json.loads(
            r2.read())['result'][0][self.request['RequestName']])

        # put in the era accordingly ## although this could be done in re-assignment
        # take care of the splitting specifications ## although this could be
        # done in re-assignment
    ...
    def getCompletionFraction(
            self,
            caller='getCompletionFraction',
            with_event=True):
        output_per_task = self.getOutputPerTask()
        task_outputs = {}
        ...
        # for all the outputs
        event_expected, lumi_expected = self.request.get(
            'TotalInputEvents', 0), self.request.get(
            'TotalInputLumis', 0)
        ...
        while True:
            ...
            if tt in self.request:
                ...
                tname_dict[tname] = tt
                if 'Input%s' % ttype not in self.request[tt] and 'RequestNumEvents' in self.request[tt]:
                    # pick up the value provided by the requester, that will
                    # work even if the filter effiency is broken
        ...
        if '%sChain' % ttype in self.request:
            ...
            while True:
                ...
                if tt in self.request:
                    ...
                    # then go back up all the way to the root task to count
                    # filter-efficiency
                    a_task = self.request[tt]
                    while 'Input%s' % ttype in a_task:
                        event_expected_per_task[tname] *= a_task.get(
                            'FilterEfficiency', 1)
        ...
        for output in self.request['OutputDatasets']:
            ...
            if lumi_expected:
                ...
                self.sendLog(
                    caller, "lumi completion %s expected %d for %s" %
                    (lumi_count, lumi_expected, output))
            output_event_expected = event_expected_per_task.get(
                task_outputs.get(output, 'NoTaskFound'))
            if output_event_expected and with_event:
                ...
                if e_fraction > percent_completions[output]:
                    ...
                    self.sendLog(
                        caller, "overiding : event completion real %s expected %s for %s" %
    ...
    def getOutputPerTask(self):
        ...
        for t in self.getWorkTasks():
            ...
            parse_what = t.subscriptions.outputModules if hasattr(
                t.subscriptions, 'outputModules') else t.subscriptions.outputSubs
            if parse_what:
    ...
    def getSplittingsNew(self, strip=False, all_tasks=False):
        ...
        splittings = []
        for spl in result:
            if not all_tasks and not spl['taskType'] in [
                    'Production', 'Processing', 'Skim']:
                continue
            if strip:
                for drop in [
                    'algorithm',
    ...
    def getSplittings(self):
        ...
        for task in self.getWorkTasks():
            ...
            translate = {
                'EventAwareLumiBased': [
                    ('events_per_job', 'avg_events_per_job')]}
            include = {
                'EventAwareLumiBased': {
                    'halt_job_on_file_boundaries_event_aware': 'True'}, 'LumiBased': {
        ...
        # this below is a functioning interface for retrieving the splitting
        # from reqmgr2 used to make the call to reqmgr2 splitting change

    def getCurrentStatus(self):
        return self.request['RequestStatus']

    def getRequestNumEvents(self):
        if 'RequestNumEvents' in self.request and int(
                self.request['RequestNumEvents']):
    ...
    def getIO(self):
        ...
        def IOforTask(blob):
            ...
            # elif 'InputDatasets' in blob: primary =
            # set(filter(None,blob['InputDatasets']))
            ...
            if 'LheInputFiles' in blob and blob['LheInputFiles'] in [
                    'True', True]:
                lhe = True
        ...
        if 'Chain' in self.request['RequestType']:
            ...
            while '%s%d' % (base, t) in self.request:
                (alhe, aprimary, aparent, asecondary) = IOforTask(
                    self.request['%s%d' % (base, t)])
                if alhe:
                    lhe = True
    ...
    def _collectinchain(self, member, func=None, default=None):
        if self.request['RequestType'] == 'StepChain':
            return self._collectin_uhm_chain(
                member, func, default, base='Step')
        elif self.request['RequestType'] == 'TaskChain':
            return self._collectin_uhm_chain(
                member, func, default, base='Task')
        else:
            raise Exception(
                "should not call _collectinchain on non-chain request")
        ...
        while '%s%d' % (base, t) in self.request:
            if member in self.request['%s%d' % (base, t)]:
                if func:
                    ...
                    coll[self.request['%s%d' %
                                      (base, t)]['%sName' %
                                                 base]] = self.request['%s%d' %
                                                                       (base, t)].get(member, default)
            t += 1
        return coll
        ...
        def invertDigits(st):
            if st[0].isdigit():
                ...
                while st[0].isdigit():
                    number += st[0]
                    st = st[1:]
                isolated = [(st[i - 1].isupper() and st[i].isupper()
                             and st[i + 1].islower()) for i in range(1, len(st) - 1)]
                if any(isolated):
    ...
    def go(self, log=False):
        ...
        # If there is 'pilot' keyword in any of processing String, just assign
        # the workflow
        ...
        if self.request['RequestType'] == 'TaskChain':
            ...
            for output in outputs:
                ...
                while True:
                    ...
                    conflicts = filter(
                        lambda wfn: wfn != self.request['RequestName'], conflicts)
                    if len(conflicts):
                        print "There is an output conflict for", self.request[
                            'RequestName'], "with", conflicts
                        # return None
                        # since we are not planned for pure extension and ever
                        # writing in the same dataset, go +1
                        version += 1
                    else:
                        break
...
def getFailedJobs(taskname, caller='getFailedJobs'):
    ...
    try:
        ...
        sendLog(
            'componentInfo',
            'not able to connect to wmstats server',
            level='critical')
        print str(e)
        return 0
    ...
    for info in reading['result']:
        if info.get('AgentJobInfo', {}) is not None:
            for agentName in info.get('AgentJobInfo', {}):
                if taskname in info['AgentJobInfo'][agentName].get(
                        "tasks", {}):
