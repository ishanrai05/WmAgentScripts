...
# def assignRequest(url, workflow, team, sites, era, procversion,
# activity, lfn, procstring, trust_site=False, replica=False,
# verbose=False, taskchain=False, trust_secondary_site=False, memory=None,
# multicore=None):
...
def assignRequest(url, **args):
    ...
    if replica:
        ...
        if taskchain:
            params['AutoApproveSubscriptionSites'] = [
                params["NonCustodialSites"]]
    if memory:
        params["Memory"] = memory if isinstance(memory, dict) else int(memory)

    if multicore:
        params["Multicore"] = multicore if isinstance(
            multicore, dict) else int(multicore)
...
def main():
    ...
    parser.add_option(
        '--special',
        help='Use it for special workflows. You also have to change the code according to the type of WF',
        dest='special')
    parser.add_option(
        '-r',
        '--replica',
    ...
    parser.add_option(
        '-p',
        '--procversion',
        help='Processing Version, if empty it will leave the processing version that comes by default in the request',
        dest='procversion')
    parser.add_option(
        '-a',
        '--activity',
        help='Dashboard Activity (reprocessing, production or test), if empty will set reprocessing as default',
        dest='activity')
    parser.add_option(
        '--xrootd',
        help='Assign with TrustSitelists=True (allows xrootd capabilities)',
        action='store_true',
        dest='xrootd')
    ...
    parser.add_option(
        '--secondary_xrootd',
        help='Assign with TrustPUSitelists=True (allows xrootd capabilities)',
        action='store_true',
        dest='secondary_xrootd')
    parser.add_option(
        '--no_secondary_xrootd',
        help='Assign with TrustPUSitelists=False',
        action='store_false',
        dest='secondary_xrootd')
    ...
    parser.add_option(
        '--test',
        action="store_true",
        help='Nothing is injected, only print infomation about workflow and Era',
        dest='test')
    parser.add_option(
        '-f',
        '--file',
        help='Text file with a list of wokflows. If this option is used, the same settings will be applied to all workflows',
        dest='file')
    parser.add_option(
        '-w',
        '--workflow',
        help='Workflow Name, or coma sperated list',
        dest='workflow')
    parser.add_option(
        '-m',
        '--memory',
        help='Set the Memory parameter to the workflow',
        dest='memory',
        default=None)
    parser.add_option(
        '--lumisperjob',
        help='Set the number of lumis per job',
        default=None,
        type=int)
    parser.add_option(
        '--maxmergeevents',
        help='Set the number of event to merge at max',
        default=None,
        type=int)
    parser.add_option(
        '-c',
        '--multicore',
        help='Set the multicore parameter to the workfllow',
        dest='multicore',
        default=None)
    ...
    # parse input workflows and files. If both -w and -f options are used,
    # then only the -w inputs are considered.
    ...
    for wfn in wfs:
        ...
        if 'OriginalRequestName' in schema:
            ...
            while ancestor_wf.request['RequestType'] == 'Resubmission':
                if 'OriginalRequestName' not in ancestor_wf.request:
                    ancestor_wf = None
                    break
                ancestor_wf = workflowInfo(
                    url, ancestor_wf.request['OriginalRequestName'])
        else:
            original_wf = None
            ancestor_wf = None
        ...
        if options.sites.lower() == 'original' and original_wf:
            ...
            print "Using", sorted(
                sites), "from the original request", original_wf.request['RequestName']
        ...
        if (schema["RequestStatus"] !=
                "assignment-approved") and not options.test:
            print(
                "The workflow '" +
                wf_name +
        ...
        # Check to see if the workflow is a task chain or an ACDC of a
        # taskchain
        taskchain = (schema["RequestType"] == "TaskChain") or (
            ancestor_wf and ancestor_wf.request["RequestType"] == "TaskChain")

        # Adding the special string - in case it was provided in the command
        # line
        if options.special:
        ...
        if (not era or not procstring) or (taskchain and (
                not isinstance(era, dict) or not isinstance(procstring, dict))):
        ...
        if options.lfn:
            ...
            sys.exit(0)
        # activity production by default for taskchains, reprocessing for
        # default by workflows
        if options.activity:
        ...
        if not is_resubmission:
            ...
            for key, value in schema.items():
                if isinstance(value, dict) and key.startswith("Task"):
            ...
            if options.sites.lower() == 'acdc':
                ...
                sites = list(
                    set([SI.SE_to_CE(site) for site in where_to_run[task]]) & set(SI.all_sites))
                print "Found", sorted(
                    sites), "as sites where to run the ACDC at, from the acdc doc of ", original_wf.request['RequestName']
            ...
            not_matching = sorted(
                (set(sites) -
                 set(memory_allowed) -
                    ...
                    set(not_existing)))
            previously_used = []
            if schema['SiteWhitelist']:
                previously_used = schema['SiteWhitelist']
            if original_wf:
                previously_used = original_wf.request['SiteWhitelist']
            if previously_used:
                not_matching = sorted(set(not_matching) & set(previously_used))
        ...
        if taskchain:
            if memory:
                ...
                tasks, set_to = memory.split(
                    ':') if ':' in memory else ("", memory)
                ...
                while True:
                    ...
                    if t in schema:
                        ...
                        if tasks and tname not in tasks:
            ...
            if multicore:
                tasks, set_to = multicore.split(
                    ':') if ':' in multicore else ("", multicore)
                tasks = tasks.split(',') if tasks else []
                set_to = int(set_to)
                multicore_dict = {}
                timeperevent_dict = {}
