...
def validateClosingWorkflow(
    ...
    for dataset in workflow.outputDatasets:
        ...
        try:
            percentage = workflow.percentageCompletion(
                dataset, skipInvalid=True)
        except Exception as e:
        ...
        try:
            if checkPhedex == 'custodial':
                phedexReqs = phedexClient.getCustodialSubscriptionRequestSite(
                    dataset)
        ...
        if ((checkEqual and percentage == closePercentage)
            ...
            if checkDuplicates:
                try:
                    duplicate = dbs3Client.duplicateRunLumi(
                        dataset, skipInvalid=True)
                except Exception:
            ...
            if (not (checkDuplicates and duplicate) and
                ...
                # if phedex check is required and has it
                    ...
                    try:
                        transPerc = phedexClient.getTransferPercentage(
                            url, dataset, phedexReqs[0])
...
def closeOutReRecoWorkflows(url, workflows):
    ...
    for wf in workflows:
        ...
        result = validateClosingWorkflow(
            url,
            workflow,
            closePercentage=1.0,
            checkEqual=True,
...
def closeOutRedigiWorkflows(url, workflows):
    ...
    for wf in workflows:
        ...
        if 'miniaod' in workflow.name:
            ...
            result = validateClosingWorkflow(
                url, workflow, 0.95, checkPhedex=False)
        else:
...
def closeOutMonterCarloRequests(url, workflows, fromGen):
    ...
    for wf in workflows:
        ...
        # check dataset health, duplicates, subscription, etc.
        result = validateClosingWorkflow(
            url, workflow, closePercentage=closePercentage)
        printResult(result)
        # if validation successful
        if result['closeOutWorkflow']:
            reqMgrClient.closeOutWorkflowCascade(url, workflow.name)
...
def validateClosingTaskChain(url, workflow):
    ...
    # if subtype doesn't come with the request, we decide based on dataset
    # names
    fromGen = False
...
def makeSubscriptions(url, workflows):
    ...
    for wf in workflows:
        ...
        if 'InputDataset' in wf.info:
            site = phedexClient.getCustodialSubscriptionRequestSite(
                wf.inputDataset)
            if not site:
                site_MSS = random.choice(T1_MSS)  # "T1_US_FNAL_MSS
            r = phedexClient.makeReplicaRequest(
                url, site_MSS, wf.outputDatasets, comments, custodial='y')
            for ds in wf.outputDatasets:
...
def main():
    ...
    noSiteWorkflows = closeOutMonterCarloRequests(
        url, workflowsCompleted['MonteCarlo'], fromGen=False)
    workflowsCompleted['NoSite-MonteCarlo'] = noSiteWorkflows
    noSiteWorkflows = closeOutMonterCarloRequests(
        url, workflowsCompleted['MonteCarloFromGEN'], fromGen=True)
