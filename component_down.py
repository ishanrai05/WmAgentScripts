...
for logpath in glob.glob('%s/*/ComponentLog' % agent_log_base_dir):
    ...
    print component
    # for grep in os.popen('grep Harness %s/%s/ComponentLog | grep Starting |
    # grep %s'%( base_dir, component,component)):
    for grep in os.popen(
        'grep terminated %s | grep %s' %
            (logpath, component)):
...
open(
    ...
    'w').write(
        json.dumps(
            ...
            indent=2))
os.system(
    "xrdcp -f /data/srv/wmagent/current/bin/*.restart.json root://eoscms.cern.ch/%s" %
    (base_eos_dir))
