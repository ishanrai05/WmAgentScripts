...
def injector(url, options, specific):
    ...
    for user in (options.user_relval.split(',')
        ...
        workflows.extend(
            getWorkflows(
                ...
                user=user,
                rtype="TaskChain"))
    for user in (options.user_storeresults.split(',')
                 if options.user_storeresults else UC.get("user_storeresults")):
        workflows.extend(
            getWorkflows(
                url,
                status=options.wmstatus,
    ...
    # browse for assignment-approved requests, browsed for ours, insert the
    # diff
    for wf in workflows:
        if specific and specific not in wf:
            continue
        ...
        if not exists:
            ...
            if not familly:
                ...
                for req_member in req_familly:
                    ...
                    owfi = workflowInfo(
                        url, req_member['RequestName'], request=req_member)
                    other_pids = owfi.getPrepIDs()
                    if set(pids) == set(other_pids):
                        # this is a real match
                        familly.extend(
                            session.query(Workflow).filter(
                                Workflow.name == req_member['RequestName']).all())

            for lwfo in familly:
                if lwfo:
                    # we have it already
                    if lwfo.status not in [
                        'forget',
                        'trouble',
                        'forget-unlock',
                            'forget-out-unlock']:
                        ...
                        sendLog(
                            'injector', "Should not put %s because of %s %s" %
                            (wf, lwfo.name, lwfo.status), level='critical')
            ...
            for d in list(prim) + list(par) + list(sec):
                ...
                if status_cache[d] != 'VALID':
                    wfi.sendLog(
                        ...
                        (d, status_cache[d]))
                    sendLog(
                        'injector', "One of the input of %s is not VALID. %s : %s" %
                        (wf, d, status_cache[d]), level='critical')
                    can_add = False
                # else:
                ...
                if not ph_files and not (
                    'StoreResults' == wfi.request.setdefault(
                        'RequestType', None)):
                    wfi.sendLog(
                        'injector',
                        "One of the input has no file in phedex: %s" %
                        d)
                    sendLog(
                        'injector',
                        "One of the input has no file in phedex: %s" %
            ...
            good_for_stepchain = wfi.isGoodToConvertToStepChain(
            ...
            if (not options.no_convert) and good_for_stepchain and not wfi.isRelval():
                ...
                wfi.sendLog(
                    'injector',
                    'Transforming %s TaskChain into StepChain' %
                    wf)
    ...
    for wf in session.query(Workflow).filter(
        ...
        print wf.name
        if specific and specific not in wf.name:
            continue
        print wf.name
        ...
        for member in familly:
            ...
            if options.replace:
                ...
                if fwl['RequestType'] == 'Resubmission':
                    continue
                if fwl['RequestStatus'] in ['None', None, 'new']:
                    continue
        ...
        if len(true_familly) > 1:
            ...
            sendLog('injector', 'Multiple wf in line, will take the last one for %s \n%s' % (
                wf.name, ', '.join(fwl['RequestName'] for fwl in true_familly)), level='critical')
        ...
        for fwl in true_familly[-1:]:
            ...
            if not new_wf:
                sendLog(
                    'injector', "putting %s as replacement of %s" %
                    (member, wf.name))
                status = 'away'
                ...
                if new_wf.status == 'forget':
                    continue
                sendLog(
                    'injector', "getting %s as replacement of %s" %
                    (new_wf.name, wf.name))
                wf.status = 'forget'
...
if __name__ == "__main__":
    ...
    parser.add_option(
        '-i',
        '--invalidate',
        help="fetch invalidations from mcm",
        default=False,
        action='store_true')
    parser.add_option(
        '-w',
        '--wmstatus',
        help="from which status in req-mgr",
        default="assignment-approved")
    parser.add_option(
        '-s',
        '--setstatus',
