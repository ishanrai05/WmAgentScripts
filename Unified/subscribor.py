...
if status == 'wmagent':
    ...
    wfs.extend([wfo.name for wfo in session.query(Workflow).filter(
        Workflow.status.startswith(status[:-1])).all()])
else:
    wfs.extend([wfo.name for wfo in session.query(
        Workflow).filter(Workflow.status == status).all()])
...
for iw, wfn in enumerate(wfs):
    ...
    for out in outs:
        ...
        for site, blocks in blocks_at_sites.items():
            if 'Buffer' in site or 'Export' in site or 'MSS' in site:
                continue
