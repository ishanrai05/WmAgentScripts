...
def assignor(url, specific=None, talk=True, options=None):
    if userLock() and not options.manual:
        return
    mlock = moduleLock()
    if mlock() and not options.manual:
        return
    if not componentInfo().check() and not options.manual:
        return
    ...
    LI = lockInfo()
    if not LI.free() and not options.go and not options.manual:
        return
    ...
    for wfo in wfos:
        ...
        options_text = ""
        if options.early:
            options_text += ", early option is ON"

        wfh.sendLog(
            'assignor', "%s to be assigned %s" %
            (wfo.name, options_text))

        # the site whitelist takes into account siteInfo, campaignInfo, memory
        # and cores
        (lheinput, primary, parent, secondary,
         sites_allowed) = wfh.getSiteWhiteList()
        output_tiers = list(set([o.split('/')[-1]
                                 for o in wfh.request['OutputDatasets']]))
        ...
        if not output_tiers:
            ...
            sendLog(
                'assignor',
                'Workflow %s has no output at all' %
                (wfo.name),
                level='critical')
            continue
        ...
        for campaign in wfh.getCampaigns():
            ...
            if campaign in CI.campaigns and 'banned_tier' in CI.campaigns[campaign]:
                banned_tier = list(
                    set(CI.campaigns[campaign]['banned_tier']) & set(output_tiers))
                if banned_tier:
                    no_go = True
                    wfh.sendLog(
                        'assignor',
        ...
        if secondary and check_secondary:
            ...
            for sec in secondary:
                # and 'parameters' in allowed_secondary[sec]:
        ...
        if wfh.request['RequestStatus'] != 'assignment-approved':
            if not options.test:
                wfh.sendLog(
                    'assignor',
                    "setting %s away and skipping" %
                    wfo.name)
                # the module picking up from away will do what is necessary of
                # it
        ...
        # retrieve from the schema, dbs and reqMgr what should be the next
        # version
        ...
        wfh.sendLog(
            'assignor', "Initial values for primary_AAA=%s and secondary_AAA=%s" %
            (primary_aaa, secondary_aaa))

        # keep track of this, after secondary input location restriction :
        # that's how you want to operate it
        ...
        if cpuh > max_cpuh_block and not options.go:
            # sendEmail('large workflow','that wf %s has a large number of CPUh
            # %s, not assigning, please check the logs'%(wfo.name,
            # cpuh))#,destination=['Dmytro.Kovalskyi@cern.ch'])
            sendLog(
                'assignor',
            ...
            wfh.sendLog(
                'assignor',
                "Requiring a large number of CPUh %s, not assigning" %
                cpuh)
            continue

        # should also check on number of sources, if large enough, we should be
        # able to overflow most, efficiently
        ...
        if primary_aaa:
            ...
            if not sites_allowed:
                ...
                wfh.sendLog(
                    'assignor',
                    "Selected to read primary through xrootd %s" %
                    sorted(sites_allowed))

        isStoreResults = (
            'StoreResults' == wfh.request.setdefault(
                'RequestType', None))
        ...
        # if any of the site allowed is low pressure : reduce to 1 copy so that
        # it gets started
        ...
        if t1_only:
            ...
            sites_out = [SI.pick_dSE([SI.CE_to_SE(ce)
                                      for ce in sites_allowed])]
        ...
        if primary_aaa:
            ...
            wfh.sendLog(
                'assignor',
                "Reading primary through xrootd at %s" %
                sorted(sites_allowed))
        ...
        if secondary_aaa:
            ...
            wfh.sendLog(
                'assignor',
                "Reading secondary through xrootd at %s" %
                sorted(sites_allowed))
        ...
        if split_check is None or split_check == False:
            n_stalled += 1
            continue
        elif split_check:
        ...
        if 'PU_RD' in pstring:
            ...
            if 'PU_RD2' in pstring:
                ...
                if lumisPerJob == 0:
                    ...
                    sendLog(
                        'assignor', "%s needs to be split by event with %s per job" %
                        (wfo.name, eventsPerJob), level='critical')
                    wfh.sendLog(
                        'assignor', "%s needs to be split by event with %s per job" %
                    ...
                    if eventsPerJobEstimated and eventsPerJobEstimated > eventsPerJob:
                        ...
                        sendLog(
                            ...
                            (wfo.name, lumisPerJob), level='critical')
                        wfh.sendLog(
                            'assignor', "%s was assigned with %s lumis/job" %
                        ...
                        sendLog(
                            'assignor', "leaving splitting untouched for %s, please check on %s" %
                            (pstring, wfo.name), level='critical')
                        wfh.sendLog(
                            'assignor', "leaving splitting untouched for PU_RD*, please check.")
        ...
        if isHEPCloudReady(url) and wfh.isGoodForNERSC():
            ...
            if secondary:
                parameters['TrustPUSitelists'] = True
            sendEmail(
                "sending work to hepcloud",
                "pleasse check on %s" %
                wfh.request['RequestName'],
                destination=['hufnagel@fnal.gov'])

        # make sure to autoapprove all NonCustodialSites
        parameters['AutoApproveSubscriptionSites'] = list(set(
            parameters['NonCustodialSites'] + parameters.get('AutoApproveSubscriptionSites', [])))
        result = reqMgrClient.assignWorkflow(
            url, wfo.name, None, parameters)  # team is not relevant anymore here
        ...
        if not options.test:
            if result:
                ...
                if wfh.producePremix() and (not wfh.isRelval()):
                    ...
                    body += "\nAssigned to: {}".format(
                        parameters['SiteWhitelist'])
                    sendEmail(title, body, destination=[
                              'cms-production-heavy-workflow-notifications@cern.ch'])

                try:
                    # refetch information and lock output
                    new_wfi = workflowInfo(url, wfo.name)
                    (_, prim, _, sec) = new_wfi.getIO()
                    for secure in list(prim) + list(sec) + \
                            new_wfi.request['OutputDatasets']:
                        # lock all outputs
...
if __name__ == "__main__":
    ...
    parser.add_option(
        '-m',
        '--manual',
        help='Manual assignment, bypassing lock check',
        action='store_true',
        dest='manual',
        default=False)
    parser.add_option(
        '-e',
        '--early',
        help='Fectch from early statuses',
        default=False,
        action="store_true")
    parser.add_option(
        '--go',
        help="Overrides the campaign go",
        default=False,
        action='store_true')
    ...
    parser.add_option(
        '--secondary_aaa',
        help="Force to use the primary location restriction",
        default=False,
        action='store_true')
    parser.add_option(
        '--limit',
        help="Limit the number of wf to be assigned",
        default=0,
        type='int')
    parser.add_option(
        '--priority',
        help="Lower limit on priority of wf to be assigned",
        default=0,
        type='int')
    parser.add_option(
        '--from_status',
        help="The unified status we should try to assign from",
        default=None)
    parser.add_option(
        '--force_options',
        help="Use the command line options as last modifiers",
        default=False,
        action='store_true')
    ...
    for key in reqMgrClient.assignWorkflow.keys:
        parser.add_option(
            '--%s' %
            key,
            help="%s Parameter of request manager assignment interface" %
            key,
            default=None)
