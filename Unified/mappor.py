...
def mappor(url, options=None):
    ...
    if options.cscs:
        use_CSCS = True
    if use_CSCS:
        over_rides.append('T0_CH_CSCS_HPC')
    ...
    for site in sites_to_consider:
        ...
        if region not in ['US', 'DE', 'IT', 'FR', 'CH',
                          'ES',
                          'UK',
                          'RU'  # latest addition
                          ]:
    ...
    for site in sites_to_consider:
        ...
        within_region = [fb for fb in sites_to_consider if any([('_%s_' % (
            reg) in fb and fb != site and site_in_depletion(fb))for reg in regions[region]])]
    ...
    europ_t1 = [site for site in sites_to_consider if site.startswith(
        'T1') and any([reg in site for reg in ['IT', 'DE', 'UK', 'FR', 'ES', 'RU']])]
    #print europ_t1
    for one in europ_t1:
        for two in europ_t1:
            if one == two:
    ...
    # mapping['T1_IT_CNAF'].extend( [site for site in SI.sites_ready if '_US_'
    # in site] ) ## all US can read from CNAF
    ...
    for site, fallbacks in mapping.items():
        ...
        for fb in fallbacks:
            ...
            if fb in take_site_out:
                ...
                mapping[site].remove(fb)
                continue
            if site not in reversed_mapping[fb]:
