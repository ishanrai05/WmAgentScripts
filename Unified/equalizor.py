...
def equalizor(url, specific=None, options=None):
    ...
    if not specific:
        if not up.check():
            return  # Only check component when running cron job with everything
    ...
    if options.t0:
        use_T0 = True
    if use_T0:
        over_rides.append('T0_CH_CERN')

    use_HLT = ('T2_CH_CERN_HLT' in UC.get("site_for_overflow"))
    if options.hlt:
        use_HLT = True
    if use_HLT:
        over_rides.append('T2_CH_CERN_HLT')
    ...
    def needs_action(wfi, task, min_idled=100):
        ...
        if (not running and idled) or (running and (
                idled / float(running) > needs_action.pressure)):
            go = True
        else:
            go = False
        print(
            "[INFO]: Task {} has {} running, {} idle. Action needed: {}".format(
                task_name,
                running,
                idled,
                go))
    ...
    def getPerf(task, stats_to_go, original_ncore=1):
        ...
        if io_data and 'aggregations' in io_data and io_data["aggregations"]["2"]["buckets"]:
            inputGB = io_data["aggregations"]["2"]["buckets"][0].get(
                'InputGB', {}).get('value', None)
            buckets = io_data["aggregations"]["2"]["buckets"][0].get(
                'RequestCpus', {}).get('buckets', [])
        ...
        buckets = filter(lambda i: i['key'] != 0, perf_data['aggregations']["2"]["buckets"]
                         ) if 'aggregations' in perf_data else []  # fail safe on ES missing data
        for bucket in buckets:
            sub_buckets = filter(
                lambda i: i['key'] != 0,
                bucket["3"]["buckets"])
        ...
        for core_count in binned_memory:
            ...
            if sum(values) < stats_to_go:
                print "not enough stats to go for core-binned mem usage", sum(
                    values), "<", stats_to_go, "at cores=", core_count
                continue
            avg = 'N/A'
            if sum(values):
                avg = sum([(a * w)
                           for (a, w) in zip(bins, values)]) / sum(values)
            print "Getting information at", core_count, avg
            print bins
            print values
            if values:
                percentiles[core_count] = weighted_percentile(
                    values, bins, memory_percentil)
        ...
        slope = max(0, int(sum([l *
                                v for (l, v) in zip(lever, slopes)]) /
                           sum(lever))) if slopes else None
        ...
        p_t = percentile_data['aggregations']["2"]["values"].get(
            "%.1f" % time_percentil, None) if 'aggregations' in percentile_data else None
        bck_p_t = percentile_data['aggregations']["2"]["values"].get(
            "%.1f" %
            backup_time_percentil,
            None) if 'aggregations' in percentile_data else None
        if p_t == "NaN":
            p_t = None
        if bck_p_t == "NaN":
            p_t = None
        ...
        if p_t:
            p_t *= 60.  # convert in mins
        if bck_p_t:
            bck_p_t *= 60  # convert in min
        w_t = 0
        ...
        print "%s%% of the jobs are running for a total core-min under %s [min]" % (
            used_percentil, b_t)
    ...
    def close(interface):
        eosFile(
            ...
            json.dumps(
                ...
                indent=2)).close()
        os.system(
            'mv %s/equalizor.json.new %s/equalizor.json' %
            (monitor_pub_dir, monitor_pub_dir))
        os.system('cp %s/equalizor.json %s/logs/equalizor/equalizor.%s.json' %
                  (monitor_pub_dir, monitor_dir, time.mktime(time.gmtime())))
        # move it where people use to see it ## should go away at some point
        os.system(
            'cp %s/equalizor.json /afs/cern.ch/user/c/cmst2/www/unified/.' %
            (monitor_pub_dir))
    ...
    try:
        gmon = json.loads(os.popen(
            'curl -s http://cms-gwmsmon.cern.ch/prodview/json/T2_CH_CERN_HLT').read())
    ...
    random.shuffle(wfs)
    for wfo in wfs:
        if wfo.status not in ['away']:
            continue
        if wfo.name in no_routing and not options.augment:
            continue

        if specific and specific not in wfo.name:
            continue
        if specific:
        ...
        if not wfi.request['RequestStatus'] in [
                'running-open', 'running-closed'] and not specific:
            continue
        ...
        for s in sites_allowed:
            sites_allowed_se.update(SI.CE_to_SEs(s))
        ...
        if not lhe and not prim and not wfi.isRelval():
            if (sec and wfi.request['TrustPUSitelists']) or (not sec):
                ...
                print (
                    "Adding {} adhoc for HPC as no task of the workflow requires any input".format(
                        wfo.name))
        ...
        needs_overide = False
        if not needs_overide and options.augment:
            needs_overide = True
        ...
        def overide_from_agent(wfi, needs_overide):
            ...
            if any([agent in agents.get(wqs, {}).keys()
        ...
        for i_task, (task, campaign) in enumerate(tasks_and_campaigns):
            ...
            if resize and isinstance(resize, dict):  # and not is_chain:
                print "adding", task.pathName, "in resizing"
                resizing[task.pathName] = copy.deepcopy(resize)
            elif resize and resize == 'auto':
                # can we add this tuning add-hoc by assuming Memory = a +
                # Ncore*b, where a is a fraction of Memory ?
                ...
                if mcore >= UC.get('min_core_for_resize'):
                    ...
                    wfi.sendLog(
                        'equalizor',
                        "Adding %s in resizing, calculating %d < %d < %d MB, using %d to %d cores" %
                        (task.pathName,
                         min_mem_per_core,
                         mem_per_core_c,
                         max_mem_per_core,
                    ...
                    resizing[task.pathName] = {"minCores": min_core,
                                               "maxCores": max_core,
                                               "memoryPerThread": quantize(mem_per_core,
                                                                           slope_quanta)}
                else:
                    print "do not start resizing a task that was set <", UC.get(
                        'min_core_for_resize')
            ...
            if tune and campaign not in tune_performance:
                tune_performance.append(campaign)

            overflow = CI.get(campaign, 'overflow', {})
            if overflow:
                if "PRIM" in overflow and campaign not in PRIM_overflow:
                    PRIM_overflow[campaign] = copy.deepcopy(overflow['PRIM'])
                    print "adding", campaign, "to PRIM overflow"
                if "PREMIX" in overflow and campaign not in PREMIX_overflow:
                    PREMIX_overflow[campaign] = copy.deepcopy(
                        overflow['PREMIX'])
                    print "adding", campaign, "to PREMIX overflow"
                if "PU" in overflow and campaign not in PU_overflow:
                    PU_overflow[campaign] = copy.deepcopy(overflow['PU'])
                    print "adding", campaign, "to PU overflow rules"
                if "LHE" in overflow and campaign not in LHE_overflow:
                    ...
                    print "adding", campaign, "to light input overflow rules", sorted(
            ...
            if campaign in tune_performance or options.tune:
                ...
                if task.taskType in ['Processing', 'Production']:
                    mcore = wfi.getCorePerTask(taskname)
                    set_memory, set_slope, set_time, set_io = getPerf(
                        task.pathName, stats_to_go, original_ncore=mcore)
                    ...
                    if set_slope:
                        ...
                        if task.pathName in resizing and "memoryPerThread" in resizing[task.pathName]:
                            resizing[task.pathName]["memoryPerThread"] = quantize(
                                set_slope, slope_quanta)
                        perf_per_config[configcache.get(
                            taskname, 'N/A')]['slope'] = set_slope
                    mem = wfi.getMemoryPerTask(taskname)
                    print taskname, mem
                    for key, add_hoc_mem in memory_correction.items():
                        if key in taskname and mem > add_hoc_mem and (
                                set_memory is None or set_memory > add_hoc_mem):
                            print "overiding", set_memory, "to", add_hoc_mem, "by virtue of add-hoc memory_correction", key
                            set_memory = min(
                                add_hoc_mem, set_memory) if set_memory else add_hoc_mem

                    if set_memory:
                        set_memory = min(
                            set_memory, 20000)  # no bigger than 20G
                        # do not go too low. not less than 1G/core.
                        set_memory = max(set_memory, mcore * 1000)
                        print "trully setting memory to", set_memory
                        performance[task.pathName]['memory'] = set_memory
                        perf_per_config[configcache.get(
                            taskname, 'N/A')]['memory'] = set_memory
                    ...
                    if set_time:
                        if set_time > warning_long_time * mcore:
                            ...
                            wfi.sendLog(
                                'equalizor',
                                'WARNING the task %s was found to run long jobs  of %d [h] %d [mins] at original %d cores setting' %
                                (taskname,
                                 divmod(
                                     set_time /
                                     mcore,
                                    ...
                                    mcore))
                            long_tasks.add(
                                (task.pathName, set_time / mcore, mcore))

                        # max to 24H per mcore
                        set_time = min(
                            set_time, int(
                                warning_long_time * mcore))
                        # min to 30min per core
                        set_time = max(set_time, int(30. * mcore))
                        performance[task.pathName]['time'] = set_time
                        perf_per_config[configcache.get(
                            taskname, 'N/A')]['time'] = set_time
                    if set_io:
                        performance[task.pathName]['read'] = set_io
                        perf_per_config[configcache.get(
                            taskname, 'N/A')]['read'] = set_io

                    # make up some warnings
                    if set_time and (
                            set_time / mcore) < warning_short_time:  # looks like short jobs all around
                        print "WHAT IS THIS TASK", task.pathName, "WITH", set_time / mcore, "small runtime"
                        wfi.sendLog(
                            'equalizor',
                            'The task %s was found to run short jobs of %.2f [mins] at original %d cores setting' %
                            (taskname,
                             set_time /
                             mcore,
                             mcore))
                    ...
                    if mem and (
                        (mem > warning_mem *
                         mcore) if wfi.request['RequestType'] != 'StepChain' else (
                            mem > warning_mem *
                            wfi.getMulticore())):
                        print "WHAT IS THIS TASK", task.pathName, "WITH", mem, "memory requirement at", mcore, "cores"
                        wfi.sendLog(
                            'equalizor',
                            'The task %s was found to be confiugred with %d MB over %d MB/core at %d cores' %
                            (taskname,
                             mem,
                             warning_mem,
                             mcore))
                    ...
                    if set_memory and (set_memory > warning_mem * mcore):
                        ...
                        wfi.sendLog(
                            'equalizor',
                            'The task %s was found to run jobs using %d MB over %d MB/core at %d cores' %
                            (taskname,
                             set_memory,
                             warning_mem,
                             mcore))
...
""" % (taskname,
            ...
            # rule to remove from the site whitelist site that do not look
            # ready for unified (local banning)
            if wfo.name in restricting_to_ready:
                if task.taskType in ['Production']:
                    new_list = list(set(SI.sites_ready) & set(
                        wfi.request['SiteWhitelist']))
                    modifications[wfo.name][task.pathName] = {
                        "ReplaceSiteWhitelist": new_list}
            ...
            if campaign in PREMIX_overflow:
                ...
                restrict_to_allowed = not (
                    (campaign in tune_performance) or (
                        CI.get(
                            campaign, 'resize', False)))

                if options.augment:
                    print "\t", task.pathName
                if False and is_chain and task.pathName.endswith(
                        '_1') and not options.augment:
                ...
                if task.taskType in ['Processing', 'Production'] and needs:
                    ...
                    for s in sec:
                        if s not in PU_locations:
                            presence = getDatasetPresence(url, s)
                            one_secondary_locations = [site for (
                                site, (there, frac)) in presence.items() if frac > 98.]
                            PU_locations[s] = one_secondary_locations
                        print "secondary is at", sorted(PU_locations[s])
                        secondary_locations = set(
                            [SI.SE_to_CE(site) for site in PU_locations[s]]) & secondary_locations
                    ...
                    if len(prim):
                        ...
                        presence = getDatasetPresence(
                            url, dataset, only_blocks=blocks)
                        aaa_prim_grid = set([SI.SE_to_CE(site)
                                             for site in presence.keys()])
                        for site in sorted(aaa_prim_grid):
                            aaa_prim_grid.update(mapping.get(site, []))

                        print sorted(aaa_prim_grid), "around primary location", sorted(
                            presence.keys())
                        print sorted(aaa_sec_grid), "aroudn secondary location", sorted(
                            secondary_locations)
                        # intersect
                    ...
                    aaa_grid = filter(
                        lambda s: s not in banned_until_you_find_a_way_to_do_this, aaa_grid)
                    print sorted(aaa_grid), "for premix"
                    aaa_grid = list(set(aaa_grid) & set(SI.sites_ready))
                    print sorted(aaa_grid), "that are ready"
                    if aaa_grid:
                        wfi.sendLog(
                            'equalizor', 'Extending site whitelist for %s to %s due to PREMIX_overflow' %
                            (task.pathName, sorted(aaa_grid)))
                        modifications[wfo.name][task.pathName] = {
                            "AddWhitelist": sorted(aaa_grid)}
            ...
            if campaign in PRIM_overflow and len(
                    prim) > 0:  # in case the overflow is set to latter campaigns in the task chain
                if task.taskType in ['Processing', 'Production']:
                    if not wfi.request['TrustSitelists']:
                        ...
                        presence = getDatasetPresence(
                            url, dataset, only_blocks=blocks)
                        in_full = [
                            SI.SE_to_CE(site) for site, (there, _) in presence.items() if there]
                        aaa_grid = set()
                        ...
                        # just add the neighbors to the existing whitelist. we
                        # could do more with block classAd
                        for site in wfi.request['SiteWhitelist']:
                            aaa_grid.update(mapping.get(site, []))
                        # and restrict to site that would be allowed at all
                        # (mcore, mem)
                        aaa_grid_ = list(aaa_grid & set(sites_allowed))
                        # and restrict to site that would be allowed at all
                        # (mcore, mem)
                        aaa_grid_in_full_ = list(
                            aaa_grid_in_full & set(sites_allowed))
                        # Only append the HPC if it's already appear in
                        # aaa_grid
                        aaa_grid_.extend([x for x in add_on if x in aaa_grid])
                        aaa_grid_in_full_.extend(
                            [x for x in add_on if x in aaa_grid_in_full])
                        aaa_grid = set(aaa_grid_)
                        aaa_grid_in_full = set(aaa_grid_in_full_)

                        gmon = wfi.getGlideMon()
                        needs, task_name, running, idled = needs_action(
                            wfi, task)
                        print needs, running, idled
                        site_in_use = set(
                            gmon[task_name]['Sites']) if gmon and task_name in gmon and 'Sites' in gmon[task_name] else set()
                        print dataset, "at", sorted(
                            in_full), len(blocks), "/", count_all
                        ...
                        print "around where the data is now in full", sorted(
                            aaa_grid_in_full)

                        if needs and not (
                                site_in_use & set(in_full)) and aaa_grid_in_full:
                            print "we could be going for replace at that point"
                            wfi.sendLog(
                                'equalizor',
                                'Replaceing site whitelie to %s dynamically' %
                                sorted(aaa_grid_in_full))
                            modifications[wfo.name][task.pathName] = {
                                "ReplaceSiteWhitelist": sorted(aaa_grid_in_full)}
                        else:
                            if needs and aaa_grid:
                                print wfo.name
                                wfi.sendLog(
                                    'equalizor',
                                    'Adding in site white list %s dynamically' %
                                    sorted(aaa_grid))
                                if wfo.name in modifications and task.pathName in modifications[
                                        wfo.name] and 'AddWhitelist' in modifications[wfo.name][task.pathName]:
                                    modifications[wfo.name][task.pathName]["AddWhitelist"].extend(
                                        sorted(aaa_grid))
                                else:
                                    modifications[wfo.name][task.pathName] = {
                                        "AddWhitelist": sorted(aaa_grid)}
                    else:
                        # the request is already is in xrootd mode (either too
                        # generous, or just about right with neighbors of full
                        # data)
                        dataset = list(prim)[0]
                        all_blocks, blocks = wfi.getActiveBlocks()
                        count_all = sum([len(v)
                        ...
                        presence = getDatasetPresence(
                            url, dataset, only_blocks=blocks)
                        # in full is really the only place we can go to safely,
                        # since we have no job-data matching
                        in_full = [
                            SI.SE_to_CE(site) for site, (there, _) in presence.items() if there]
                        gmon = wfi.getGlideMon()
                        needs, task_name, running, idled = needs_action(
                        ...
                        # symptomatic of data have been repositionned
                        new_ones = set(in_full) - \
                            set(wfi.request['SiteWhitelist'])
                        common = set(in_full) & set(
                            wfi.request['SiteWhitelist'])
                        # symptomatic of too generous site-whitelist
                        extra_shit = set(
                            wfi.request['SiteWhitelist']) - aaa_grid
                        # restrict to site that would be allowed at all (mcore,
                        # mem)
                        aaa_grid_ = list(aaa_grid & set(sites_allowed))
                        # Only append the HPC if it's already appear in
                        # aaa_grid
                        ...
                        print dataset, "is in full ", len(
                            blocks), "/", count_all, " at", in_full
                        print '\n'.join(sorted(blocks))
                        print "running at", site_in_use
                        print "in common of the site whitelist", sorted(common)
                        print "site now also hosting the data", sorted(
                            new_ones)
                        print "site in whitelist with no data", sorted(
                            extra_shit)  # with no data and not within aaa reach
                        # if new_ones:
                        ...
                        if needs and aaa_grid:
                            print wfo.name, "would replace for", sorted(
                                aaa_grid)
                            #print "but no thanks"
                            wfi.sendLog(
                                'equalizor',
                                'Changing the site whitelist to %s dynamically' %
                                (sorted(aaa_grid)))
                            modifications[wfo.name][task.pathName] = {
                                "ReplaceSiteWhitelist": sorted(aaa_grid)}
                        elif new_grid:
                            print wfo.name, "would complement up to", sorted(
                                aaa_grid)
                            wfi.sendLog(
                                'equalizor',
                                'Adding site white list to %s dynamically' %
                                sorted(new_grid))
                            modifications[wfo.name][task.pathName] = {
                                "AddWhitelist": sorted(new_grid)}
                        elif len(extra_shit) > 5:
                            if aaa_grid:
                                print wfo.name, "would be restricting down to", sorted(
                                    aaa_grid), "because of", sorted(extra_shit)
                                wfi.sendLog(
                                    'equalizor',
                                    'Restricting the white list to %s dynamically' %
                                    sorted(aaa_grid))
                                modifications[wfo.name][task.pathName] = {
                                    "ReplaceSiteWhitelist": sorted(aaa_grid)}
                        else:
            ...
            if wfo.name in remove_from and task.taskType in [
                ...
                if intersection:
                    ...
                    modifications[wfo.name][task.pathName] = {
                        "ReplaceSiteWhitelist": sorted(restrict_to)}
            ...
            if wfo.name in add_to:
                if task.taskType in ['Production', 'Processing']:
                    ...
                    if wfo.name in modifications and task.pathName in modifications[
                        ...
                        modifications[wfo.name][task.pathName] = {
                            "AddWhitelist": augment_to}

            # rule to avoid the issue of taskchain secondary jobs being stuck
            # at sites processing the initial step
            if campaign in LHE_overflow:
                ...
                # it is not the same as doing the intersection with
                # sites_allowed, as it can be broader here
                neighboring_ces = set()
                for s in SI.sites_ready:
                    s_se = SI.CE_to_SEs(s)
                ...
                # at the intersection
                extend_to = set(LHE_overflow[campaign]) & neighboring_ces
                ...
                if needs or needs_overide:
                    wfi.sendLog('equalizor', 'Extending site whitelist for %s to %s due to LHE_overflow' % (
                        task.pathName.split('/')[-1], sorted(extend_to)))
                    ...
                    wfi.sendLog(
                        'equalizor', '%s of %s is running %d and pending %d. no light overflow' %
            ...
            if campaign in PU_overflow:
                ...
                better_secondary_locations = wfi.getClassicalPUOverflow(
                    taskname)
                if not better_secondary_locations:
                    ...
                    # intersect with the sites that are allowed from the
                    # request requirement

                    secondary_locations = secondary_locations & set(
                        sites_allowed)
                else:
                    print "Agents know that the secondary is at", better_secondary_locations
                    secondary_locations = set(
                        better_secondary_locations) & set(memory_allowed)
                ...
                if any([task.pathName.endswith(finish) for finish in ends]):
                    needs, task_name, running, idled = needs_action(wfi, task)
                    # removing the ones in the site whitelist already since
                    # they encode the primary input location
                    if stay_within_site_whitelist:
                        original_site_in_use = set(
                            wfi.request['SiteWhitelist'] & set(secondary_locations))
                    else:
                    ...
                    if gmon and task_name in gmon and 'Sites' in gmon[
                            task_name] and mode == 'AddWhitelist':
                        site_in_use = set(gmon[task_name]['Sites'])
                        site_in_use = set(
                            [])  # at this time I cannot find a reason to apply such limitation
                        print "removing", sorted(site_in_use)
                        # that determines where you want to run in addition
                        augment_by = list(
                            (set(secondary_locations) - site_in_use) & original_site_in_use)
                    else:
                    ...
                    needs_overide = overide_from_agent(wfi, needs_overide)
                    if augment_by and (
                            needs or needs_overide or force) and PU_overflow[campaign]['pending'] < PU_overflow[campaign]['max']:
                        PU_overflow[campaign]['pending'] += idled
                        print "raising overflow to", PU_overflow[campaign][
                            'pending'], "for", PU_overflow[campaign]['max']
                        # the step with an input ought to be the digi part :
                        # make this one go anywhere
                        modifications[wfo.name][task.pathName] = {
                            mode: augment_by, "Running": running, "Pending": idled, "Priority": wfi.request['RequestPriority']}
                        altered_tasks.add(task.pathName)
                        wfi.sendLog(
                            'equalizor',
                            '%s of %s is running %d and pending %d, taking action : %s \n %s' %
                            (task_name,
                             wfo.name,
                             running,
                             idled,
            ...
            if campaign in [
                'Run2015D',
                    'Run2015C_25ns'] and task.taskType == 'Skim':
                original_swl = wfi.request['SiteWhitelist']
                needs, task_name, running, idled = needs_action(wfi, task)
                if (needs or needs_overide):
                    modifications[wfo.name][task.pathName] = {
                        'AddWhitelist': original_swl, "Running": running, "Pending": idled, "Priority": wfi.request['RequestPriority']}
                    altered_tasks.add(task.pathName)
                    wfi.sendLog(
                        'equalizor',
            ...
            if campaign in [
                ...
                # what are the site you want to take out. What are the jobs in
                # whitelist, make the diff and replace
                t1s = set(
                    [site for site in SI.all_sites if site.startswith('T1')])
                ust2s = set(
                    [site for site in SI.all_sites if site.startswith('T2_US')])
                ...
                if set_for:
                    modifications[wfo.name][task.pathName] = {
                        "ReplaceSiteWhitelist": sorted(set_for)}

            # add the HLT at partner of CERN
            if 'T2_CH_CERN' in wfi.request['SiteWhitelist'] and i_task in [
                    0, 1] and use_HLT and not wfi.request['TrustSitelists']:
                needs, task_name, running, idled = needs_action(wfi, task)
                if options.augment:
                    needs = True
                needs = True
                # needs = random.random()<0.40 remove the random, just add up
                # to a limit
                if (needs or needs_overide) and pending_HLT < max_HLT:
                    ...
                    if task.pathName in modifications[wfo.name] and 'AddWhitelist' in modifications[wfo.name][task.pathName]:
                        modifications[wfo.name][task.pathName]["AddWhitelist"].append(
                            "T2_CH_CERN_HLT")
                        wfi.sendLog(
                            'equalizor', 'also adding the HLT in whitelist of %s to %d for %d' %
                    ...
                    # this Replace does not work at all for HLT
                        ...
                        modifications[wfo.name][task.pathName] = {"AddWhitelist": ["T2_CH_CERN_HLT"],
                                                                  ...
                                                                  "Pending": idled}
                        wfi.sendLog(
                            'equalizor', 'adding the HLT in whitelist of %s to %d for %d' %
                            (task.pathName, pending_HLT, max_HLT))
            ...
            if i_task == 0 and not sec and use_T0 and False:
                ...
                read_lhe = (
                    ('LheInputFiles' not in wfi.request) or bool(
                        wfi.request['LheInputFiles']))
                good_type &= not read_lhe
                if not good_type and not options.augment:
                    needs = False
                ...
                if (needs or needs_overide):
                    pending_T0 += idled
                    if task.pathName in modifications[wfo.name] and 'AddWhitelist' in modifications[wfo.name][task.pathName]:
                        if "T0_CH_CERN" not in modifications[wfo.name][task.pathName]["AddWhitelist"]:
                            modifications[wfo.name][task.pathName]["AddWhitelist"].append(
                                "T0_CH_CERN")
                            wfi.sendLog(
                        ...
                        if "T0_CH_CERN" not in modifications[wfo.name][task.pathName]["ReplaceSiteWhitelist"]:
                            modifications[wfo.name][task.pathName]["ReplaceSiteWhitelist"].append(
                                "T0_CH_CERN")
                            wfi.sendLog(
                                'equalizor', 'adding the T0 to replacement for %s to %d for %d' %
                                (task.pathName, pending_T0, max_T0))
                    else:
            ...
            if options.manual and options.manual.count(':') == 2:
                ...
                if manual_task == taskname:
                    ...
                    if a_sites:
                        ...
                        if task.pathName in modifications[wfo.name] and 'AddWhitelist' in modifications[wfo.name][task.pathName]:
                            modifications[wfo.name][task.pathName]['AddWhitelist'] = list(
                                set(modifications[wfo.name][task.pathName]['AddWhitelist'] + a_sites.split(',')))
                        else:
                            modifications[wfo.name][task.pathName] = {
                                'AddWhitelist': list(set(a_sites.split(',')))}
                    elif r_sites:
                        print "replacing manually", r_sites, "for", task.pathName
                        if task.pathName in modifications[
                                wfo.name] and 'ReplaceSiteWhitelist' in modifications[wfo.name][task.pathName]:
                            modifications[wfo.name][task.pathName]['ReplaceSiteWhitelist'] = list(set(
                                modifications[wfo.name][task.pathName]['ReplaceSiteWhitelist'] + r_sites.split(',')))
                        else:
                            modifications[wfo.name][task.pathName] = {
                                'ReplaceSiteWhitelist': list(set(r_sites.split(',')))}
    # completely add-hoc
    if options.manual and options.manual.count(':') == 3:
        ...
        if a_sites:
            if path in modifications[wf] and 'AddWhitelist' in modifications[wf][path]:
                modifications[wf][path]['AddWhitelist'] = list(
                    set(modifications[wf][path]['AddWhitelist'] + a_sites.split(',')))
            else:
                modifications[wf][path] = {
                    'AddWhitelist': list(set(a_sites.split(',')))}
        elif r_sites:
            if path in modifications[wf] and 'ReplaceSiteWhitelist' in modifications[wf][path]:
                modifications[wf][path]['ReplaceSiteWhitelist'] = list(
                    set(modifications[wf][path]['ReplaceSiteWhitelist'] + r_sites.split(',')))
            else:
                modifications[wf][path] = {
                    'ReplaceSiteWhitelist': list(set(r_sites.split(',')))}
    ...
    if short_tasks:
        sendLog(
            'equalizor',
            'These tasks are running very short jobs. Shorter than %d [min] \n %s' %
            (warning_short_time,
             '\n'.join(
                 [
                     "%s, %.f [mins] observed time at original %d core count" %
                     (a,
                      b,
                      c) for (
                         a,
                         b,
                         c) in sorted(short_tasks)])),
            level='critical')

    if long_tasks:
        sendLog(
            'equalizor',
            'These tasks are running very long jobs. Longer than %d [min] per core.\n %s' %
            (warning_long_time,
             '\n'.join(
                 [
                     "%s, %.f [h] %.f [mins] observed time at original %d core count" %
                     (a,
                      divmod(
                          b,
                          60)[0],
                         ...
                         a,
                         b,
                         c) in sorted(long_tasks)])),
            level='critical')

    if hungry_tasks:
        sendLog('equalizor', 'These tasks are running with more memory than usual\n %s' %
                ('\n'.join(["%s, %.f [MB] observed concumption at original %d core count" %
                            (a, b, c) for (a, b, c) in sorted(hungry_tasks)])), level='critical')

    if bad_hungry_tasks:
        sendLog(
            'equalizor',
            'These tasks are configured with more memory than usual\n %s' %
            ('\n'.join(
                [
                    "%s, %.f [MB] submitted requirement at original %d core count" %
                    (a,
                     b,
                     c) for (
                        a,
                        b,
                        c) in sorted(bad_hungry_tasks)])),
            level='critical')
    ...
    release_wf = list(set(already_holding) - set(hold_wf)) + \
        interface['release'].keys()
    ...
    release_site = list(set(already_holding) - set(hold_site)) + \
        interface['release_site'] + site_drive.get('release', [])
...
if __name__ == "__main__":
    ...
    parser.add_option(
        '-a',
        '--augment',
        help='add on top of the document',
        default=False,
        action='store_true')
    parser.add_option(
        '-r',
        '--remove',
        help='remove on workflow from the document',
        default=False,
        action='store_true')
    parser.add_option(
        '--t0',
        help="Allow to use T0",
        default=False,
        action='store_true')
    parser.add_option(
        '--hlt',
        help="Allow to use HLT",
        default=False,
        action='store_true')
    parser.add_option(
        '--cscs',
        help="Allow to use CSCS",
        default=False,
        action='store_true')
    parser.add_option(
        '--tune',
        help='Enable performance tuning',
        default=False,
        action='store_true')
    parser.add_option(
        '--high_prio',
        help='Toggle in the list of high priority workflows',
        default=None)
    parser.add_option(
        '--manual',
        help='Just add something fully by hand in there',
        default=None)
