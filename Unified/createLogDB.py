...
if check_years:
    ...
    years = filter(None, os.popen(
        'ls /eos/cms/store/logs/prod/').read().split('\n'))
...
n_index = 0
for year in years:
    if options.max and n_index > options.max:
        break
    if check_years and year not in check_years:
        continue
    if check_months:
        months = check_months
    else:
    ...
    for month in months:
        if options.max and n_index > options.max:
            break
        if check_months and month not in check_months:
            continue
        if specific:
            workflows = specific
        else:
            workflows = filter(
                None, os.popen(
                    'ls /eos/cms/store/logs/prod/%s/%s/WMAgent/' %
                    (year, month)).read().split('\n'))
        ...
        for workflow in workflows:
            ...
            if specific and not any(
                    s in workflow or workflow in s for s in specific):
                continue

            if any(v in workflow or workflow in v for v in vetoes):
                continue
            month = "%02d" % int(month)
            tars = filter(
                None, os.popen(
                    'ls /eos/cms/store/logs/prod/%s/%s/WMAgent/%s/' %
                    (year, month, workflow)).read().split('\n'))
            if not tars:
                continue
            print workflow, ":", len(tars), "tarballs"
            #already = []
            if options.force:
                N_deleted = session.query(LogRecord).filter(
                    LogRecord.workflow == workflow).delete()
                print N_deleted, "deleted entries"
                already = []
            else:
                already = session.query(LogRecord).filter(
                    LogRecord.workflow == workflow).all()
            ...
            tasks = set(filter(lambda b: b.startswith('LogCollect'), [
                        '-'.join(tar.replace(workflow, '').split('-', 4)[1:4]) for tar in tars]))
            ...
            for task in tasks:
                tars = filter(
                    None, os.popen(
                        'ls /eos/cms/store/logs/prod/%s/%s/WMAgent/%s/*%s*.tar' %
                        (year, month, workflow, task)).read().split('\n'))
                print task, len(tars)
                for tar in tars:
                    tar = tar.split('/')[-1]
                    # this is heavy, how can we avoid this ?
                    path = '/eos/cms/store/logs/prod/%s/%s/WMAgent/%s/%s' % (
                        year, month, workflow, tar)
                    if path in already_in_db:
                        print path, "already in DB"
                        continue

                    # a full copy is a bit too much
                    tarloc = '/eos/cms/store/logs/prod/%s/%s/WMAgent/%s/%s' % (
                        year, month, workflow, tar)

                    logs = filter(None, map(lambda b: b.split(
                        '/')[-1], os.popen('tar tf %s' % (tarloc)).read().split('\n')))
                    for log in logs:
                        record = LogRecord(
                            workflow=workflow,
                            logfile=log,
