...
def rejector(url, specific, options=None):
    ...
    if not componentInfo(soft=['wtc', 'jira']).check():
        return
    ...
    for wfo in wfs:
        ...
        comment = ""
        if options.comments:
            comment = ", reason: " + options.comments
        if options.test:
            if options.keep:
                print 'invalidating the workflow by unified operator {}{}'.format(
                    username, comment)
            else:
                print 'invalidating the workflow and outputs by unified operator {}{}'.format(
                    username, comment)
            results = [True]
        else:
            if options.keep:
                wfi.sendLog(
                    'rejector',
                    'invalidating the workflow by unified operator {}{}'.format(
                        username,
                        comment))
            else:
                wfi.sendLog(
                    'rejector',
                    'invalidating the workflow and outputs by unified operator {}{}'.format(
                        username,
                        comment))

            results = invalidate(
                url, wfi, only_resub=True, with_output=(
                    not options.keep))
            ...
            if options and options.clone:
                ...
                if 'ProcessingVersion' in schema:
                    schema['ProcessingVersion'] = int(
                        schema['ProcessingVersion']) + 1  # dubious str->int conversion
                else:
                ...
                if options.short_task and schema['RequestType'] == 'TaskChain':
                    ...
                    for k in schema.get('ProcessingString', {}).keys():
                        schema['ProcessingString'][translate[k]
                                                   ] = schema['ProcessingString'].pop(k)
                    for k in schema.get('AcquisitionEra', {}).keys():
                        schema['AcquisitionEra'][translate[k]
                                                 ] = schema['AcquisitionEra'].pop(k)

                if options.Multicore:
                    # to do : set it properly in taskchains
                    if schema['RequestType'] == 'TaskChain':
                        tasks, set_to = options.Multicore.split(
                            ':') if ':' in options.Multicore else ("", options.Multicore)
                        set_to = int(set_to)
                    ...
                    while True:
                        if 'Task%d' % step in schema:
                            ...
                            if 'Multicore' in schema[sname] and schema[sname]['Multicore'] == 1:
                                ...
                                # remove explicit assignment of the number of
                                # cores
                                tmcore = schema[sname].pop('Multicore')
                            tmem = schema[sname].pop('Memory')
                            if mcore and tmcore != mcore:
                                if options.test:
                                    print 'the conversion of %s to stepchain encoutered different value of Multicore %d != %d' % (
                            ...
                            up_s = sname
                            while True:
                                # climb up a step. supposedely already all
                                # converted
                                up_s = s_n.get(
                                    schema[up_s].get(
                                        'InputStep', None), None)
                            ...
                            if 'KeepOutput' not in schema[sname]:
                                # this is a weird translation capability.
                                # Absence of keepoutput in step means : keep
                                # the output. while in TaskChain absence means
                                # : drop
                                schema[sname]['KeepOutput'] = False
                            schema['TimePerEvent'] += eff * \
                                schema[sname].pop('TimePerEvent')
                            schema['SizePerEvent'] += eff * \
                                schema[sname].pop('SizePerEvent')
                            step += 1
                        else:
                            break
                ...
                if options.test:
                    ...
                    wfi.notifyRequestor(
                        'Rejected by unified operator %s' %
                        (comment), do_batch=False)
                    session.commit()
...
if __name__ == "__main__":
    ...
    parser.add_option(
        '-t',
        '--test',
        help="test mode - no changes are made",
        default=False,
        action="store_true")
    parser.add_option(
        '-c',
        '--clone',
        help="clone the workflow",
        default=False,
        action="store_true")
    parser.add_option(
        '--comments',
        help="Give a comment to the clone",
        default="")
    parser.add_option(
        '-k',
        '--keep',
        help="keep the outpuy in current status",
        default=False,
        action="store_true")
    parser.add_option(
        '--set_trouble',
        help="When rejecting but the status should be trouble instead of forget",
        default=False,
        action="store_true")
    # options for cloning
    parser.add_option(
        '--Memory',
        help="memory parameter of the clone",
        default=0,
        type=int)
    parser.add_option(
        '--Multicore',
        help="Set the number of core in the clone",
        default=None)
    parser.add_option(
        '--ProcessingString',
        help="change the proc string",
        default=None)
    parser.add_option(
        '--AcquisitionEra',
        help="change the acq era",
        default=None)
    parser.add_option('--PrepID', help='change the prepid', default=None)
    parser.add_option(
        '--EventsPerJob',
        help="set the events/job on the clone",
        default=0,
        type=int)
    parser.add_option(
        '--priority',
    ...
    parser.add_option(
        '--EventAwareLumiBased',
        help="set the splitting algo of the clone",
        default=False,
        action='store_true')
    parser.add_option(
        '--TimePerEvent',
        help="set the time/event on the clone",
        default=0,
        type=float)
    parser.add_option(
        '--filelist',
        help='a file with a list of workflows',
        default=None)
    parser.add_option(
        '--no_output',
    ...
    parser.add_option(
        '--deterministic',
        help='set the splitting to deterministic in the clone',
        default=False,
        action='store_true')
    parser.add_option(
        '--runs',
        help='set the run whitelist in the clone',
        default=None)
    parser.add_option(
        '--short_task',
