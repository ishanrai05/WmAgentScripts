...
for g in goods:
    prim_to_distribute.extend(
        [site for site in SI.sites_ready if site.startswith(g)])

prim_to_distribute = [
    SI.SE_to_CE(site) for site in [
        SI.CE_to_SE(site) for site in prim_to_distribute]]
...
for l in open(read_file).read().split('\n'):
    ...
    if l.startswith('#'):
        continue
    all_ds = filter(lambda w: w.count('/') ==
                    3 and w.startswith('/'), l.split())
    dss.update(all_ds)
