...
def spawn_harvesting(url, wfi, in_full):
    ...
    if ('EnableHarvesting' in wfi.request and not wfi.request['EnableHarvesting']) and (
            'DQMConfigCacheID' in wfi.request and wfi.request['DQMConfigCacheID']):
        if 'MergedLFNBase' not in wfi.request:
            print "fucked up"
            sendEmail(
                'screwed up wl cache',
                '%s wl cache is bad' %
            ...
            for dqm_input in dqms:
                ...
                for site in sites:
                    res = updateSubscription(
                        url, site, dqm_input, priority='reserved')
        ...
        for dqm_input in dqms:
            ...
            harvesting_schema['RequestString'] = 'HARVEST-' + \
                wfi.request['RequestString']
            ...
            if harvest_request:
                ...
                # should we protect for setting approved ? no, it's notified
                # below, assignment will fail, likely
                ...
                parameters = {
                    'SiteWhitelist': [
                        SI.SE_to_CE(se) for se in wfi.request['NonCustodialSites']],
                ...
                result = reqMgrClient.assignWorkflow(
                    url, harvest_request, team, parameters)
                if not result:
                    ...
                    wfi.sendLog(
                        'closor', '%s was created at announcement of %s in %s, failed to assign' %
                        (harvest_request, dqm_input, wfi.request['RequestName']))
                    sendLog(
                        'closor', '%s was created at announcement of %s in %s, failed to assign' %
                        (harvest_request, dqm_input, wfi.request['RequestName']), level='critical')
                else:
                    ...
                    wfi.sendLog(
                        'closor', '%s was created at announcement of %s in %s, and assigned' %
                        (harvest_request, dqm_input, wfi.request['RequestName']))
...
def closor(url, specific=None, options=None):
    ...
    if jump_the_line:
        ...
        wfs = session.query(Workflow).filter(
            ...
            sqlalchemy.not_(
                Workflow.status.contains('announced'))).all()
    else:
    ...
    if float(failed_threads / run_threads.n_threads) > 0:
        sendLog(
            ...
            (failed_threads, run_threads.n_threads), level='critical')
        sendEmail(
            'checkor', '%d/%d threads have failed, better check this out' %
    ...
    really_late_files = [
        info for info in really_late_files if info['delay'] / (60 * 60 * 24.) >= days_late]

    if really_late_files:
        subject = 'These %d files are lagging for %d days and %d retries announcing dataset \n%s' % (
            len(really_late_files), days_late, retries_late, json.dumps(really_late_files, indent=2))
...
class CloseBuster(threading.Thread):
    ...
    def close(self):
        ...
        if wfi.isRelval():
            ...
            if batch_name not in batch_go:
                # do the esimatation whethere this can be announced : only once
                # per batch
                in_batches = getWorkflowByCampaign(
                    url, batch_name, details=True)
                batch_go[batch_name] = all(map(lambda s: s not in ['completed',
                                                                   'running-open',
                                                                   'running-closed',
                                                                   'acquired',
                                                                   'staged',
        ...
        if wfi.request['RequestStatus'] in ['announced',
                                            'normal-archived'] and not options.force:
            # manually announced ??
            self.to_status = 'done'
            self.to_wm_status = wfi.request['RequestStatus']
            wfi.sendLog(
                'closor', '%s is announced already : %s' %
        ...
        expected_lumis = 1
        if 'TotalInputLumis' not in wfi.request:
        ...
        for out in outputs:
            ...
            if where:
                ...
                going_to = wfi.request['NonCustodialSites'] + \
                    wfi.request['CustodialSites']
                wfi.sendLog(
                    'closor',
                    "%s is not in full anywhere. send to %s" %
                    (out,
                     ",".join(
                         sorted(going_to))))
                at_destination = dict(
                    [(k, v) for (k, v) in presence.items() if k in going_to])
                else_where = dict(
                    [(k, v) for (k, v) in presence.items() if k not in going_to])
        ...
        if all(all_OK.values()) and ((wfi.request['RequestStatus'] in [
                'closed-out']) or options.force or jump_the_line):
            ...
            if not results:
                for out in outputs:
                    ...
                    if all_OK[out] and wfi.isRelval():
                        ...
                        if tier in UC.get("tiers_to_rucio_relval"):
                            wfi.sendLog(
                                'closor', "Data Tier: %s is blacklisted, so skipping dataset placement for: %s" %
                                (tier, out))
                            continue
                        ...
                        if destinations:
                            wfi.sendLog('closor', '%s to go to %s' %
                        ...
                        if campaign and campaign in CI.campaigns and 'toDDM' in CI.campaigns[
                                campaign] and tier in CI.campaigns[campaign]['toDDM']:
                            to_DDM = True
                        ...
                        if tier in UC.get("tiers_to_DDM"):
                            to_DDM = True
                        # check for unitarity
                        if tier not in UC.get(
                                "tiers_no_DDM") + UC.get("tiers_to_DDM"):
                            ...
                            sendLog(
                                'closor', "could not recognize %s for injecting in DDM" %
                                out, level='critical')
                            continue
                        ...
                        if to_DDM and campaign and campaign in CI.campaigns and 'DDMcopies' in CI.campaigns[
                            ...
                            if isinstance(ddm_instructions, int):
                                n_copies = CI.campaigns[campaign]['DDMcopies']
                            elif isinstance(ddm_instructions, dict):
                        ...
                        if to_DDM:
                            ...
                            if status:
                                wfi.sendLog(
                                    'closor', '%s is send to dynamo in %s copies %s %s' %
                                    (out, n_copies, sorted(destinations), group_spec))
                            else:
                                sendLog(
                                    'closor',
                                    "could not add " +
                ...
                if all(map(lambda result: result in [
                       'None', None, True], results)):
                    if not jump_the_line:
                        ...
                        if res not in ['None', None]:
                            # check the status again, it might well have
                            # toggled
            ...
            if all(map(lambda result: result in [
                   'None', None, True], results)):
                if jump_the_line:
                    if 'announced' not in wfo.status:
            ...
            if wfi.isRelval():
                ...
                self.to_wm_status = wfi.request['RequestStatus']
                wfi.sendLog(
                    'closor', "%s is %s, but will not be set in trouble to find a replacement." %
                    (wfo.name, self.to_wm_status))
            else:
...
if __name__ == "__main__":
    ...
    parser.add_option(
        '--no_harvest',
        help='Bypass the harvesting',
        default=False,
        action='store_true')
    parser.add_option(
        '--limit',
        help="Number of workflow to pass",
        default=0,
        type=int)
    parser.add_option(
        '--force',
        help="Force pushing the workflow through",
        default=False,
        action='store_true')
    parser.add_option(
        '--announce',
        help="Announce the outputs that should be announced",
        default=False,
        action='store_true')
    parser.add_option(
        '--threads',
