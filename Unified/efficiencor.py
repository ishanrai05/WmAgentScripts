...
while True:
    ...
    for (wf,lf) in wfs:
        ...
        if ":" in lf:
            ...
            #print outs
            look_in_files = []
            look_in_files.extend( [_f for _f in os.popen('find %s -name "*.log"'% ( condor_dir )).read().split('\n') if _f] )
            look_in_files.extend( [_f for _f in os.popen('find %s -name "*.out"'% ( condor_dir )).read().split('\n') if _f] )
            lfs = []
            flfs = []
            for out in look_in_files:
                ...
                for line in fhr.split('\n'):
                    if 'logArchive.tar.gz' in line:
                        fullpath = [w for w in line.split() if 'logArchive.tar.gz' in w][0]
