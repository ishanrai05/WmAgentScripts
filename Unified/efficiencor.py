...
while True:
    c += 1
    if len(sys.argv) > 1 and c > int(sys.argv[1]):
        break
    ...
    for who in whos:
        try:
            rw = open(
                '/afs/cern.ch/user/%s/%s/public/ops/retrieve.json' %
                (who[0], who))
    ...
    for (wf, lf) in wfs:
        ...
        if ":" in lf:
            ...
            condor_dir = '%s/logmapping/condorlogs/%s/0/ATask/%s/%s_%s/Job_%s' % (
                monitor_eos_dir, wf, jobid[:3], agent.split('.')[0], jobid, jobid)
            ...
            for out in look_in_files:
                ...
                for line in fhr.split('\n'):
                    if 'logArchive.tar.gz' in line:
                        fullpath = filter(
                            lambda w: 'logArchive.tar.gz' in w, line.split())[0]
        ...
        # then do the rest
        not_found = (
            lf and 'nothing found' in os.popen(
                'Unified/whatLog.py --workflow %s --log %s' %
                (wf, lf)).read())
        nothing_indexed = (
            'nothing found' in os.popen(
                'Unified/whatLog.py --workflow  %s' %
                wf).read())
        ...
        if lf:
            ...
            if os.path.isfile(final_dest):
                ...
                continue
            # show that one
            com = 'Unified/whatLog.py --workflow  %s --log %s > %s/%s.txt' % (
                wf, lf, expose_all, lf)
            print 'Ex:', com
            os.system(com)
            # get it locally
            com = 'Unified/whatLog.py --workflow  %s --log %s --get ' % (
                wf, lf)
            print 'Ex:', com
            os.system(com)
            lfile = os.popen(
                'find /tmp/vlimant/ -name "*%s"' %
                lf).read().replace(
                '\n',
                '')
            ...
            if not(lfile and os.path.isfile(lfile)):
                ...
                com = 'XRD_REQUESTTIMEOUT=10 xrdcp root://cms-xrd-global.cern.ch/%s /tmp/vlimant/%s' % (
                    fullpath, lf)
                print 'Ex:', com
                os.system(com)
            lfile = os.popen(
            ...
            if lfile and os.path.isfile(lfile):
                ...
                os.system(com)
                if agent and jobid:
                    com = 'cp %s %s/%s_%s.tar.gz' % (lfile,
                                                     expose_all, agent, jobid)
                    print 'Ex:', com
                    os.system(com)
