...
#    payload['RequestPriority'] *= 10
    ...
    payload['RequestPriority'] = min(
        500000,
        payload['RequestPriority'] *
        2)  # never above 500k
    ...
    if actions:
        for action in actions:
            if action.startswith(
                    'mem') and actions[action] != "" and actions[action] != 'Same':
                # if multicore parameter is also used, need to scale memory by
                # the new number of cores
                if 'multicore' in actions and actions['multicore'] != "":
                    continue
                # Taskchains needs to be treated special to set the memory to
                # all tasks
                try:
                ...
                if 'TaskChain' in initial:
                    ...
                    while True:
                        ...
                        it += 1
                        if t in initial:
                            tname = payload.setdefault(
                                t, initial[t])['TaskName']
                            mem = mem_dict.setdefault(
                                tname, payload[t]['Memory'])
            ...
            if action.startswith('multicore') and actions[action] != "":
                set_to = int(actions[action])
                # Taskchains needs to be treated special to set the multicore
                # and memory values to all tasks
                if 'TaskChain' in initial:
                    mem_dict = payload['Memory'] if isinstance(
                    ...
                    while True:
                        ...
                        if t in initial:
                            ...
                            # Need to scale the memory by the new number of
                            # cores
                            ...
                            mem_dict[tname] = int(
                                mem + (set_to - initial_cores) * mem_per_core_c)
            ...
            if action.startswith('split'):
                ...
                for key in initial:
                    if key == 'SplittingAlgo' and (
                            initial[key] in ['EventBased']):
                        split_alert = True
                        ...
                        for key2 in initial[key]:
                            if key2 == 'TaskName':
                                ...
                                if (recover_task == this_taskname) and (
                                        initial[key]['SplittingAlgo'] in ['EventBased']):
                                    # the task to be recovered is actually of
                                    # the wrong type to allow change of
                                    # splitting
                                    sendLog(
                                        'actor',
                ...
                if split_alert:
                    sendLog(
                        'actor', 'Cannot change splitting for %s' %
                        initial['RequestName'], level='critical')
        ...
        for action in actions:
            if action.startswith('split'):
                ...
                if actions[action] != 'Same' and actions[action] != 'max':
                    factor = int(actions[action][0:-1]
                                 ) if 'x' in actions[action] else 2
                    for split in splittings:
                        split_par = split['splitParams']
                        if split['splitAlgo'] in ['EventBased']:
                            sendLog(
                                'actor', "Changing the splitting on %s for %s is not permitted. Not changing." %
                                (split['splitAlgo'], initial["RequestName"]), level='critical')
                            continue
                ...
                if actions[action] != 'Same' and actions[action] != 'max' and actions[action] != '':
                    ...
                    for split in splittings:
                        ...
                        for act in [
                            ...
                            if act in split_par:
                                wfi.sendLog(
                                    'actor', 'Changing %s (%d) by a factor %d' %
                                    (act, split_par[act], factor))
                                split_par[act] /= factor
                                print "to", split_par[act]
                                break
   ...
   # CI = campaignInfo()
    ...
    for wfname in wf_list:
        ...
        to_force = action_list[wfname].get(
            'Action',
            None) == 'special' and action_list[wfname].get(
            'Parameters',
            {}).get(
            'action',
            None) in [
        ...
        if not tasks and to_acdc:
            sendLog(
                'actor',
                'Empty action submitted for workflow %s' %
                wfname,
                level='critical')
...
# ===========================================================
        if to_clone and options.do:
            ...
            comment = ""
            if 'comment' in tasks:
                comment = ", reason: " + tasks['comment']
            wfi.sendLog(
                'actor',
                "invalidating the workflow by traffic controller %s" %
                comment)

            # Reject all workflows in the family
            inv_results = invalidate(
                url, wfi, only_resub=False, with_output=True)
            all_good = all(inv_results)
            if all_good:
                wfi.sendLog('actor', "%s and children are rejected" % wfname)
            else:
                wfi.sendLog(
                    'actor', "Failed to reject the request and dependents")
                sendLog(
                    'actor', 'Failed to reject the familly of %s' %
                    wfname, level='critical')
                continue
            ...
            try:
                ...
                print str(e)
                # let's not remove the action other the workflow goes to
                # "trouble" and the WTC cannot set the action again
            if not cloned:
                recover = False
                wfi.sendLog('actor', 'Failed to create clone for %s!' % wfname)
                sendLog(
                    'actor', 'Failed to create clone for %s!' %
                    wfname, level='critical')

            else:
                wfi.sendLog(
                    'actor', "Workflow %s cloned into %s" %
                    (wfname, cloned))
...
# ===========================================================
            ...
            # need a way to verify that this is the first round of ACDC, since
            # the second round will have to be on the ACDC themselves
...
#                continue
            ...
            if WMErr:
                for task in WMErr:
                    ...
                    if 'Cleanup' in task:
                        continue
                    if 'jobfailed' not in WMErr[task]:
                        continue
                    else:
                    ...
                    if fulltaskname == task_info.pathName:
                        if task_info.taskType not in [
                            ...
                            wfi.sendLog(
                                ...
                                (fulltaskname,
                                 task_info.taskType))
                if fulltaskname not in where_to_run.keys():
                    wrong_task = True
                    wfi.sendLog(
                        'actor',
...
#                        wfi.sendLog('actor',"Skipping %s for now until Allie fixes memory parameter for TaskChain ACDCs."%wfname)
                if 'sites' not in actions:
                    assign_to_sites = []
                    for site in where_to_run[task]:
                        assign_to_sites.extend(SI.SE_to_CEs(site))
                    assign_to_sites = sorted(set(assign_to_sites))
                    print "Found", sorted(
                        assign_to_sites), "as sites where to run the ACDC at, from the acdc doc of ", wfname
                        ...
                        if options.do:
                            if recovering:
                                ...
                                sendLog(
                                    ...
                                    (wfname,
                                     len(recovering),
                                        num_tasks_to_recover,
                                        list(recovering)),
                                    level='critical')
                                wfi.sendLog(
                                    'actor', "%s has had %s/%s recoveries %s only" %
                                    (wfname, len(recovering), num_tasks_to_recover, list(recovering)))
                                break
                            else:
...
#                        sendLog('recoveror', "ACDC for %s can be done automatically"% wfname, level='critical')
                        ...
                        wfi.sendLog(
                            'actor', 'ACDC created for task %s. Actions taken \n%s' %
                    ...
                    # hackery for ACDC merge assignment
                        if wfi.request['RequestType'] == 'TaskChain' and 'Merge' in task.split(
                                '/')[-1]:
                ...
                # xrootd setttings on primary and secondary
                        ...
                        if 'secondary' in actions:
                            ...
                            # in case secondary is blank or not set to enabled
                            # or disabled
                        ...
                        if options.ass:
                            ...
                            sendLog(
                                'actor', "%s needs to be assigned" %
                                (acdc), level='critical')
                            wfi.sendLog(
                                'actor',
                                "%s needs to be assigned by hand" %
                                (acdc))
                            continue
 ...
 #                       print parameters
                        ...
                        try:
                            if jira_comment:
                                ...
                                if len(jiras) == 1:
                                    # put a comment on the single corresponding
                                    # ticket
...
if __name__ == '__main__':
    ...
    parser.add_option(
        '--spec',
        default=None,
        help='a specific workflow to consider')
    parser.add_option(
        '--actions',
        default=None,
        help='a file name with the actions to be taken')
