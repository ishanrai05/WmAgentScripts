...
for wf in wfs:
    ...
    for wq in wqs:
        ...
        if wqe['Status'] in ['Running']:
            ...
            for running_agent,info in stats['AgentJobInfo'].items():
                if not agent in running_agent: continue ## look only at the agent with running blocks
                for task,tinfo in info['tasks'].items():
        ...
        for b in wqe['Inputs']:
            ...
            if not '#' in b: 
                ...
                for rdoc in doc:
                    in_files.update( list(rdoc['files'].keys()) )
...
if agents_down:
    for agent,tasks in agents_down.items():
...
RB = replacedBlocks()
for site,blocks in list(try_me.items()):
