...
for wf in wfs:
    if spec and spec not in wf['RequestName']:
        continue
    ...
    if any([len(s['taskName']) > task_path_name_limit for s in splits]):
        ...
        msg = "The full task path name exceeds the limit of {} characters. The requestor should either shorten task names or have fewer tasks in the chain.".format(
            task_path_name_limit)
        wfi.sendLog("GQ", msg)
    ...
    if 'AgentJobInfo' not in stats:
        stats['AgentJobInfo'] = {}

    # skip wf that unified does not know about, leaves acdc
    wfo = session.query(Workflow).filter(
        Workflow.name == wf['RequestName']).first()
    ...
    if 'TotalInputLumis' in wf and 'TotalEstimatedJobs' in wf and wf['TotalEstimatedJobs']:
        ...
        if heavy > 5000. and 'InputDataset' not in wf:
            heavy_duty[wf['RequestName']] = {'TotalInputLumis': wf['TotalInputLumis'],
                                             'TotalEstimatedJobs': wf['TotalEstimatedJobs'], 'ratio': heavy, }
    ...
    for wq in wqs:
        ...
        if wqe['Status'] in ['Running']:
            ...
            # find out the numbe of jobs running on that workflow, from that
            # agent
            for running_agent, info in stats['AgentJobInfo'].iteritems():
                if agent not in running_agent:
                    continue  # look only at the agent with running blocks
                for task, tinfo in info['tasks'].iteritems():
                    if 'LogCollect' in task:
                        continue
                    if 'Cleanup' in task:
                        continue
                    if 'status' not in tinfo:
                        # should also look for the previous step that has
                        # nothing running
                        if task in agents_down[agent]:
                            continue
                        print task, "stalled in the agent:", agent
                        a_stall = False
                        for s in all_input_loc:
                            if s not in si.sites_pressure:
                                continue
                            matching, running, _ = si.sites_pressure[s]
                            maxcpu = si.cpu_pledges.get(s, -1)
                            one_stall = ((running + matching) < maxcpu)
                            print "%20s idled %10d running %10d : available %10d %s" % (
                                s, matching, running, maxcpu, 'READY' if one_stall else '')
                            if one_stall:
                                a_stall = True
                        if a_stall:
        ...
        if wqe['NoInputUpdate']:
            # input is remote: one day we'd like to move it to disk
            # automatically, but not now
        ...
        for secondary in wqe['PileupData']:
            if pileup_location is None:
                ...
                pileup_location = list(
                    set(pileup_location) & set(
                        wqe['PileupData'][secondary]))
        ...
        if not swl:
            sendLog(
                'GQ',
                "There is no site at which the workflow %s can run Was provided with %s" %
                (wf['RequestName'],
                 ','.join(
                    wqe['SiteWhitelist'])),
                level='critical')
            wfi.sendLog(
                'GQ',
                "There is not site at which the workflow can run. Was provided with %s" %
                (','.join(
                    wqe['SiteWhitelist'])))
            continue
        ...
        for b in wqe['Inputs']:
            ...
            if '#' not in b:
                ...
                original = workflowInfo(
                    url, wfi.request['OriginalRequestName'])
                ...
                dataset = None
                for dataset in prim:
                    if dataset not in files_locations:
                        files_locations[dataset] = getDatasetFileLocations(
                            url, dataset)
                ...
                if dataset:
                    ...
                    for in_file in in_files:
                        ...
                        if in_file in files_locations[dataset]:
                            ...
                            site_with_data_and_listed = list(
                                set(files_se) & set(swl))
                            if not site_with_data_and_listed:
                                print "File", in_file, "is at", ",".join(
                                    files_se), "while asked to run at", ",".join(sorted(swl))
                ...
                acdc_location = sorted(
                    set([si.CE_to_SE(s) for s in wqe['Inputs'][b]]))
                # meaning we have a secondary and we care about it
                if pileup_location is not None and not wqe['NoPileupUpdate']:
                    print "intersecting with secondary", ','.join(
                        sorted(pileup_location))
                    acdc_location = list(
                        set(acdc_location) & set(pileup_location))
                ...
                se_whitelist = sorted(
                    set([si.CE_to_SE(s) for s in wqe['SiteWhitelist'] if s in si.sites_ready]))
                missing_in_whitelist = sorted(
                    [si.SE_to_CE(s) for s in (set(acdc_location) - set(se_whitelist))])
                # (se_whitelist>=acdc_location):
                if wqe['NoInputUpdate'] == False and missing_in_whitelist:
            ...
            # b is the block
            ds = b.split('#')[0]
            if ds not in block_locations:
                s_block_locations[ds] = getDatasetBlockAndSite(
                    url, ds, complete='y', vetoes=[])
                for s in s_block_locations[ds]:
                    for bl in s_block_locations[ds][s]:
                        block_locations[ds][bl].append(s)

            if b not in block_locations[ds]:
            ...
            # in case there is an element for which the intersection of actual
            # location and site listed is empty
            if not site_with_data_and_listed:
                wfs_no_location_in_GQ[wqe['RequestName']].append(
                    (wq['_id'], b, swl))
                unprocessable.add(b)
                not_processable.add(b)
                print "Block", b, "is at", ",".join(
                    sorted(block_se)), "while asked to run at", ",".join(
                    sorted(swl))
        ...
        if not_processable:
            wfi.sendLog(
                'GQ', 'The following blocks/files need to be put back on disk \n%s' %
...
eosFile(
    ...
    'w').write(
        json.dumps(
            sorted(stuck_all_done),
            indent=2)).close()

if failed_workflow:
    sendLog(
        'GQ',
        'These workflows have failed wqe and will stay stuck:\n%s' %
...
if unprocessable:
    ...
    eosFile(
        '%s/missing_blocks.json' %
        monitor_dir,
        'w').write(
        json.dumps(
            sorted(unprocessable),
            indent=2)).close()
...
if heavy_duty:
    sendLog(
        'GQ',
        'There are some heavy duty workflows in the system, likely to cause damage\n%s\n%s' %
        (sorted(
            heavy_duty.keys()),
            json.dumps(
            heavy_duty,
