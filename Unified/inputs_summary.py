...
for status in [
    ...
    all_checks[status] = session.query(Workflow).filter(
        Workflow.status.startswith(status)).all()
    check_those.extend(all_checks[status])


check_those = filter(lambda wfo: 'RunIISummer16DR80' in wfo.name, check_those)
check_those = filter(
    lambda wfo: 'RunIISummer16DR80Premix' in wfo.name,
    check_those)
...
if sys.argv[1] == 'pertape':
    ...
    for wfo in check_those:
        ...
        fraction = max([f for s, (t, f) in i['on_disk'].items()]) / \
            100. if i['on_disk'] else 0.
    ...
    for prio in sorted(by_prio.keys(), key=lambda o: int(o)):
        ...
        for site in by_site:
            ...
            sum_by_site[site] = int(sum([o['staging']
                                         for wf, o in by_site[site].iteritems()]))
            disk_by_site[site] = int(sum([o['staged']
                                          for wf, o in by_site[site].iteritems()]))
            top_10[site] = dict(sorted([(wf,
                                         dict([(kk,
                                                vv) for kk,
                                               vv in o.items() if kk in ['workflow',
                                                                         'staged',
                                                                         'staging',
                                                                         'status',
                                                                         'wm_status',
                                                                         ...
                                                                         'fraction',
                                                                         'available']])) for (wf,
                                                                                              o) in by_site[site].iteritems()],
                                       key=lambda k: k[1]['staging'],
                                       reverse=True)[:top])
            print [o['staging'] for o in top_10[site].values()]
            if site in rate:
                # in days
                delay_by_site[site] = '%.2f' % (
                    (sum_by_site[site] / float(rate[site])) / (60 * 60 * 24))
        print "in GB"
        ...
        print "worse input"
        for site in top_10:
            l_top_10[site] = sorted([o for wf,
                                     o in top_10[site].items()],
                                    key=lambda o: o['staging'],
                                    reverse=True)
    ...
    for site in top_top_10:
        top_top_10[site] = dict(sorted([(wf, o) for wf, o in top_top_10[site].items(
        )], key=lambda k: k[1]['staging'], reverse=True)[:top])
        l_top_top_10[site] = sorted([o for wf,
                                     o in top_top_10[site].items()],
                                    key=lambda o: o['staging'],
                                    reverse=True)
    ...
    o.write("#" * 40 + "\n")
    o.write(
        "Total input already on disk per priority in GB\n%s\n" %
        json.dumps(
            i_on_disk,
            indent=2))
    o.write(
        "Total input still on tape per priority in GB\n%s\n" %
        json.dumps(
            i_on_tape,
            indent=2))
    o.write(
        "Approximate retrieval time in days\n%s\n" %
        json.dumps(
            tape_delay,
            indent=2))
    o.write("#" * 40 + "\n")
...
if not redo:
    check_those = [
        wfo for wfo in check_those if wfo.name not in all_info.keys()]
...
for wfo in check_those:
    if wfo.name in all_info:
        if 'updatded' in all_info[wfo.name] and (
                now - all_info[wfo.name]['updated']) < delay:
            print "too fresh"
            continue
