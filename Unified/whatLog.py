...
parser.add_option(
    '--logfile',
    help='What log file in particular',
    default=None)
parser.add_option('--task', help='What task to look for', default=None)
parser.add_option(
    '--get',
    help='Retrieve the log file from eos',
    default=False,
    action='store_true')
parser.add_option(
    '--local',
    help='Where to get the log file',
    default='/tmp/%s' %
    (os.getenv('USER')))
...
if options.logfile:
    if options.workflow:
        ...
        logs = session.query(LogRecord).filter(
            LogRecord.logfile == options.logfile).filter(
            LogRecord.workflow == options.workflow).all()
    else:
        #logs = session.query(LogRecord).filter(LogRecord.logfile.contains( options.logfile)).all()
        logs = session.query(LogRecord).filter(
            LogRecord.logfile == options.logfile).all()
else:
    if options.workflow:
        if options.task:
            ...
            logs = session.query(LogRecord).filter(
                LogRecord.workflow == options.workflow).filter(
                LogRecord.task == options.task).all()
        else:
...
for log in logs:
    ...
    if options.get:
        out_dest = ('%s/%s' %
                    (options.local, log.path.split('/')[-1])).replace('//', '/')
        if options.eos:
            com = (
                '%s cp %s %s' %
                (eos,
        ...
        #                                                                        out_dest,
        # log.logfile)
        com = "cd %s ; tar xvf %s `tar tvf %s | grep %s | awk '{print $NF}'`" % (
            options.local, log.path, log.path, log.logfile)
        print com
        os.system(com)
        # os.system( 'rm -f %s' % )
