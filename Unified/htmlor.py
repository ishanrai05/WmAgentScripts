...
def htmlor(caller=""):
    ...
    for backup in ['statuses.json', 'siteInfo.json', 'equalizor.json']:
        ...
        os.system(
            'cp %s/%s /afs/cern.ch/user/c/cmst2/www/unified/.' %
            (monitor_pub_dir, backup))
    ...
    def wfl(
        ...
        if pid:
            if ms:
                ...
                text += ', <a href="https://dmytro.web.cern.ch/dmytro/cmsprodmon/workflows.php?prep_id=%s" target="_blank">ac</a>' % (
                    wl_pid)
        text += ', <a href="https://%s/couchdb/workqueue/_design/WorkQueue/_rewrite/elementsInfo?request=%s" target="_blank">gq</a>' % (
            reqmgr_url, wfn)
        text += ', <a href="https://its.cern.ch/jira/issues/?jql=(text~%s OR text~task_%s) AND project = CMSCOMPPR" target="_blank">jira</a>' % (
            pid, pid)
        if status:
        ...
        if ongoing:
            ...
            if wfs != 'acquired':
                text += '<a href="https://cms-gwmsmon.cern.ch/prodview/%s" target="_blank"><img src="https://cms-gwmsmon.cern.ch/prodview/graphs/%s/daily" style="height:50px"></a>' % (
                    wfn, wfn)
        ...
        if ongoing:
           ...
           #date2 = time.strftime('%Y-%m-%d+%H:%M', time.gmtime())
            ...
            # text+=', <a
            # href="http://dashb-cms-job.cern.ch/dashboard/templates/web-job2/#table=Jobs&date1=%s&date2=%s&sortby=site&task=wmagent_%s">
            # 1h</a>'%( date1, date2, wfn )
    ...
    view_modules = [
        ...
        'recoveror',
        'actor',
        'closor'] + view_not_a_module

    all_modules = list(set(view_modules + ['actor',
                                           'addHoc',
                                           'assignor',
                                           'batchor',
                                           'checkor',
...
""" %
        (time.asctime(
            ...
            time.gmtime()), reqmgr_url, ', '.join(
                [
                    '<a href=https://cms-unified.web.cern.ch/cms-unified/showlog/?search=critical&module=%s&limit=100 target=_blank><b><font color=red>%s</b></font></a>' %
                    (m, m) for m in ['heartbeat'] + view_modules])))
    ...
    for wf in session.query(Workflow).filter(
        ...
        #print wf.name
        text += "<li> %s (%d) </li> \n" % (wfl(wf, p=True),
                                           int(wl['RequestPriority']))
        count += 1
    text_by_c = ""
    for c in count_by_campaign:
        text_by_c += '<li><a href=https://dmytro.web.cern.ch/dmytro/cmsprodmon/workflows.php?campaign=%s&in_production=1>%s</a> <a href="https://its.cern.ch/jira/issues/?jql=text~%s AND project = CMSCOMPPR">JIRA</a> (%d) : ' % (
            c, c, c, sum(count_by_campaign[c].values()))
...
""" % (count,
    ...
    all_active = sorted(set([ts.phedexid for ts in session.query(
    ...
    for phedexid in all_active:
        ...
        stext = '<li> %s serves %d workflows<br><a href="javascript:showhide(\'%s\')">[show/hide]</a> <div id="%s" style="display:none;"><ul>\n' % (
            phl(phedexid), t_count, phedexid, phedexid) + stext
    ...
    try:
        stuck_transfer = json.loads(
            eosRead(
                '%s/stuck_transfers.json' %
                monitor_pub_dir))
    except BaseException:
        stuck_transfer = {}
...
""" % (count,
    ...
    html_doc.write(
        """Worflow ready for assigning (%d) <a href=logs/stagor/last.log target=_blank>log</a> <a href=logs/assignor/last.log target=_blank>postlog</a> <a href=GQ.txt target=_blank> GQ</a>
...
""" %
    ...
    for wf in session.query(Workflow).filter(Workflow.status == 'away').all():
        wl = getWL(wf.name)
        count_by_campaign[wl['Campaign']][int(wl['RequestPriority'])] += 1
        # color = 'orange' if wf.name in relvals else 'black' ## this
        # difference of color can be put back somehow using batchInfo
        color = 'black'
        lines.append("<li> <font color=%s>%s</font> <hr></li>" %
                     (color, wfl(wf, view=True, ongoing=True)))
        count += 1
    text_by_c = ""
...
""" % (c, c, c,
        ...
        for p in sorted(count_by_campaign[c].keys()):
            text_by_c += "%d (%d), " % (p, count_by_campaign[c][p])
        text_by_c += '<img src=https://dmytro.web.cern.ch/dmytro/cmsprodmon/images/%s-history_nevents-limit-30.png style="height:70px">' % (
            c)
        text_by_c += '<img src=https://dmytro.web.cern.ch/dmytro/cmsprodmon/images/%s-history_requests-limit-30.png style="height:70px">' % (
            c)
        text_by_c += "</li>"
...
""" % (len(lines),
    ...
    text = ""
    count = 0
    # for wf in session.query(Workflow).filter(Workflow.status ==
    # 'assistance-custodial').all():
    for wf in session.query(Workflow).filter(
...
""" % count)
    ...
    for assistance_type in sorted(assistance_by_type.keys()):
        text += "<li> %s (%d) <a href=\"javascript:showhide('%s')\">[Click to show/hide]</a><br><div id=\"%s\" style=\"display:none;\"><ul>" % (
            assistance_type, len(assistance_by_type[assistance_type]), assistance_type, assistance_type, )
...
""" % count)
    ...
    all_locks = [l.item.split('#')[0] for l in session.query(
        Lock).filter(Lock.lock).all() if l.item]
    try:
        waiting_custodial = json.loads(
            eosRead(
                '%s/waiting_custodial.json' %
                monitor_dir))
    except Exception as e:
        print str(e)
        print "eos is screwing with us"
        waiting_custodial = {}
    all_pending_approval_custodial = dict([(k, item) for k, item in waiting_custodial.items(
    ) if 'nodes' in item and not any([node['decided'] for node in item['nodes'].values()])])
    ...
    try:
        missing_approval_custodial = json.loads(
            eosRead(
                '%s/missing_approval_custodial.json' %
                monitor_dir))
    except Exception as e:
        print str(e)
        print "eos is screwing with us"
    ...
    if len(stuck_custudial):
        stuck_string = ', <font color=red>%d appear to be <a href=public/stuck_custodial.json>stuck</a></font>' % len(
            stuck_custudial)
    else:
        stuck_string = ''
    ...
    for ds in waiting_custodial:
        ...
        if out:
            ...
            a_waiting_custodial_string = "<li>on week %s : %s %s</li>" % (time.strftime(
                "%W (%x %X)",
                time.gmtime(
                    out.date)),
                ol(
                out.datasetname),
                ' %s [GB] %s to %s on %s (<a href="https://cmsweb.cern.ch/phedex/datasvc/xml/prod/requestlist?dataset=%s&node=T*MSS">%d missing</a>)' % (size,
                                                                                                                                                         action,
                                                                                                                                                         destination,
                                                                                                                                                         time.asctime(
                                                                                                                                                             time.gmtime(
            ...
            if out.datasetname in waiting_custodial:
                ...
                try:
                    ...
                    custodial = ' %s [GB] %s to %s on %s (<a href="https://cmsweb.cern.ch/phedex/datasvc/xml/prod/requestlist?dataset=%s&node=T*MSS">%d missing</a>)' % (
                        size, action, destination, time.asctime(time.gmtime(info['checked'])), out.datasetname, info['nmissing'])
                except Exception as e:
                    #print info
                    #print str(e)
                    pass
    ...
    per_day_s = ", ".join(["day %s (%d)" % (day, per_day_this_week[day])
                           for day in sorted(per_day_this_week.keys())])
...
""" %
        (len(lines_lastweek) +
         ...
         n_pending_approval,
         long_approve_string,
         len(lagging_custudial),
         UC.get('transfer_timeout'),
         stuck_string,
         len(waiting_custodial),
         waiting_custodial_string,
         len(lines_lastweek),
...
""" %
    ...
    for module_name in all_modules:
        if module_name in ['messagor', 'cleanor']:
            continue
    ...
    html_doc.write(
        "<table border=1><thead><tr><th>Module</th><th>Last Ran</th><th>Last Runtime</th><th>Avg Runtime</th></tr></thead>")
    ...
    for m in sorted(last_module.keys()):
        ...
        if since_last > (heart_beat_time_out * 60 * 60):  # 6h heart beat
            sendLog('heartbeat', "The module %s has not ran in %s hours, now %s" % (
                m, heart_beat_time_out, display_time(since_last)), level='critical')
        else:
            print "module %s has ran last since %s" % (
                m, display_time(since_last))
...
<tr>
 ...
 <td width=300>%s</td>
    ...
    for c in sorted(CI.campaigns.keys()):
        ...
        text += '<img src=https://dmytro.web.cern.ch/dmytro/cmsprodmon/images/%s-history_nevents-limit-30.png style="height:70px">' % (
            c)
        text += '<img src=https://dmytro.web.cern.ch/dmytro/cmsprodmon/images/%s-history_requests-limit-30.png style="height:70px">' % (
            c)
        text += """
...
# <ul>"""
    ...
    for team, agents in getAllAgents(reqmgr_url).items():
        for agent in agents:
            if 'WMBS_INFO' not in agent:
                continue
            if 'sitePendCountByPrio' not in agent['WMBS_INFO']:
                continue
            for site in agent['WMBS_INFO']['sitePendCountByPrio']:
                a = sum(
                    agent['WMBS_INFO']['sitePendCountByPrio'][site].values())
                #print site,team,a
                #print a
                #print agent['WMBS_INFO']['sitePendCountByPrio'][site]
                if a:
                    upcoming_by_site[team][site] += a
    ...
    for t in [
        ...
        for site in sorted(getattr(SI, t)):
            ...
            if c == 0:
                text += "<tr>"
            if not disk:
                ht_disk = '<a href=remaining_%s.html><font color=red>Disk available: %s</font></a>' % (
                    SI.CE_to_SE(site), disk)
            else:
            ...
            if site_se in upcoming:
                ...
                up_com += "<br><a href=GQ.json>Jobs available  %d</a> (av./pl.=%.2f)" % (
                    u, (u / float(cpu) if usage else 0))
                if usage:
                    available_ratios[site] = u / float(cpu)
            ...
            for team in ['production', 'relval']:
                if site in upcoming_by_site[team]:
                    if upcoming_by_site[team][site]:
                        ...
                        up_com += '<br>%s jobs upcoming %d' % (
                            team, upcoming_by_site[team][site])
            text += '<td>'
            ...
            text += '<a href="https://cms-gwmsmon.cern.ch/prodview/%s" target="_blank"><img src="https://cms-gwmsmon.cern.ch/prodview/graphs/%s/daily" style="height:75px"></a>' % (
                site, site)
            text += '<img src="https://cms-gwmsmon.cern.ch/totalview/graphs/%s/fairshare/daily" style="height:75px"><br>' % (
                site)
            text += '<img src="https://cms-gwmsmon.cern.ch/prodview/graphs/siteprioritysummarycpuspending/%s/daily" style="height:75px">' % (
                site)
            text += '<img src="https://cms-gwmsmon.cern.ch/prodview/graphs/siteprioritysummarycpusinuse/%s/daily" style="height:75px"><br>' % (
                site)
            ...
            # text+=', <a
            # href="http://dashb-cms-job.cern.ch/dashboard/templates/web-job2/#user=&refresh=0&table=Jobs&p=1&records=25&activemenu=1&site=%s&submissiontool=&check=submitted&sortby=activity&scale=linear&bars=20&date1=%s&date2=%s">1h</a>'%(
            # site,date1h,now )
            text += ', <a href="https://cms-site-readiness.web.cern.ch/cms-site-readiness/SiteReadiness/HTML/SiteReadinessReport.html#%s">SAM</a><br>' % (
                site)
            text += '%s<br>' % (ht_cpu)
    ...
    def outliers(val_d, trunc=0.9, N=4):
        sub_vals = filter(lambda v: v < trunc *
                          max(val_d.values()), val_d.values())
        outl = {}
        if sub_vals:
    ...
    if outlier_upcoming:
        sendLog('GQ', 'There is an inbalance of upcoming work at %s' % (', '.join(
            [site for site in sorted(outlier_upcoming.keys())])), level='warning')
...
""" % (desc, div_name, div_name)
    ...
    for site in sorted(SI.sites_not_ready):
        text += '<li> %s <a href="https://cms-site-readiness.web.cern.ch/cms-site-readiness/SiteReadiness/HTML/SiteReadinessReport.html#%s">SAM</a><br>' % (
            site, site)
    text += "</ul></div></li>"
    ...
    try:
        equalizor = json.loads(
            eosRead(
                '%s/equalizor.json' %
                monitor_pub_dir))['reversed_mapping']
    except BaseException:
        time.sleep(10)
        try:
            equalizor = json.loads(
    ...
    oo.write(json.dumps(dict([(t, getattr(SI, t)) for t in [
             'sites_T0s', 'sites_T1s', 'sites_T2s', 'sites_with_goodIO']]), indent=2))
    oo.close()
...
""" % (site))
    ...
    oos_tables = "\n".join(
        [info[0] for site, info in chart_data.items() if site in out_of_space])
    oos_draws = "\n".join(
        [info[1] for site, info in chart_data.items() if site in out_of_space])
...
""" %
                                    (site, ','.join(['"%s"' %
                                                     reason for reason in sorted(all_reasons)]), site, ','.join(['%s' %
                                                                                                                 (by_reason_at_site[site][reason]) for reason in sorted(all_reasons)])))
...
""" % (site))
    ...
    for i_oos, site in enumerate(sorted(by_reason_at_site.keys())):
        ...
        print rem_link
        rem_divs_table += "<td>%s %s</td>" % (
            rem_chart_data[site][2], rem_link)
...
""")
    ...
    for team, agents in getAllAgents(reqmgr_url).items():
        if team not in ['production', 'relval', 'highprio']:
            continue
        ...
        for agent in agents:
            ...
            if agent['status'] in ['error', 'down']:
                ...
                sendLog(
                    'htmlor', 'Agent %s with %d component down: %s' %
                    (name, len(
                        agent['down_components']), ", ".join(
                        agent['down_components'])), level='warning')
                bgcolor = 'bgcolor=red'
            ...
            for component in agent['down_components']:
                ...
                for det in agent['down_component_detail']:
                    if isinstance(det, dict) and det['name'] == component:
                        message += '<br>%s' % det['error_message']
                        this_week = str(
                            int(time.strftime("%W", time.gmtime())))
                        alert_type = agent['status']
                        if not JC:
                            continue
                        if component in component_auto_restart:
                            continue
                        ...
                        alert_summary = '%s %s %s' % (
                            short_name, component, alert_type)
                        ...
                        if alert_type is not 'heartbeat':
                            ...
                            if len(jiras) == 0:
                                ...
                                # find the last such, and add a comment after
                                # the grace period
                                j = sorted(
                                    jiras, key=lambda o: JC.created(o))[-1]
                                ...
                                # 4h at least between pings in the agent
                                # comment
                                if reopened or (
                                    seconds_since > (
                                        agent_comment_graceperiod * 60 * 60)):
...
""")
    ...
    eosFile(
        '%s/statuses.json' %
        monitor_pub_dir).write(
        json.dumps(wfs)).close()
