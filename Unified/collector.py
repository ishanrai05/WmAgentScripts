...
def collector(url, specific, options):
    ...
    datasets = list(will_be_used.keys())
    if options.limit:
    ...
    for dataset in datasets:
        ...
        for request in will_be_used[dataset]:
            ...
            prim_location = [site for (site,info) in list(destinations.items()) if info['completion']==100 and info['data_fraction']==1]
            prim_destination = [site for site in list(destinations.keys()) if not site in prim_location]
            ...
            copies_being_made = [ sum([list(info['blocks'].keys()).count(block) for site,info in list(destinations.items()) if site in prim_destination]) for block in all_block_names]
            ...
            for (site,items) in list(spreading.items()):
    ...
    for (site,items_to_transfer) in all_transfers.items():
