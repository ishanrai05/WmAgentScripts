...
def collector(url, specific, options):
    ...
    for campaign, status in itertools.product(
            fetch_in_campaigns, mcm_statuses):
        queries = []
        if campaign:
    ...
    for dataset in datasets:
        ...
        #NL.lock( dataset )
        # we could get the reqmgr dictionnary from McM if it was implemented
        # and use standard workflowInfo !!!
        for request in will_be_used[dataset]:
            ...
            destinations, all_block_names = getDatasetDestinations(
                url, dataset, within_sites=[SI.CE_to_SE(site) for site in sites_allowed])
            print json.dumps(destinations, indent=2)
            prim_location = [site for (site, info) in destinations.items(
            ) if info['completion'] == 100 and info['data_fraction'] == 1]
            prim_destination = [
                site for site in destinations.keys() if site not in prim_location]
            prim_destination = [site for site in prim_destination if not any(
                [osite.startswith(site) for osite in SI.sites_veto_transfer])]
            copies_needed = max(0, copies_needed - len(prim_location))
            copies_being_made = [sum([info['blocks'].keys().count(block) for site, info in destinations.items(
            ) if site in prim_destination]) for block in all_block_names]

            prim_to_distribute = [
                site for site in sites_allowed if not SI.CE_to_SE(site) in prim_location]
            prim_to_distribute = [
                site for site in prim_to_distribute if not SI.CE_to_SE(site) in prim_destination]
            # take out the ones that cannot receive transfers
            prim_to_distribute = [site for site in prim_to_distribute if not any(
                [osite.startswith(site) for osite in SI.sites_veto_transfer])]
            ...
            if copies_needed:
                ...
                chops, sizes = getDatasetChops(
                    dataset, chop_threshold=options.chopsize)
                spreading = distributeToSites(
                    chops,
                    prim_to_distribute,
    ...
    for (site, items_to_transfer) in all_transfers.iteritems():
        ...
        blocks = [block for block in blocks if not block.split('#')[
            0] in datasets]
...
if __name__ == "__main__":
    ...
    parser.add_option(
        '--test',
        '-t',
        help='only test and see what is going out',
        default=True,
        action='store_true')
    parser.add_option(
        '--spread',
        '-s',
        help='use the campaign to try and preplace properly the dataset',
        default=False,
        action='store_true')
    parser.add_option(
        '-c',
        '--chopsize',
        help='The threshold for choping input dataset',
        default=UC.get('chopping_threshold_in_GB'),
        type=int)
    parser.add_option(
        '--limit',
        '-l',
        help='limit the amount of dataset to browse for',
        default=200,
        type=int)
