...
def singleRecovery(url, task, initial, actions, do=False):
    ...
    if actions:
        for action in actions:
            ...
            if action.startswith('split') and (initial['RequestType'] in ['MonteCarlo'] or (
                    initial['RequestType'] in ['TaskChain'] and 'InputDataset' not in initial['Task1'])):
...
def new_recoveror(url, specific, options=None):
    ...
    try:
        from_operator = json.loads(
            os.popen('curl -s http://vocms0113.cern.ch/actions/test.json').read())
    ...
    for wfo in wfs:
        ...
        if not specific and 'manual' in wfo.status:
            continue
        ...
        for task in task_to_recover:
            ...
            if not_ready:
                ...
                injection_time = time.mktime(time.strptime('.'.join(
                    map(str, wfi.request['RequestDate'])), "%Y.%m.%d.%H.%M.%S")) / (60. * 60.)
        ...
        if send_recovery:
            ...
            for task in task_to_recover:
                actions = list(set([case['solution']
...
def recoveror(url, specific, options=None):
    ...
    for wfo in wfs:
        ...
        # need a way to verify that this is the first round of ACDC, since the
        # second round will have to be on the ACDC themselves
        ...
        for task, errors in all_errors.items():
            print "\tTask", task
            # collect all error codes and #jobs regardless of step at which it
            # occured
            all_codes = []
            for name, codes in errors.items():
                if isinstance(codes, int):
                    continue
                all_codes.extend([(int(code),
                                   info['jobs'],
                                   name,
                                   list(set([e['type'] for e in info['errors']])),
                                   list(set([e['details'] for e in info['errors']]))) for code,
                                  info in codes.items()])
        ...
        if len(task_to_recover) != len(all_errors):
            ...
            sendLog(
                'recoveror',
                'do not want to make partial acdc on %s' %
                wfo.name,
                level='warning')
            recover = False
        ...
        if task_to_recover and recover:
            ...
            for task in task_to_recover:
                ...
                actions = list(set([case['solution']
                                    for code, case in task_to_recover[task]]))
                acdc = singleRecovery(
                    url, task, wfi.request, actions, do=options.do)
                ...
                if not acdc:
                    if options.do:
                        ...
                        sendLog(
                            'recoveror', "ACDC for %s can be done automatically" %
                            wfo.name, level='critical')
                        continue

                # and assign it ?
                team = wfi.request['Team']
                # assign_to_sites = set(SI.sites_ready) ## that needs to be
                # massaged to prevent assigning to something out.
