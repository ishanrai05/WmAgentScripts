...
class ParseBuster(threading.Thread):
    ...
    def run(self):
        self.task_error, self.one_explanation = parse_one(
            self.url, self.wfn, self.options)
...
class AgentBuster(threading.Thread):
    ...
    def run(self):
        ...
        os.system(
            'ssh %s %s/WmAgentScripts/Unified/exec_expose.sh %s %s %s %s %s %s' %
            (self.agent,
             self.base_eos_dir,
             self.workflow,
             self.wmbs,
...
class ReadBuster(threading.Thread):
    ...
    def run(self):
        self.readable = (
            os.system(
                'XRD_REQUESTTIMEOUT=10 xrdfs root://cms-xrd-global.cern.ch stat %s' %
                self.file) == 0)
                        ...
                        if True:
                            if True:
                                if True:
                                    ...
                                    os.system('mkdir -p /tmp/%s' %
                                              (os.getenv('USER')))
                                    local = '/tmp/%s/%s' % (os.getenv('USER'),
                                                            self.out_lfn.split('/')[-1])
                                    #command = 'xrdcp root://cms-xrd-global.cern.ch/%s %s'%( self.out_lfn, local)
                                    command = 'XRD_REQUESTTIMEOUT=10 xrdcp root://cms-xrd-global.cern.ch/%s %s' % (
                                        self.out_lfn, local)
                                    ...
                                    if os.system(
                                            'ls %s' % local) != 0 and self.from_eos:
                                        print "no file retrieved using xrootd, using eos source"
                                        # will parse eos and not doing anything
                                        # for things already indexed
                                        date_option = ""
                                        if self.date:
                                            this_month = int(
                                                time.strftime("%m", time.gmtime()))
                                            N_months_back = 3
                                            this_year = int(
                                                time.strftime("%Y", time.gmtime()))
                                            if this_month - N_months_back < 0:
                                                m_s = ','.join(map(str, list(
                                                    range(1, this_month + 1)) + list(range(this_month - N_months_back + 12, 13))))
                                                y_s = '%d,%d' % (
                                                    this_year, this_year - 1)
                                            else:
                                                m_s = ','.join(
                                                    map(str, range(this_month - N_months_back, this_month + 1)))
                                                y_s = str(this_year)
                                            date_option = ' --year %s --month %s ' % (
                                                y_s, m_s)
                                            print "using", date_option, "to find logs on eos"
                                        os.system(
                                            'Unified/createLogDB.py --workflow %s %s ' %
                                            (self.wfn, date_option))
                                        os.system('Unified/whatLog.py --workflow  %s --log %s --get' % (
                                            self.wfn, self.out_lfn.split('/')[-1]))
                                        os.system('mv `find /tmp/%s/ -name "%s"` %s' %
                                                  (os.getenv('USER'), self.out_lfn.split('/')[-1], local))
                                    ...
                                    if os.system('ls %s' % local) == 0:
                                        ...
                                        actual_logs = os.popen(
                                            'find %s -type f' % (m_dir)).read().split('\n')
                                        for fn in actual_logs:
                                            ...
                                            if not fn.endswith('log'):
                                                continue
                                            if any(
                                                    [p in fn for p in ['stdout.log']]):
                                                trunc = '/tmp/%s/%s' % (
                                                    os.getenv('USER'), label)
                                                #print fn
                                                #print trunc
                                                head = tail = 1000
                                                os.system(
                                                    '(head -%d ; echo;echo;echo "<snip>";echo;echo ; tail -%d ) < %s > %s' %
                                                    (head, tail, fn, trunc))
                                                os.system(
                                                    'mv %s %s.trunc.txt' %
                                                    (trunc, fn))
...
def parse_one(url, wfn, options=None):
    ...
    for agent in stat['AgentJobInfo']:
        for task in stat['AgentJobInfo'][agent]['tasks']:
            if 'status' not in stat['AgentJobInfo'][agent]['tasks'][task]:
                continue
            for status in stat['AgentJobInfo'][agent]['tasks'][task]['status']:
                ...
                if isinstance(info, dict):
                    status_per_task[task][status] += sum(
                        stat['AgentJobInfo'][agent]['tasks'][task]['status'][status].values())
                else:
    ...
    html += '<a href=https://dmytro.web.cern.ch/dmytro/cmsprodmon/workflows.php?prep_id=%s>ac</a>, ' % (
        wfi.request['PrepID'])
    html += '<a href=https://cms-gwmsmon.cern.ch/prodview/%s>gwmsmon</a>, ' % (
        wfn)
    html += '<a href="https://cmsweb.cern.ch/couchdb/workqueue/_design/WorkQueue/_rewrite/elementsInfo?request=%s">WQE, </a>' % (
        wfn)
    r_type = wfi.request.get(
        'OriginalRequestType',
        wfi.request.get(
            'RequestType',
            'NaT'))
    ...
    if prim:
        ...
        for dataset in prim:
            html += '<b>%s </b>(events/lumi ~%d)' % (dataset,
                                                     getDatasetEventsPerLumi(dataset))

            available = getDatasetBlocksFraction(
...
""" % (dash_board_h)
    ...
    for task in tasks:
        ...
        expose_archive_code = dict([(str(code), defaultdict(
            lambda: n_expose)) for code in UC.get('expose_archive_code')])
        expose_condor_code = dict([(str(code), defaultdict(
            lambda: n_expose)) for code in UC.get('expose_condor_code')])
        ...
        for agent in stat['AgentJobInfo']:
            for site in stat['AgentJobInfo'][agent]['tasks'].get(
                    task,
                    {}).get(
                    'skipped',
                    {}):
        ...
        if any([v in task.lower() for v in ['logcol', 'cleanup']]):
            continue
        ...
        count_top_N = min(sorted(s_per_code.values(), reverse=True)[
                          :expose_top_N]) if s_per_code else -1
        ...
        for exittype in err[task]:
            ...
            for errorcode_s in err[task][exittype]:
                ...
                #print "\t\t",err[task][exittype][errorcode_s].keys()
                force_code = (
                    count_top_N > 0 and s_per_code[errorcode_s] >= count_top_N)
                if force_code:
                    print "will expose", errorcode_s, "anyways"
                for site in err[task][exittype][errorcode_s]:
                    ce = SI.SE_to_CE(site)
                    count = err[task][exittype][errorcode_s][site]['errorCount']
                    ###total_count[errorcode_s] += count
                    ...
                    for sample in err[task][exittype][errorcode_s][site]['samples']:
                        ...
                        for step in sample['errors']:
                            for report in sample['errors'][step]:
                                if report['type'] == 'CMSExeption':
                                    continue
                                # if int(report['exitCode']) ==
                                # int(errorcode_s):
                                one_explanation[errorcode_s].add(
                                    "%s (Exit code: %s) \n%s" %
                                    (report['type'], report['exitCode'], report['details']))
                                per_task_explanation["%s:%s" % (task_short, errorcode_s)].add(
                        ...
                        if force_code:
                            if errorcode_s not in expose_condor_code:
                                expose_condor_code[errorcode_s] = defaultdict(
                                    lambda: n_expose)
                            if errorcode_s not in expose_archive_code:
                                expose_archive_code[errorcode_s] = defaultdict(
                                    lambda: n_expose)

                        if do_CL and (
                                (errorcode_s in expose_condor_code and expose_condor_code[errorcode_s][agent])) and 'cern' in agent:
                        ...
                        for out in sample['output']:
                            ...
                            if out['type'] == 'logArchive':
                                if do_JL and (
                                        (errorcode_s in expose_archive_code and expose_archive_code[errorcode_s][agent] > 0)):
                                    ...
                                    threads.append(LogBuster(
                                        ...
                                        monitor_eos_dir=monitor_eos_dir,
                                        wfn=wfn,
                                        date=wfi.request.get(
                                            'RequestDate', []),
                                        errorcode_s=errorcode_s,
                                        task_short=task_short,
                                        # if options else True),
                                        from_eos=(not options.not_from_eos),
                                    ))
        ...
        if missing_events:
            if feff != 1.:
                html += ' is missing %s events in input and <b>about %s events in output</b>' % (
                    "{:,}".format(missing_events), "{:,}".format(int(missing_events * feff)))
            else:
        ...
        for site in sorted(all_sites):
            ...
            if site not in SI.sites_ready:
                color = 'bgcolor=indianred'
                site_in = '<b>No</b>'
                if task in missing_to_run_at and missing_to_run_at[task][SI.CE_to_SE(
                        site)] == 0 or min_rank == task_rank:
                    color = 'bgcolor=aquamarine'
                    site_in = '<b>No</b> but fine'

            if not no_error:
                site_in += " (%s events)" % ("{:,}".format(
                    missing_to_run_at[task][SI.CE_to_SE(site)]) if task in missing_to_run_at else '--')
            html += '<tr><td %s>%s</td>' % (color, site)
            for code in sorted(all_codes):
                if code == notreported:
                    html += '<td %s width=200>%s events </td>' % (
                        color, "{:,}".format(missing_to_run_at[task][SI.CE_to_SE(site)]))
                else:
                    if error_site_count[code][site]:
                        er_frac = float(error_site_count[code][site]) / \
                            s_per_code[code] if s_per_code[code] else 0.
                        si_frac = float(error_site_count[code][site]) / \
                            total_per_site[site] if total_per_site[site] else 0.
                        ...
                        html += ', <font color=%s>&uarr; %.1f%%</font>, <font color=%s>&rarr; %.1f%%</font></td>' % (
                            palette(er_frac), 100. * er_frac, palette(si_frac), 100. * si_frac)
                    else:
    ...
    for task in reported_tasks:
        ...
        if len(needed_blocks_loc):
            html += "<b>Blocks (%d/%d) needed for %s</b><br>" % (
                len(needed_blocks_loc), len(all_blocks), task_n)
            for block in sorted(needed_blocks_loc.keys()):
                html += '%s @ %s<br>' % (block,
                                         ','.join(sorted(needed_blocks_loc[block])))
            RI.set_blocks(wfn, task_n, needed_blocks_loc)

        html += '<a name=FILE></a>'
        if len(files_and_loc_in_dbs.keys()):
            html += "<b>%s Files in block for %s</b><br>" % (
                len(files_and_loc_in_dbs.keys()), task_n)
            #print files_and_loc_in_dbs
            RI.set_files(wfn, task_n, files_and_loc_in_dbs)
        if len(files_and_loc_notin_dbs.keys()):
            html += "<b>%s Files in no block for %s</b><br>" % (
                len(files_and_loc_notin_dbs.keys()), task_n)
        ...
        check_files = [f for f in files_and_loc_notin_dbs.keys()
                       if '/store' in f]
        check_location_mode = options.check_location
        if check_location_mode is None and len(check_files) < 10:
            # fallback to xrootd if nothing was said, and there aren't many
            # files
            check_location_mode = 'xrootd'
        files_status, files_location = checkFilesLocations(
            check_files, mode=check_location_mode)
        files_html = ""
        existing_html = ""
        lost_html = ""
        ...
        for f in sorted(display_files):
            ...
            for s in files_and_loc_notin_dbs[f]:
                expected_files[s] += 1
            for s in files_and_loc_notin_dbs[f]:
                missing_files[s] += (1 if (readable == 1 and s not in actual_locs) or (
                    readable == 0 and s not in actual_locs) else 0) if readable != -1 else 0

            html_line = '<font color="%s">%s</font> <b>@</b> %s <br>' % (color, f, ','.join(['<font color="%s">%s</font>' % (
                ('green' if site in actual_locs else 'red') if readable != -1 else 'black', site) for site in all_locs]))
            if not separate_h:
        ...
        # the table it not relevant without upto date information on file
        # locations
        if any(missing_files.values()) and False:
            ...
            for site in sorted(expected_files.keys()):
                html += "<tr bgcolor=%s><td>%s</td><td>%d</td><td>%d</td></tr>" % (
                    "red" if missing_files[site] else "", site, expected_files[site], missing_files[site])
            html += "</table><br>"
...
def parse_top(url, options=None):
        ...
        for agent in diagnose_by_agent_by_site[task]:
            for site in diagnose_by_agent_by_site[task][agent]:
                diagnose_by_site[task][site] = add_condensed(
                    diagnose_by_site[task][site], diagnose_by_agent_by_site[task][agent][site])
                diagnose_by_agent[task][agent] = add_condensed(
                    diagnose_by_agent[task][agent], diagnose_by_agent_by_site[task][agent][site])
                diagnose[task] = add_condensed(
                    diagnose[task], diagnose_by_agent_by_site[task][agent][site])
    ...
    top_cooloff = dict(sorted([(t,
                                i.get('cooloff_f',
                                      0)) for t,
                               i in diagnose.items()],
                              key=lambda o: o[1],
                              reverse=True)[:top_N])
    top_failure = dict(sorted([(t,
                                i.get('failure_f',
                                      0)) for t,
                               i in diagnose.items()],
                              key=lambda o: o[1],
...
if __name__ == "__main__":
    ...
    parser.add_option(
        '--fast',
        help="Retrieve from cache and no logs retrieval",
        action="store_true",
        default=False)
    parser.add_option(
        '--ongoing',
        help="Retrieve for all ongoing",
        action="store_true",
        default=False)
    parser.add_option(
        '--manual',
        help="Retrieve for all workflows needing help",
        action="store_true",
        default=False)
    parser.add_option(
        '--top',
        help="Retrieve the top N offenders",
        action="store_true",
        default=False)
    parser.add_option(
        '--from_status',
        help="The coma separated list of status keywords",
        default="")
    parser.add_option(
        '--workflow',
        '-w',
        help="The workflow to make the error report of",
        default=None)
