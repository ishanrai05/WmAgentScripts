...
def time_point(label="", sub_lap=False, percent=None):
    ...
    if percent:
        print "[lockor] finishing in about %.2f [s]" % (
...
# add an addHoc list of things to lock. empyting this list would result in
# unlocking later
try:
...
print "additional lock for workflows not knonw to unified", len(
    also_locking_from_reqmgr)
...
for dataset in already_locked - newly_locking - also_locking_from_reqmgr:
    try:
        ...
        if tier in tiers_keep_on_disk:
            ...
            if use_mcm:
                requests_using = mcm.getA(
                    'requests',
                    query='input_dataset=%s' %
                    dataset)
                pending_requests_using = filter(
                    lambda req: req['status'] not in [
                        'submitted', 'done'], requests_using)
                ...
                outs = session.query(Output).filter(
                    Output.datasetname == dataset).all()
                delay_days = 30
                delay = delay_days * 24 * 60 * 60  # 30 days
                if outs:
                    odb = outs[0]
                    # all([(now-odb.date) > delay for odb in outs]):
                    if (now - odb.date) > delay:
                        unlock = True
                        print "unlocking", dataset, "after", (
                            now - odb.date) / 24 * 60 * 60, "[days] since announcement, limit is", delay_days, "[days]"
                    else:
                        unlock = False
        ...
        if unlock:
            ...
            for creator in creators:
                for wfo in session.query(Workflow).filter(
                    ...
                    if 'unlock' not in wfo.status and any(
                            [wfo.status.startswith(key) for key in ['done', 'forget']]):
                        wfo.status += '-unlock'
...
# then for all that would have been invalidated from the past, check
# whether you can unlock the wf based on output
