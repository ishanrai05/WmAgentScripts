...
def checkor(url, spec=None, options=None):
    ...
    for site in custodials:
        ...
        for group,items in list(items_at.items()):
    ...
    def check(self):
        ...
        for output in wfi.request['OutputDatasets']:
            ...
            if tier in set(UC.get('tiers_to_rucio_relval')) | set(UC.get('tiers_to_rucio_nonrelval')):
                ...
                all_blocks = set([x[0] for x in phedex_filecount_pb]) | set([x[0] for x in rucio_filecount_pb])
                ...
                if len(all_blocks) == len(all_filecount_pb):
                    phedex_presence[output] = sum([x[1] for x in all_filecount_pb])
                else:
        ...
        one_output_not_in_phedex = any([Nfiles==0 for Nfiles in list(phedex_presence.values())])
        ...
        if not all([len(sites)!=0 for sites in list(disk_copies.values())]):
