...
def get_campaign(output, wfi):
    ...
    if 'Campaign' in wfi.request:
        wf_campaign = wfi.request['Campaign']
    try:
...
def checkor(url, spec=None, options=None):
    ...
    def time_point(label="", sub_lap=False, percent=None):
        ...
        if percent:
            print "[checkor] finishing in about %.2f [s]" % (
                (now - time_point.start) / percent)
    ...
    standings = session.query(Workflow).filter(
        Workflow.status.startswith('assistance')).all()

    # intersect with what is actually in completed status in request manager
    # now
    ...
    if options.update:
        ...
        wfs.extend(filter(lambda wfo: wfo.name not in all_completed, runnings))
    ...
    if options.review:
        ...
        these = filter(lambda wfo: 'custodial' not in wfo.status, standings)
        if options.recovering:
            print "review-recovering is on: checking only the workflows that had been already acted on"
            these = filter(lambda wfo: 'manual' not in wfo.status, these)
            wfs.extend(these)
        if options.manual:
    ...
    if JC:
        ...
        to_check = JC.find(
            {'text': force_complete_jira_string, "status": "!CLOSED"})
        for j in to_check:
            jira = JC.get(j.key)
            for c in jira.fields.comment.comments:
                if force_complete_jira_string in c.body:
                    ...
                    keyword = c.body[(c.body.find(
                        force_complete_jira_string) + len(force_complete_jira_string)):].split()[0]
        ...
        for j in to_check:
            ...
            for c in jira.fields.comment.comments:
                if bypass_jira_string in c.body:
                    ...
                    prepid = jira.fields.summary.split()[0]
                    keyword = c.body[(c.body.find(
                        bypass_jira_string) + len(bypass_jira_string)):].split()[0]
                    if keyword and user in actors:
    ...
    if max_per_round and not spec:
        ...
        def rank(wfn):
            return all_completed_plus.index(
                wfn) if wfn in all_completed_plus else 0
    ...
    if n_wfs and float(failed_threads / n_wfs) > 0:
        sendLog(
            ...
            (failed_threads, n_wfs), level='critical')
        # remove once it's all good
        sendEmail(
            'checkor', '%d/%d threads have failed, better check this out' %
            (failed_threads, n_wfs))
    ...
    if wfs:
        now = time.mktime(time.gmtime())
        time_spend_per_workflow = float(
            now - time_point.start) / float(float(len(wfs)))
        print "Average time spend per workflow is", time_spend_per_workflow
        # set a threshold to it
        if time_spend_per_workflow > 120:
            sendLog(
                'checkor',
                'The module checkor took %.2f [s] per workflow' %
                (time_spend_per_workflow),
                level='critical')
    ...
    if not spec and in_manual != 0:
        ...
        some_details += '\n'.join(['%3d in status %s' % (count_statuses[st], st)
    ...
    if os.path.isfile('.checkor_stop'):
        ...
        sendEmail(
            'checkor',
            'Checkor loop was shortened artificially using .checkor_stop')
...
class CheckBuster(threading.Thread):
    ...
    def check(self):
        ...
        timeout_for_damping_fraction = UC.get('damping_fraction_pass')
        damping_time = UC.get('damping_fraction_pass_rate')
        damping_fraction_pass_max = float(
            UC.get('damping_fraction_pass_max') / 100.)
        acdc_rank_for_truncate = UC.get('acdc_rank_for_truncate')
        ...
        time_point(
            "Starting checkor with %s Progress [%d/%d]" %
            (wfo.name,
             iwfo,
             will_do_that_many),
            percent=float(iwfo) /
        ...
        if wfo.wm_status in ['closed-out',
                             'announced'] and wfo.name not in exceptions:
            # manually closed-out
            wfi.sendLog(
                'checkor', "%s is already %s, setting close" %
            ...
            if wfi.isRelval():
                wfi.sendLog(
                    'checkor', "%s is %s, but will not be set in trouble to find a replacement." %
                    (wfo.name, wfo.wm_status))
                self.to_status = 'forget'
            else:
                self.to_status = 'trouble'
                wfi.sendLog(
                    'checkor', "%s is in trouble %s" %
                    (wfo.name, wfo.wm_status))
            return
        ...
        if wfo.wm_status != 'completed' and wfo.name not in exceptions:
            # for sure move on with closeout check if in completed
            wfi.sendLog(
                'checkor', "no need to check on %s in status %s" %
                (wfo.name, wfo.wm_status))
            return

        # session.commit()
        # sub_assistance="" # if that string is filled, there will be need for
        # manual assistance
        existing_assistance_tags = set(
            wfo.status.split('-')[1:])  # [0] should be assistance
        assistance_tags = set()
        ...
        for bypass in bypasses:
            ...
            if bypass == wfo.name:
                wfi.sendLog(
                    'checkor', "we can bypass checks on %s because of keyword %s " %
                    (wfo.name, bypass))
                bypass_checks = True
                break
        pids = wfi.getPrepIDs()
        ...
        for force in forcings:
            if force in pids:
                wfi.sendLog(
                    'checkor', "we can bypass checks and force complete %s because of prepid %s " %
                    (wfo.name, force))
                bypass_checks = True
                force_by_mcm = True
                break
        ...
        delays = [(now_s - completed_log[-1]['UpdateTime']) / (60. * 60. * 24.) if completed_log else 0 for completed_log in [filter(
            lambda change: change["Status"] in ["completed"], m['RequestTransition']) for m in wfi.getFamilly(details=True, and_self=True)]]
        delays = filter(None, delays)
        # take the shortest time since a member of the familly completed
        min_completed_delays = min(delays)
        completed_log = filter(
            lambda change: change["Status"] in ["completed"],
            wfi.request['RequestTransition'])
        delay = (now_s - completed_log[-1]['UpdateTime']) / \
            (60. * 60. * 24.) if completed_log else 0  # in days
        completed_delay = delay  # this is for the workflow itself
        #onhold_completed_delay = delay
        # this is for any workflows (itself, and ACDC)
        ...
        tiers_with_no_check = copy.deepcopy(
            UC.get('tiers_with_no_check'))  # dqm*
        vetoed_custodial_tier = copy.deepcopy(UC.get(
            'tiers_with_no_custodial')) if not wfi.isRelval() else []  # no veto for relvals
        ...
        for out, c in campaigns.items():
            if c in CI.campaigns and 'custodial_override' in CI.campaigns[c]:
                if isinstance(CI.campaigns[c]['custodial_override'], list):
                    ...
                    # would remove DQM from the vetoed check
                    tiers_with_no_check = list(
                        set(tiers_with_no_check) - set(CI.campaigns[c]['custodial_override']))
                elif CI.campaigns[c]['custodial_override'] == 'notape':
                    vetoed_custodial_tier = sorted(
                        set([o.split('/')[-1] for o in wfi.request['OutputDatasets']]))

        print campaigns

        check_output_text = "Initial outputs:" + \
            ",".join(sorted(wfi.request['OutputDatasets']))
        ...
        check_output_text += "\nWill check on:" + \
            ",".join(sorted(wfi.request['OutputDatasets']))
        check_output_text += "\ntiers out:" + \
            ",".join(sorted(tiers_with_no_check))
        check_output_text += "\ntiers no custodial:" + \
            ",".join(sorted(vetoed_custodial_tier))
        ...
        for member in familly:
            ...
            if member['RequestDate'] < wfi.request['RequestDate']:
                continue
            if member['PrepID'] != wfi.request['PrepID']:
                continue
            # if 'OriginalRequestName' in member and (not 'ACDC' in
            # member['OriginalRequestName']) and member['OriginalRequestName']
            # != wfo.name: continue
            if member['RequestStatus'] is None:
                continue

            if not set(
                    member['OutputDatasets']).issubset(
                    set(expected_outputs)):
                if not member['RequestStatus'] in [
                        'rejected-archived', 'rejected', 'aborted', 'aborted-archived']:
            ...
            if member['RequestStatus'] in [
                ...
                if (force_by_mcm or force_by_user) and not forced_already:
                    wfi.sendLog(
                        'checkor',
                        '%s is being forced completed while recovering' %
                        wfo.name)
                    wfi.notifyRequestor(
                        "The workflow %s was force completed" %
                        wfo.name, do_batch=False)
        ...
        if acdc_failed:
            sendLog('checkor', 'For %s, ACDC %s failed' %
                    (wfo.name, ','.join(acdc_failed)), level='critical')
        if acdc_bads:
            sendLog(
                'checkor',
                'For %s, ACDC %s is inconsistent, preventing from closing or will create a mess.' %
                (wfo.name,
                 ','.join(acdc_bads)),
                level='critical')
        ...
        # this will create funky issue with LHEGS where the two output of a
        # task can have different expected #of events ... as predicted this is
        ...
        for task, outs in output_per_task.items():
            for out in outs:
                ...
                task_outputs[out] = wfi.request.get(
                    task, {}).get('TaskName', task)

        # lumi_expected is constant over all tasks
        # event_expected is only valid for the "first task" and one needs to
        # consider all efficiency on the way
        event_expected, lumi_expected = wfi.request.get(
            'TotalInputEvents', None), wfi.request.get(
            'TotalInputLumis', None)

        if event_expected is None:
            sendEmail(
                "missing member of the request",
                "TotalInputEvents is missing from the workload of %s" %
                wfo.name)
            sendLog(
                'checkor',
                "TotalInputEvents is missing from the workload of %s" %
        ...
        while True:
            ...
            if tt in wfi.request:
                ...
                tname_dict[tname] = tt
                if 'Input%s' % ttype not in wfi.request[tt] and 'RequestNumEvents' in wfi.request[tt]:
        ...
        for output in wfi.request['OutputDatasets']:
            ...
            # fractions_truncate_recovery[output] = 0.98 ## above this
            # threshold if the request will pass stats check, we close the acdc
            c = campaigns[output]
            if c in CI.campaigns and 'earlyannounce' in CI.campaigns[c]:
                wfi.sendLog(
                    'checkor', "Allowed to announce the output %s over %.2f by campaign requirement" %
            ...
            if c in CI.campaigns and 'fractionpass' in CI.campaigns[c]:
                if isinstance(CI.campaigns[c]['fractionpass'], dict):
                    tier = output.split('/')[-1]
                    priority = str(wfi.request['RequestPriority'])
                    # defined per tier
                    fractions_pass[output] = CI.campaigns[c]['fractionpass'].get(
                        'all', default_pass)
                    if tier in CI.campaigns[c]['fractionpass']:
                        tier_pass_content = CI.campaigns[c]['fractionpass'][tier]
                        if isinstance(tier_pass_content, dict):
                            fractions_pass[output] = CI.campaigns[c]['fractionpass'][tier].get(
                                'all', default_pass)
                ...
                wfi.sendLog(
                    'checkor', "overriding fraction to %s for %s by campaign requirement" %
            ...
            weight_pass = delay
            weight_under_pass = 2 * \
                delay if int(wfi.request['RequestPriority']) < 80000 else 0.  # allow to drive it below the threshold
            weight_under_pass = 0.  # otherwise we can end-up having request at 94% waiting for 95%
            fractions_truncate_recovery[output] = (
                fractions_pass[output] * weight_pass + 1. * weight_full + pass_percent_below * weight_under_pass) / (
                weight_pass + weight_full + weight_under_pass)

            if c in CI.campaigns and 'truncaterecovery' in CI.campaigns[c]:
                wfi.sendLog(
                    'checkor', "Allowed to truncate recovery of %s over %.2f by campaign requirement" %
                    (out, CI.campaigns[c]['truncaterecovery']))
                fractions_truncate_recovery[output] = CI.campaigns[c]['truncaterecovery']
            else:
                wfi.sendLog(
                    'checkor', "Can truncate recovery of %s over %.2f" %
                    (out, fractions_truncate_recovery[output]))

            if fractions_truncate_recovery[output] < fractions_pass[output]:
                print "This is not going to end well if you truncate at a lower threshold than passing", fractions_truncate_recovery[
                    output], fractions_pass[output]
                # floor truncating
        ...
        fraction_damping = min(0.01 *
                               (max(completed_delay -
                                    timeout_for_damping_fraction, 0) /
                                damping_time), damping_fraction_pass_max)
        ...
        for out in fractions_pass:
            # strictly ones cannot be set less than one
            if fractions_pass[out] != 1.0 and fraction_damping:
        ...
        if acdc_order > acdc_rank_for_truncate:
            # there is high order acdc on-going. chop the output at the pass
            # fraction
            wfi.sendLog(
                'checkor',
                'Truncating at pass threshold because of ACDC of rank %d' %
                acdc_order)
        ...
        for out in fractions_pass:
            ...
            for a in ancestors:
                # multiply by fraction of all ancestors
                descending_pass *= fractions_pass.get(a, 1.)
        ...
        for output in wfi.request['OutputDatasets']:
            ...
            events_per_lumi[output] = event_count / \
                float(lumi_count) if lumi_count else 100
            percent_completions[output] = 0.

            if lumi_expected:
                wfi.sendLog(
                    'checkor', "lumi completion %s expected %d for %s" %
            ...
            output_event_expected = event_expected_per_task.get(
                task_outputs.get(output, 'NoTaskFound'), event_expected)
            ...
            if output_event_expected:
                ...
                if e_fraction > percent_completions[output]:
                    ...
                    wfi.sendLog(
                        'checkor', "overiding : event completion real %s expected %s for %s" %
        ...
        pass_stats_check = dict([(out, bypass_checks or (
            percent_completions[out] >= fractions_pass[out])) for out in fractions_pass])
        ...
        # need to come to a way to do this "fast" so that it can be done more
        # often
        if not all(pass_stats_check.values()):  # and False:
            n_runs = 1
            # should recalculate a couple of things to be able to make a better
            # check on expected fraction
            for p in prim:
                nr = getDatasetRuns(p)
                if len(nr) > 1:
                    print "fecthing input lumis and files for", p
                    lumis_per_run[p], files_per_rl[p] = getDatasetLumisAndFiles(
                        p, runs=rwl, lumilist=lwl)
            ...
            for out in pass_stats_check:
                if prim and n_runs > 1:
                    ...
                    lumis_per_run[out], files_per_rl[out] = getDatasetLumisAndFiles(
                        out)
                    fetched[out] = True
                    ...
                    all_runs = sorted(
                        set(lumis_per_run[a_primary].keys() + lumis_per_run[out].keys()))
                    for run in all_runs:
                        denom = lumis_per_run[a_primary].get(run, [])
                        numer = lumis_per_run[out].get(run, [])
                        if denom:
                            fraction_per_run[run] = float(
                                len(numer)) / len(denom)
                        else:
                    ...
                    if fraction_per_run:
                        ...
                        average_fraction = sum(
                            fraction_per_run.values()) / len(fraction_per_run.values())
        ...
        pass_stats_check_to_truncate_recovery = dict(
            [(out, (percent_avg_completions[out] >= fractions_truncate_recovery[out])) for out in fractions_truncate_recovery])
        ...
        if pass_stats_check_to_announce and all(
                pass_stats_check_to_announce.values()):
            wfi.sendLog(
                'checkor',
                ...
                (json.dumps(
                    ...
                    indent=2)))
            assistance_tags.add(
                'announced' if 'announced' in wfo.status else 'announce')
        ...
        if not all(pass_stats_check.values()):
            ...
            if possible_recoveries == []:
                ...
                sendLog(
                    'checkor',
                    '%s is not completed, but has nothing to be recovered, passing along ?' %
                    wfo.name,
                    level='critical')
                # do not bypass for now, until Alan understands why we are
                # loosing ACDC docs
                bypass_checks = True
            else:
                wfi.sendLog(
                    'checkor', '%s is not completed.\nCurrent stats:\n%s \nRequired stats:\n%s' %
                    (wfo.name, json.dumps(
                        percent_completions, indent=2), json.dumps(
                        fractions_pass, indent=2)))
            ...
            wfi.sendLog(
                'checkor', 'passing stats check \nCurrent stats:\n%s \nRequired stats:\n%s' %
                (json.dumps(
                    percent_completions, indent=2), json.dumps(
                    fractions_pass, indent=2)))

        if acdc and all(pass_stats_check.values()) and all(
                pass_stats_check_to_truncate_recovery.values()):
            print "This is essentially good to truncate"

            wfi.sendLog(
                'checkor',
                'Will force-complete the recovery to speed things up')
            forceComplete(url, wfi)
        ...
        if over_100_pass and all(pass_stats_check_over_completion.values()):
            ...
            wfi.sendLog(
                'checkor',
                'Should force-complete the request going over 100%')
        ...
        for output in wfi.request['OutputDatasets']:
            ...
            if wfi.request['RequestType'] in ['ReDigi', 'ReReco']:
                lumi_upper_limit[output] = -1

        if (not wfi.isRelval()) and (not prim) and any(
                [(events_per_lumi[out] <= lumi_lower_limit) for out in events_per_lumi if not out.endswith(('DQMIO', 'ALCARECO'))]):
            wfi.sendLog(
                'checkor',
        ...
        if any([(lumi_upper_limit[out] > 0 and events_per_lumi[out]
                 >= lumi_upper_limit[out]) for out in events_per_lumi]):
            wfi.sendLog('checkor', "%s has large lumisections\n%s" %
                        (wfo.name, json.dumps(events_per_lumi, indent=2)))
        ...
        custodial_presences = {}
        for output in wfi.request['OutputDatasets']:
            custodial_presences[output] = [
                s for s in any_presence[output] if 'MSS' in s]
            custodial_locations[output] = phedexClient.getCustodialSubscriptionRequestSite(
                output)
        ...
        for output in wfi.request['OutputDatasets']:
            ...
            if tier in set(
                    UC.get('tiers_to_rucio_relval')) | set(
                    UC.get('tiers_to_rucio_nonrelval')):
                ...
                phedex_filecount_pb = phedexClient.getFileCountPerBlock(
                    url, output)
                rucio_filecount_pb = rucioClient.getFileCountPerBlock(output)
                all_filecount_pb = set(
                    phedex_filecount_pb) | set(rucio_filecount_pb)
                all_blocks = set(map(lambda x: x[0], phedex_filecount_pb)) | set(
                    map(lambda x: x[0], rucio_filecount_pb))
                ...
                if len(all_blocks) == len(all_filecount_pb):
                    phedex_presence[output] = sum(
                        map(lambda x: x[1], all_filecount_pb))
                else:
                    # TODO: to check if we need to rise a higher level of alarm
                    # here.
                    msg = "There are inconsistences of number of files per block"
                    msg += "between Phedex and Rucio for dataset: {}".format(
                        output)
                    wfi.sendLog('checkor', msg)
                    phedex_presence[output] = 0
                    # we do not announce this output untill the discrepancy
                    # from above is resolved
                del(all_filecount_pb)
                del(all_blocks)
            else:
                phedex_presence[output] = phedexClient.getFileCountDataset(
                    url, output)

        one_output_not_in_phedex = any(
            [Nfiles == 0 for Nfiles in phedex_presence.values()])
        if one_output_not_in_phedex and 'announce' in assistance_tags:
            wfi.sendLog(
                'checkor',
                'No files in phedex yet, no good to announce')
        ...
        for output in wfi.request['OutputDatasets']:
            ...
            dbs_invalid[output] = dbs3Client.getFileCountDataset(
                output, onlyInvalid=True)

        # prepare the check on having a valid subscription to tape
        out_worth_checking = [out for out in custodial_locations.keys(
        ) if out.split('/')[-1] not in vetoed_custodial_tier]
        size_worth_checking = sum([(getDatasetSize(out) / 1023. if not wfi.isRelval() else 0.)
                                   for out in out_worth_checking])  # size in TBs of all outputs
        size_worht_going_to_ddm = sum([getDatasetSize(
            out) / 1023. for out in out_worth_checking if out.split('/')[-1] in to_ddm_tier])  # size in TBs of all outputs
        all_relevant_output_are_going_to_tape = all(map(lambda sites: len(
            sites) != 0, [custodial_locations[out] for out in out_worth_checking]))

        show_N_only = 10  # number of files to include in a report log

        time_point("dbs file count", sub_lap=True)

        if not all([dbs_presence[out] == (dbs_invalid[out] + phedex_presence[out])
                    for out in wfi.request['OutputDatasets']]) and not options.ignorefiles:
            ...
            if 'recovering' not in assistance_tags:
                ...
                for out in dbs_presence:
                    dbs_filenames, phedex_filenames, missing_phedex, missing_dbs = getDatasetFiles(
                        url, out)

                    # Corrections to the lists of files present in Phedex for
                    # the data Tiers managed by Rucio
                    _, dsn, process_string, tier = output.split('/')
                    if tier in set(
                            UC.get('tiers_to_rucio_relval')) | set(
                            UC.get('tiers_to_rucio_nonrelval')):
                        # Here recalculating the filenames as a union of the
                        # phedex_files | rucio_files
                        all_filenames = set(phedex_filenames) | set(
                            rucioClient.getFileNamesDataset(out))
                        missing_phedex = list(
                            set(dbs_filenames) - all_filenames)
                    ...
                    if missing_phedex:
                        wfi.sendLog('checkor', "These %d files are missing in phedex, or extra in dbs, showing %s only\n%s" % (
                            len(missing_phedex), show_N_only, "\n".join(missing_phedex[:show_N_only])))
                        were_invalidated = sorted(
                            set(missing_phedex) & set(TMDB_invalid))
                        if were_invalidated:
                            ...
                            dbs3Client.setFileStatus(
                                were_invalidated, newstatus=0)
                    ...
                    if missing_dbs:
                        ...
                        were_invalidated = sorted(
                            set(missing_dbs) & set(TMDB_invalid))
                        if were_invalidated:
                            wfi.sendLog('checkor', "These %d files were invalidated globally,showing %d only\n%s" % (
                                len(were_invalidated), show_N_only, "\n".join(were_invalidated[:show_N_only])))
            # if not bypass_checks:
        ...
        # check for duplicates prior to making the tape subscription ## this is
        # quite expensive and we run it twice for each sample
        if (not stop_duplicate_check) and (is_closing or bypass_checks) and (
                not all_relevant_output_are_going_to_tape):
            ...
            for output in wfi.request['OutputDatasets']:
                ...
                if output not in lumis_per_run or output not in files_per_rl:
                    lumis_per_run[output], files_per_rl[output] = getDatasetLumisAndFiles(
                        output)
                    fetched[output] = True

                lumis_with_duplicates[output] = [
                    rl for (rl, files) in files_per_rl[output].items() if len(files) > 1]
            ...
            if is_closing and any(duplications.values()):
                ...
                #duplicate_notice += '\n'
                # TO DO, make the file list invalidation analysis. to find the
                # files with least number of lumis
                duplicate_notice += "This number of lumis are duplicated\n"
                duplicate_notice += json.dumps(
                    dict([(o, len(badl)) for o, badl in lumis_with_duplicates.items()]), indent=2)
                wfi.sendLog('checkor', duplicate_notice)

                bad_files = {}
                for out in duplications:
                    bad_files[out] = duplicateAnalyzer(
                    ).files_to_remove(files_per_rl[out])
                    if bad_files[out]:
                        duplicate_notice = "These files %d will be invalidated, showing %d only\n%s" % (
                            len(bad_files[out]), show_N_only, "\n".join(bad_files[out][:show_N_only]))
                        wfi.sendLog('checkor', duplicate_notice)
                        # sending the list is not possible
                        duplicate_notice += json.dumps(
                            sorted(bad_files[out]), indent=2)
                ...
                for out, bads in bad_files.items():
                    if bads:
                        ...
                        # invalidate in dbs in the meantime so that the dataset
                        # goes into filemismatch category
                        dbs3Client.setFileStatus(bads, newstatus=0)
                    pass
        ...
        if is_closing and not all_relevant_output_are_going_to_tape:
            ...
            if not force_custodial and custodial and float(
                    SI.storage[custodial]) < size_worth_checking:
            ...
            tape_size_limit = options.tape_size_limit if options.tape_size_limit else UC.get(
                "tape_size_limit")
            ...
            if not custodial and prim and use_parent_custodial:
                ...
                parents_custodial = phedexClient.getCustodialSubscriptionRequestSite(
                    parent_dataset)
                ...
                if len(parents_custodial):
                    ...
                    sendLog(
                        'checkor',
                        "Please take a look at %s for missing custodial location" %
                        parent_dataset)
            ...
            if custodial and size_worht_going_to_ddm > tape_size_limit:
                wfi.sendLog(
                    'checkor', "The total output size (%s TB) is too large for the limit set (%s TB)" %
            ...
            if picked_a_tape:
                ...
                # remember how much you added this round already ; this stays
                # locally
                SI.storage[custodial] -= size_worth_checking
                # register the custodial request, if there are no other big
                # issues
                holding = []
                for output in out_worth_checking:
                    if not len(custodial_locations[output]):
                        if phedex_presence[output] >= 1:
                            wfi.sendLog(
                                'checkor', 'Using %s as a tape destination for %s' %
                                (custodial, output))
                            self.custodials[custodial].append(output)
                            if group:
                                self.custodials[custodial][-1] += '@%s' % group
                            ...
                            # there is no file in phedex, but the actual stats
                            # check is OK, meaning we are good to let this pass
                            # along. the dbs/phedex check will pick this up
                            # anyways otherwise
                            wfi.sendLog(
                                'checkor',
                                'No file in phedex for %s, but statistics check passed' %
                                output)
                        else:
                            # does not look good
                            wfi.sendLog(
                                'checkor',
                                'No file in phedex for %s, not good to add to custodial requests' %
                                output)
                            holding.append(output)
                if not holding:
                    is_closing = True
        ...
        for output in wfi.request['OutputDatasets']:
            disk_copies[output] = [
                s for s in any_presence[output] if (
                    'MSS' not in s) and (
                    'Buffer' not in s)]
        ...
        if not all([(dbs_invalid[out] <= int(fraction_invalid * dbs_presence[out]))
                    for out in wfi.request['OutputDatasets']]) and not options.ignoreinvalid:
        ...
        for output in wfi.request['OutputDatasets']:
            if output not in put_record['datasets']:
                put_record['datasets'][output] = {}
            ...
            rec['percentage'] = math.floor(
                percent_completions[output] * 10000) / 100.  # round down
            rec['fractionpass'] = math.floor(
                fractions_pass.get(output, 0) * 10000) / 100.
            rec['duplicate'] = duplications[output] if output in duplications else 'N/A'
            rec['phedexReqs'] = float('%.2f' % any_presence[output][custodial_presences[output][0]][1]) if len(
                custodial_presences[output]) != 0 else 'N/A'
            rec['closeOutDataset'] = is_closing
            rec['transPerc'] = float('%.2f' % any_presence[output][disk_copies[output][0]][1]) if len(
                disk_copies[output]) != 0 else 'N/A'
            rec['correctLumis'] = int(events_per_lumi[output]) if (
                events_per_lumi[output] > lumi_upper_limit[output]) else True
            rec['missingSubs'] = False if len(custodial_locations[output]) == 0 else ','.join(
                list(set(custodial_locations[output])))
        ...
        if wfi.request['RequestType'] == 'ReReco':
            try:
                ...
                os.system(
                    'python Unified/lumi_summary.py %s %d > /dev/null' %
                    (wfi.request['PrepID'], 0 if all(
                        fetched.values()) else 1))  # no need for fresh fetch if that has been done for all already
                os.system(
                    'python Unified/lumi_plot.py %s > /dev/null' %
                    (wfi.request['PrepID']))
                wfi.sendLog(
                    'checkor', 'Lumi summary available at %s/datalumi/lumi.%s.html' %
                    (unified_url, wfi.request['PrepID']))
            except Exception as e:
                print str(e)
        ...
        if is_closing:
            ...
            if 'recovering' in assistance_tags:
                # if active ACDC, being under threshold, filemismatch do not
                # matter
                assistance_tags = assistance_tags - \
                    set(['recovery', 'filemismatch', 'manual'])
            if 'recovery' in assistance_tags and 'recovered' in assistance_tags:
                # should not set -recovery to anything that had ACDC already
                assistance_tags = assistance_tags - \
                    set(['recovery', 'recovered'])
                # straight to manual
                assistance_tags.add('manual')
            if 'recovery' in assistance_tags and 'manual' in assistance_tags:
                # this is likely because something bad is happening, so leave
                # it to manual
                assistance_tags = assistance_tags - set(['recovery'])
                assistance_tags.add('manual')
            if 'custodial' in assistance_tags:
                assistance_tags = assistance_tags - \
                    set(['announce', 'announced'])
            if any(
                (tag in assistance_tags) for tag in [
                    'duplicates',
                    'filemismatch',
                    'agentfilemismatch']):
                assistance_tags = assistance_tags - set(['announce'])

            # that means there is something that needs to be done acdc, lumi
            # invalidation, custodial, name it
            print wfo.name, "needs assistance with", ",".join(assistance_tags)
            print wfo.name, "existing conditions", ",".join(
                existing_assistance_tags)

            #########################################
            ##### notification to requester #########
            go_notify = False
            if assistance_tags and 'manual' not in existing_assistance_tags and existing_assistance_tags != assistance_tags:
                go_notify = True
            ...
            if go_notify:
                ...
                splitlink = 'https://cmsweb.cern.ch/reqmgr/view/splitting/%s' % (
                    wfo.name)
                # notify templates
                messages = {
            ...
            if 'manual' not in wfo.status or new_status != 'assistance-recovery':
                #wfo.status = new_status
                if not options.test:
                    wfi.sendLog(
                        'checkor', 'setting %s to %s' %
                        (wfo.name, new_status))
                    # session.commit()
...
if __name__ == "__main__":
    ...
    parser.add_option(
        '-t',
        '--test',
        help='Only test the checkor',
        action='store_true',
        default=False)
    parser.add_option(
        '--go',
        help='Does not check on duplicate process',
        action='store_true',
        default=False)
    #parser.add_option('--wait',help='Wait for another process to clear', action='store_true', default=False)
    parser.add_option(
        '--strict',
        help='Only running workflow that reached completed',
        action='store_true',
        default=False)
    parser.add_option(
        '--update',
        help='Running workflow that have not yet reached completed',
        action='store_true',
        default=False)

    parser.add_option(
        '--clear',
        help='Only the workflow that have reached custodial',
        action='store_true',
        default=False)
    parser.add_option(
        '--review',
    ...
    parser.add_option(
        '--manual',
        help='Look at the workflows in "manual"',
        action='store_true',
        default=False)

    parser.add_option(
        '--limit',
        help='The number of workflow to consider for checking',
        default=0,
        type=int)
    parser.add_option(
        '--fractionpass',
        help='The completion fraction that is permitted',
        default=0.0,
        type='float')
    parser.add_option(
        '--ignorefiles',
        help='Force ignoring dbs/phedex differences',
        action='store_true',
        default=False)
    parser.add_option(
        '--ignoreinvalid',
        help='Force ignoring high level of invalid files',
        action='store_true',
        default=False)
    parser.add_option(
        '--lumisize',
        help='Force the upper limit on lumisection',
        default=0,
        type='float')
    parser.add_option(
        '--ignoreduplicates',
        help='Force ignoring lumi duplicates',
        default=False,
        action='store_true')
    parser.add_option(
        '--tape_size_limit',
        help='The limit in size of all outputs',
        default=0,
        type=int)
    parser.add_option(
        '--html',
        help='make the monitor page',
        action='store_true',
        default=False)
    parser.add_option(
        '--no_report',
        help='Prevent from making the error report',
        action='store_true',
        default=False)
    parser.add_option(
        '--threads',
        help='Number of threads for processing workflows',
        default=10,
        type=int)
