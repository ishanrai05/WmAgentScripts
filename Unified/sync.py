...
parser.add_argument(
    ...
    action='store',
    required=True,
    help='The workflow name to change the status of')
parser.add_argument(
    '-s',
    type=str,
    action='store',
    required=True,
    help='The new status to be set')
parser.add_argument(
    '--comments',
    type=str,
    action='store',
    required=True,
    help='Reason for manual intervention, to be registered in the Unified log')
...
if __name__ == "__main__":
    ...
    for wf in session.query(Workflow).filter(
        ...
        if spec and spec not in wf.name:
            continue
        ...
        wfi.sendLog('sync', 'Setting status from {} to {} ({}). Reason: {}'.format(
            old_status, wf.status, wf.wm_status, comment))  # Please always put reason
    session.commit()
