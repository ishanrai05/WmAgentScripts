...
def batchor(url):
    ...
    wfs = filter(lambda r: r['SubRequestType'] ==
                 'RelVal' if 'SubRequestType' in r else False, all_wfs)
    # need a special treatment for those
    hi_wfs = filter(
        lambda r: r['SubRequestType'] == 'HIRelVal' if 'SubRequestType' in r else False,
        all_wfs)
    ...
    for campaign in by_campaign:
        ...
        print "Campaign %s correctly created in ReqMgr2: %s" % (
            wmcoreCamp['CampaignName'], res)
    ...
    for new_campaign in list(set(add_on.keys()) -
                             set(CI.all(c_type='relval'))):
...
This is an automated message""" % (new_campaign,
    ...
    for old_campaign in CI.all(c_type='relval'):
        ...
        if not all_in_batch:
            continue
        is_batch_done = all(map(lambda s: s not in ['completed',
                                                    'force-complete',
                                                    'running-open',
                                                    'running-closed',
                                                    'acquired',
        ...
        if is_batch_done:
            ...
            print "Campaign %s correctly deleted in ReqMgr2: %s" % (
                old_campaign, res)
