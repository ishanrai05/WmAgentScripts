...
for wf in wfs:
    ...
    if not input_json:
        ...
        for r in input_json:
            input_rl.extend([(int(r), l) for l in input_json[r]])
    ...
    if 'errors' in s:
        ...
        for task in errors:
            ...
            for etype in errors[task]:
                ...
                if etype not in ['cmsRun1', 'cmsRun2', 'stageOut1']:
                    continue
                if not isinstance(errors[task][etype], dict):
                    continue
                for ecode in errors[task][etype]:
                    # if ecode != '134': continue
                    #print "\t\t",ecode
                    if int(ecode) in ecode_ban:
                        continue
                    eruns = []
                    details = ""
                    types = ""
                    ...
                    affected = filter(
                        lambda t: t[1], [
                            (out, set(
                                missing_rl[out]) & seruns) for out in affected_outputs])
                    #print "affected",affected
                    for ds, affected_ls in affected:
                        for ls in affected_ls:
                            errors_by_lb[ds][ls].add(
                                (date, str(task), str(etype), int(ecode), str(details), str(types)))
    else:
...
open(
    ...
    (monitor_dir, pid), 'w').write(
        json.dumps(
            missing_rl_with_exp, indent=2))
