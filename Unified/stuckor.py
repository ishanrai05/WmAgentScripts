...
def stuckor(url = reqmgr_url):
    ...
    for phid,datasets in list(datasets_by_phid.items()):
    ...
    report+="\nbad sources "+",".join(list(bad_sources.keys()))+"\n"
    for site,blocks in list(bad_sources.items()):
        report+="\n\n%s:"%site+"\n\t".join(['']+list(blocks))
    report+="\nbad destinations "+",".join(list(bad_destinations.keys()))+"\n"
    for site,blocks in list(bad_destinations.items()):
        report+="\n\n%s:"%site+"\n\t".join(['']+list(blocks))
    ...
    stuck_transfers = dict([(k,v) for (k,v) in list(missing_in_action.items()) if k in really_stuck_dataset])
