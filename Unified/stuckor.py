...
def stuckor(url=reqmgr_url):
    ...
    missing_in_action = json.loads(
        eosRead(
            '%s/incomplete_transfers.json' %
            monitor_dir))
    stuck_transfers = dict(
        [(k, v) for (k, v) in missing_in_action.items() if k in really_stuck_dataset])
    ...
    eosFile(
        ...
        json.dumps(
            stuck_transfers,
            indent=2)).close()
    eosFile('%s/logs/incomplete_transfers.log' %
            monitor_dir, 'w').write(report).close()
