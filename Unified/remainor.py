...
class SiteBuster(threading.Thread):
    ...
    def run(self):
        ...
        for i_ds, (size, dataset) in enumerate(ds):
            if n_ds and i_ds >= n_ds:
                break
            remainings[dataset] = {"size": size, "reasons": []}
            #print "-"*10
            if dataset not in locks:
        ...
        try:
            eosFile(
                '%s/remaining_%s.json' %
                (monitor_dir, site), 'w').write(
                json.dumps(
                    remainings, indent=2)).close()
        ...
        eosFile(
            '%s/remaining_%s.html' %
            (monitor_dir, site), 'w').write(table).close()
...
def parse(options):
    ...
    locks = [l.item.split('#')[0]
             for l in session.query(Lock).filter(Lock.lock).all()]
    waiting = {}
    stuck = {}
    missing = {}
    si = siteInfo()
    ...
    threads = []
    for site in sis:
        if spec_site and site not in spec_site:
            continue
        space = si.disk[site]
...
def summary():
    ...
    for site in RDI.sites():
        ...
        tags = [
            ...
            'missing-tape']
        for tag in tags:
            v = sum([info['size'] for ds, info in load.items()
                     if tag in info['reasons']]) / 1024.
...
if __name__ == "__main__":
    ...
    parser.add_option(
        '--subs-to-anaops',
        dest='change_dataops_subs_to_anaops_once_unlocked',
        default=False,
        action='store_true')
    parser.add_option(
        '--invalidate',
        dest='invalidate_anything_left_production_once_unlocked',
        default=False,
        action='store_true')
