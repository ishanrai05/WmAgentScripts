...
_site_re = re.compile(r"(T\d)_([A-Z]{2})_([A-Z]{1}[A-Z,a-z,_]+)$")
...
def makeHoldSiteAds(config):
    ...
    for site in held_site:
        ...
        anAd["Requirements"] = classad.ExprTree(
            str('regexp("%s",DESIRED_Sites) && HasBeenHeldFrom%s isnt true' % (site, site)))
        ...
        anAd["eval_set_DESIRED_Sites"] = classad.ExprTree(
            str('removeSite("%s",Holding_DESIRED_Sites)' % site))
...
def makeReleaseSiteAds(config):
    ...
    for site in relase_site:
        ...
        anAd["Requirements"] = classad.ExprTree(
            str('HasBeenHeldFrom%s is true' % site))
        anAd["copy_DESIRED_Sites"] = "Releasing_DESIRED_Sites"
        anAd["eval_set_DESIRED_Sites"] = classad.ExprTree(
            str('strcat(Releasing_DESIRED_Sites,",%s")' % site))
...
def makeHoldAds(config):
    ...
    for task, where in config.get('hold', {}).items():
        ...
        # where is either an empty list=all sites, or a list of sites (not
        # implemented)
        anAd = classad.ClassAd()
...
def makeReleaseAds(config):
    ...
    for task, where in config.get('release', {}).items():
        ...
        anAd["GridResource"] = "condor localhost localhost"
        exp = '(HasBeenSetHeld is true) && (target.WMAgent_SubTaskName =?= %s)' % classad.quote(
            str(task))
        anAd["Requirements"] = classad.ExprTree(str(exp))
...
def makeReadAds(config):
    for needs, tasks in config.get('read', {}).items():
        ...
        exp = classad.ExprTree(
            ...
            (task_names_escaped, int(set_read)))  # just set to a different value
        anAd["Requirements"] = classad.ExprTree(str(exp))
        anAd["set_HasBeenRouted"] = False
...
def makeOverflowAds(config):
    ...
    for workflow, tasks in config.get('modifications', {}).items():
        for taskname, specs in tasks.items():
            ...
            if "ReplaceSiteWhitelist" in specs:
                ...
                anAd["eval_set_DESIRED_Sites"] = str(
                    ",".join(specs['ReplaceSiteWhitelist']))
    ...
    for whitelist_sites, tasks in overflow_tasks.items():
        ...
        anAd["eval_set_DESIRED_Sites"] = classad.ExprTree(
            'ifThenElse(siteMapping("", []) isnt error, siteMapping(Pre_DESIRED_Sites, %s), Pre_DESIRED_Sites)' % str(
                classad.ClassAd(source_to_dests)))

        # Where possible, prefer to run at a site where the input can be read
        # locally.
        anAd['set_Rank'] = classad.ExprTree(
            "stringlistmember(GLIDEIN_CMSSite, ExtDESIRED_Sites)")
        anAd['set_HasBeenRouted'] = False
...
def makeResizeAds(config):
    ...
    for workflow, info in config.get('resizing', {}).items():
        ...
        workflows = policies.setdefault(
            (minCores, maxCores, memoryPerThread), set())
...
def makePerformanceCorrectionsAds(configs):
    ...
    for memory in m_config:
        ...
        exp = classad.ExprTree(
            ...
            (memory_names_escaped, int(memory)))  # just set to a different value
        anAd["Requirements"] = classad.ExprTree(str(exp))
...
def makeDrainAds(config=None):
    ...
    for agent in draining_agents:
        ...
        exp = 'regexp("%s",GlobalJobId) && JobStatus == 1 && JobPrio<%d' % (
            str(agent), set_To)
        anAd["Requirements"] = classad.ExprTree(str(exp))
...
def makeAdhocAds(config):
    ...
    anAd["Requirements"] = classad.ExprTree(
        "MaxWallTimeMins == 3000 && JobStatus == 1")
...
if __name__ == "__main__":
    ...
    try:
        config = json.load(
            urllib.urlopen(
                htcondor.param['UNIFIED_OVERFLOW_CONFIG']))
        makeAds(config)
