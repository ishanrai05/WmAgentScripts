    ...
    for irun, run in enumerate(runs):
        ...
        if skipInvalid:
            files = [f['logical_file_name']
                     for f in reply if f['is_file_valid'] == 1]
...
def getFileCountDataset(dataset, skipInvalid=False, onlyInvalid=False):
    ...
    if main_lfn:
        bads = filter(
            lambda f: not f['logical_file_name'].startswith(main_lfn),
            reply)
        if bads:
            print "bad files"
            print bads
    ...
    if skipInvalid:
        ...
        if main_lfn:
            reply = filter(
                lambda f: f['logical_file_name'].startswith(main_lfn), reply)
            #print "restricted"
    elif onlyInvalid:
        if main_lfn:
            reply = filter(
                lambda f: f['is_file_valid'] == 0 or not f['logical_file_name'].startswith(main_lfn),
                reply)
        else:
...
def main():
    ...
    for dataset in datasets:
        ...
        print " Creation:", datetime.datetime.fromtimestamp(
            info[1]).strftime('%Y-%m-%d %H:%M:%S')
        print " Last update:", datetime.datetime.fromtimestamp(
            info[2]).strftime('%Y-%m-%d %H:%M:%S')
