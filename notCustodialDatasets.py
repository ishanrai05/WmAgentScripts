...
def BlockPresentatSite(url, block, node):
    ...
    r1 = conn.request(
        "GET",
        '/phedex/datasvc/json/prod/blockreplicas?block=' +
        block +
        '&node=' +
        node)
    r2 = conn.getresponse()
...
def blockSetCustodiallyandCompletedSomewherElse(url, block, node):
    ...
    r1 = conn.request(
        "GET",
        '/phedex/datasvc/json/prod/subscriptions?percent_min=100&block=' +
        block +
        '&node=T1_*_MSS&custodial=y')
    r2 = conn.getresponse()
    result = json.loads(r2.read())
...
def dataSetCustodialiySubscribedSomewhereElse(url, dataset, node):
    ...
    r1 = conn.request(
        "GET",
        '/phedex/datasvc/json/prod/subscriptions?percent_min=100&dataset=' +
        dataset +
        '&node=T1_*&custodial=y')
    r2 = conn.getresponse()
    result = json.loads(r2.read())
    if len(result['phedex']['dataset']) == 0:
        return False
...
def getBlocksProducedLastWeek(url):
    ...
    r1 = conn.request(
        "GET",
        '/phedex/datasvc/json/prod/data?create_since=' +
        str(oneWeekTime) +
        '&dataset=/*/*/GEN-SIM&level=block')
    r2 = conn.getresponse()
    result = json.loads(r2.read())
