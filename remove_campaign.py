...
for key, value in campaign_db.items():
    if key == options.name:
        ...
        remove_from_campaign_config = 'python campaignsConfiguration.py --name ' + \
            options.name + ' --remove'
        ...
        with open('archive_campaigns.json', 'w') as out:
            # putting the campaign into archive_campaigns.json file
            json.dump(destination, out, indent=2)
        with open('campaigns.json', 'w') as Origin:
            # deleting the campaign from campaigns.json file
            del campaign_db[key]
