...
class phedexApi:
    ...
    def data(self, dataset='', block='', fileName='', level='block',
        ...
        dataURL = urllib.basejoin(
            self.phedexBase, "%s/%s/data" %
            (format, instance))
        ...
        if format == "json":
            try:
                ...
                return 1, " ERROR - ValueError in call to url %s : %s" % (
                    dataURL, str(e))
            if not data:
    ...
    def xmlData(self, datasets=[], instance='prod'):
        ...
        for dataset in datasets:
            check, response = self.data(
                dataset=dataset, level='file', instance=instance)
            if check:
                return 1, " Error"
    ...
    def subscribe(
        ...
        values = {
            ...
            'no_mail': noMail,
            'comments': comments}
        subscriptionURL = urllib.basejoin(
            self.phedexBase, "%s/%s/subscribe" %
            (format, instance))
    ...
    def delete(self, node='', data='', level='dataset', rmSubscriptions='y',
        ...
        deleteURL = urllib.basejoin(
            self.phedexBase, "%s/%s/delete" %
            (format, instance))
...
def convertSizeToGb(sizeTxt):
    ...
    if len(sizeTxt) < 3:
        print ' ERROR - string for sample size (%s) not compliant. EXIT.' % (
            sizeTxt)
        sys.exit(1)
...
def findExistingSubscriptions(
    ...
    r1 = conn.request(
        "GET",
        '/phedex/datasvc/json/prod/subscriptions?group=%s&node=%s&block=%s%%23*&collapse=y' %
        (group,
         sitePattern,
         dataset))
    r2 = conn.getresponse()
...
"""    webServer = 'https://cmsweb.cern.ch/'
    ...
    for line in subprocess.Popen(cmd,shell=True,stdout=subprocess.PIPE).stdout.readlines():
        ...
        try:
            sublines = re.split(r"<replica\ ",line)
...
def chooseMatchingSite(tier2Sites, nSites, sizeGb, debug):
    ...
    while len(sites) < nSites:
        ...
        cmd = 'wget http://t3serv001.mit.edu/~cmsprod/IntelROCCS/Detox/result/' + \
            site + '/Summary.txt'
...
def submitSubscriptionRequests(
        sites,
        datasets=[],
        debug=0,
        group='AnalysisOps'):
    ...
    for site in sites:
        if debug > -1:
            print r" --> phedex.subscribe(node=%s,data=....,comments=%s', \ " % (
                site, message)
            print "                      group=%s,instance='prod')" % group

        check, response = phedex.subscribe(
            node=site, data=data, comments=message, group=group, instance='prod')
        if check:
            rc = 1
...
def submitUpdateSubscriptionRequest(
        sites,
        datasets=[],
        debug=0,
        group='AnalysisOps'):
    ...
    for site in sites:
        if debug > -1:
            print r" --> phedex.updateSubscription(node=%s, \ " % (site)
            print r"                               data=%s, \ " % (dataset)
            print "                               group=%s ) " % (group)

        check, response = phedex.updateSubscription(
            node=site, dataset=dataset, group=group, instance='prod')
        if check:
            rc = 1
...
# Define the valid options which can be specified and check out the
# command line
valid = [
    'dataset=',
    'debug=',
...
size = str(sum([block['file_size']
                for block in dbsapi.listBlockSummaries(dataset=dataset)])) + 'UB'
sizeGb = convertSizeToGb(size)

# in case this is an open subscription we need to adjust sizeGb to the
# expected size
if expectedSizeGb > 0:
...
if len(tier2Sites) > 0:
    ...
    if exe:
        # make AnalysisOps the owner of all copies at Tier-1 site(s)
        rc = submitUpdateSubscriptionRequest(
            tier2Sites, datasets, debug, group=group)
        if rc != 0:
            sys.exit(1)
    else:
        print '\n -> WARNING: not doing anything .... please use  --exec  option.\n'
...
if len(siteNames) >= nCopies:
    ...
    print ' Requested %d copies at Tier-2. Only %d copies found.' % (
        nCopies, len(siteNames))
...
sites, quotas, lastCps = chooseMatchingSite(
    tier2Sites, nAdditionalCopies, sizeGb, debug)
