...
def main():
    ...
    parser.add_option(
        "-a",
        "--all",
        help="Make acdc for all tasks to be recovered",
        default=False,
        action='store_true')
    ...
    if not ((options.workflow) or (options.path) or (options.file)):
        parser.error(
            "Provide the -w Workflow Name or the -p path or the -f workflow filelist")
        sys.exit(1)
    ...
    if options.file:
        ...
        parser.error(
            "Either provide a -f filelist or a -w workflow or -p path")
        sys.exit(1)
    ...
    if not wf_and_task:
        parser.error(
            "Provide the -w Workflow Name and the -t Task Name or --all")
        sys.exit(1)
